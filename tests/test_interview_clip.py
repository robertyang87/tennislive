...
def test_顶栏比分一盘里只有赢的那个数字绿():
    ...
    # 抢七的 "3" **剥掉括号、小一档单独一个 run**（账号所有者 2026-08-31
    # 「小分不带括号」）：不染色，也不干扰谁大谁小的判断。
    ...
    score_runs2 = [(text, tags, size) for text, kind, tags, size in dropped
                   if kind == "num"]
    assert [t for t, _, _ in score_runs2] == ["7-", "6", "3 ", "4-", "6 ", "6-", "4"]
    ...
    assert "(" not in s1tb[0] and _MARK_COLOUR not in s1tb[1], (
        f"抢七小分要裸数字、不染色：{s1tb}")
    assert s1tb[2] < s1b[2], (
        f"抢七小分（{s1tb[2]}px）要比比分数字（{s1b[2]}px）小一档，"
        f"不然裸数字满号渲就读成「7-63」：{s1tb}")
