...
# `copy.html` 是**复制页**，不是中间物——它由 GitHub Pages 提供，推送正文里
# 那个按钮指的就是它，**没进仓库就是 404**（判据在下面那条
# `test_复制页写在提交之前推送排在提交之后`）。白名单按后缀写，于是复制页一落地
# 就被这条判成中间物；而 `output/**` 的提交不触发 CI，所以它红在 main 上没人看见。
# 按名字放行，别放行整个 `.html`——`cover.html` 是字体 base64 内嵌的那份，12.5 MB。
