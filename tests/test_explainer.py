...
#: 「开球之前」2026-08-17（停做前瞻）之前发过的片子。历史记录，不再是硬闸——
#: 见下面 `test_开球之前恢复新增前瞻` 的说明。表自己有自检：名字写错、
#: 或者某条其实已经不是这个栏目了，都会当场红。
...
#: 2026-08-17 之后新增的「开球之前」，逐条写清楚为什么这次是例外
#: （不能只塞名字——那样和被推翻的老豁免表是同一个毛病）。
_NEW_PREVIEWS_SINCE_20260817: dict[str, str] = {
    "zheng-you": (
        "账号所有者 2026-08-24 直接点名要的：郑钦文 VS 尤晓迪，美网资格赛"
        "第一轮，附了 app 截图（TNNS 实时赛程卡）。不是顺手挑的选题，是"
        "指名要求——下面 test_不再做比赛前瞻 的历史记录管的是「我该不该"
        "主动去找前瞻题材」，不管「他点名要一条我该不该做」。"
    ),
}
...
def test_不再做比赛前瞻():
    ...
    ⚠️ **这条规矩 2026-08-24 被账号所有者本人的新指令部分推翻了**——
    见下面 `test_开球之前恢复新增前瞻`。这里的历史记录留着不删：它管的仍然是
    「我该不该主动去找一场还没打的比赛做成前瞻」，这一层判断没有错，
    2026-08-24 那次不是推翻它，是加了一个新指令没覆盖到的情形。
    ...
    **东西没做错，是这个品类不该做。** 账号所有者看到之后一句话把它停掉。
    ...
    ⚠️ **停的是「主动去找」，不是底下那套工具**（和日报那次同一个形状）：
    `COLUMNS` 里那一条、封面的比赛坐标两行、`perishable` 那套过期逻辑全部
    留着——已经发出去的 9 条还要能渲、能查。
    """
    ...
    # 2026-08-17 之后新增、但已经在 `_NEW_PREVIEWS_SINCE_20260817` 里
    # 说清楚理由的，不算「顺手挑的」——那张表是下面那条测试的判据，这里放行。
    fresh = sorted(now - _LEGACY_PREVIEWS - set(_NEW_PREVIEWS_SINCE_20260817))
    assert not fresh, (
        f"这几条是新的「开球之前」，却没有写进"
        f"`_NEW_PREVIEWS_SINCE_20260817` 说明为什么这次是例外：{fresh}\n"
        "⚠️ 账号所有者 2026-08-17 定过：不要主动去找还没打的比赛做前瞻。\n"
        "账号所有者亲口点名要的除外（写进 `_NEW_PREVIEWS_SINCE_20260817`），"
        "别的还是老规矩：等球打完做「赛场之上」，或者把来路做成"
        "常青的「网球有故事」。")


def test_开球之前恢复新增前瞻():
    """账号所有者 2026-08-24：点名要一条郑钦文 VS 尤晓迪的美网资格赛前瞻。

    ⭐⭐ **上面 `test_不再做比赛前瞻` 管的是「我该不该主动去找」，这条测试
    管的是「他点名要的那条，有没有留下判据」**——两者不是同一件事，所以
    没有删掉上一条测试，是在它旁边加了一条新的。

    `_NEW_PREVIEWS_SINCE_20260817` 就是这条判据：新增的「开球之前」必须
    要么在 2026-08-17 之前发过（`_LEGACY_PREVIEWS`），要么在这张新表里
    写清楚为什么这次是例外。**不能只塞一个名字**——2026-08-17 那条测试的
    docstring 早就写过「真要恢复：把这条测试连同它的理由一起改掉」，指的
    正是这个：豁免要带着理由，不然下一个人（或者换了上下文的我自己）看到
    一个空荡荡的名字，不知道它凭什么在那儿。
    """
    from tennislive.video.explainer import _SCRIPTS, explainer_column
    ...
    # 判据自己的判据：新表里的每一条都要真的存在、真的还是「开球之前」，
    # 而且理由不能是空字符串——写了个空壳等于没写。
    for slug, reason in _NEW_PREVIEWS_SINCE_20260817.items():
        assert slug in now, (
            f"`_NEW_PREVIEWS_SINCE_20260817` 里的「{slug}」不存在，"
            "或者已经不是「开球之前」了")
        assert reason.strip(), f"「{slug}」的理由是空的，等于没写"
