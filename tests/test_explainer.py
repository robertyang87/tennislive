...
def test_slide_html_is_native_9x16_and_escapes_text():
    assert (W, H) == (1080, 1920)  # native vertical 9:16
    seg = ExplainerSegment("cause", "前因后果", "起<点>", "旁白 & 文本")
    doc = _slide_html(0, seg, "7.25")
    assert "① 前因后果" in doc
    assert "起&lt;点&gt;" in doc and "<点>" not in doc
    assert "旁白 &amp; 文本" in doc
    # Sized to the 9:16 canvas, not the 3:4 card.
    assert "width:1080px;height:1920px" in doc
