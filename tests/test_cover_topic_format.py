"""封面副标题（`cover.topic`）的格式：「ATP250 杭州站 第二轮 · 梅德韦杰夫 VS 鲁瓦耶」。
...
def test_按顶栏去年份再接两个名字拼():
    assert rf.cover_topic("2026 ATP250 杭州站 第二轮",
    ...
    # 缺名字就拼不出，不管
    assert rf.cover_topic("2026 ATP250 杭州站 第二轮", {"matchup": [{"name": "A"}]}) is None


def test_合格式的放行_不合的拦_认领了放行():
    ok = _spec("2026 ATP250 杭州站 第二轮", "ATP250 杭州站 第二轮 · 梅德韦杰夫 VS 鲁瓦耶")
    assert rf.cover_topic_problem(ok) is None
    for bad in ("杭州公开赛第二轮 · 梅德韦杰夫挺进8强",           # 老写法
                "ATP250 杭州站 第二轮 · 梅德韦杰夫 vs 鲁瓦耶",     # vs 小写
                "2026 ATP250 杭州站 第二轮 · 梅德韦杰夫 VS 鲁瓦耶",  # 带了年份
                "ATP250 杭州站 第二轮 · 鲁瓦耶 VS 梅德韦杰夫"):     # 名字顺序反了
        assert rf.cover_topic_problem(_spec("2026 ATP250 杭州站 第二轮", bad)), bad
    claimed = _spec("2026 ATP250 杭州站 第二轮", "别的写法", _topic_format_why="特例")
...
def test_渲染入口在dry_run就拦():
    ...
    spec["cover"]["topic"] = "ATP250 杭州站 第二轮 · 梅德韦杰夫 VS 鲁瓦耶"
    reel._topbar_lines(spec)
