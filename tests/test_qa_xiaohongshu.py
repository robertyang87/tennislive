...
def test_publish_qa_warns_but_does_not_block_an_untranslated_top_500_player(monkeypatch):
    from tennislive import qa

    digest = _digest()
    digest.results[0].home[0].rank = 400
    ...
    assert not any("前500球员中文名" in item for item in fatal)
