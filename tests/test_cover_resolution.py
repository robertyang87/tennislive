...
_DRAWN_COVERS = {
    ...
    "qualifier-ceiling",
    # 封面那一问是「12040 下球，算违规吗？」。那个数是这一届某一位球员的
    # 统计，把他的脸压在这句话下面，读者读到的就是「他违规了」——而答案
    # 恰恰是「不违规」，规则确实没写。这和 gamesmanship 是同一条：带着
    # 指控的封面不放被议论的那张脸，改放**证据本身**。所以画的是那条
    # 时间轴：一分之间，钟只走到「一发击出」为止，后半段是空的。
    # 四张官方实拍（赛后拥抱、接发特写、全场起立、看台视角）都在正文屏。
    "second-serve-clock",
}
