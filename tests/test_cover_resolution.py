...
# lucky-loser 的封面不在这张名单上，但它的做法值得记一句：原图是 1179×1219 的
# 截图（干净原图四条路全走完都取不到），**只在顶部垫了 353px**、交界羽化 110px
# （画布高的 7%，仓库里量出来的数——2.6% 那条亮度差还是一条看得见的横线）。
#
# 为什么只垫顶不垫底，是账号所有者定的：**顶部是看台背景，虚化压在台头底下正好；
# 底部有奖杯底座和摄影署名，一个像素都不能动**。而且这么做同时解决了三件事——
# 不裁宽所以署名完整、1.09x 铺满所以不用放大、顶栏与下方只差 +2.5 所以顶栏读
# 起来是透明的（硬贴那版是 +33.7）。
