...
_DRAWN_COVERS = {
    ...
    "qualifier-ceiling",
    # 封面那一问是「打了一年，为什么只算 19 个？」，而它的答案是一个**可数的
    # 结构**：19 个格子＝4 个大满贯 ＋ 8 个强制 1000 ＋ 年终总决赛 ＋ 6 个自己挑
    # （WTA 那行 18 个）。**图里就是标题问的那 19 个格**，一眼数得出来；
    # 而任何一张球员实拍只能证明「有人在打球」，说不出「只算 19 个」。
    # 和 `tour-balls` 那条同一个形状：答案是分布/结构，不是某一个人。
    # ⚠️⚠️ **换掉照片的理由是「题不对」，不是分辨率**——原来那张郑钦文美网举拳
    # 是 WTA 官方 4000×2476，铺满只要 0.58×，这条 1.00x 的地板它过得绰绰有余。
    # 账号所有者 2026-09-19：「封面换一个能表示积分的图片」，而举拳表达的是
    # 「赢球」。**两件事别混**（和上面 gamesmanship 那条末尾同一句警告）：
    # 混了的话，下次有人会以为「找到更大的实拍就能把脸放回封面」。
    "ranking-math",
    # 封面那一问是「中网外卡为什么给了美国人？」，而它的答案是一个**可数的
    # 结构**：32 个正赛位置 ＝ 直接入围 23-25 ＋ 资格赛 4 ＋ 外卡 3-4 ＋ 特殊豁免
    # 0-1，第 4 张外卡是额外加出来、从直入那一栏让出来的。**图里就是那 32 个格**，
    # 一眼数得出来，也一眼看得出哪一个是 A+ 那张。
    # 和 `ranking-math`（19 个格子）、`tour-balls` 同一个形状：答案是结构，
    # 不是某一个人。任何一张勒纳·钱的实拍都只能证明「有人在打球」，说不出
    # 「这一张是额外的、只能给这种人」。
    # ⚠️ 两张中网官方实拍（去年的亚军盘）**不是没拿到，是故意不放封面**：
    # 它们讲的是「他去年打到决赛」，那是第 ② 屏和末屏的事，不是封面问的那件事。
    "a-plus-wildcard",
}
...
_UNDERSIZED = {
    ...
    # 1280×720，同一条渠道同一个天花板（见上一条）。⚠️ **这条封面一度是示意图**
    # ——我按 gamesmanship 那条「带着指控的封面不放被议论的那张脸」把它画成了
    # 时间轴，而账号所有者 2026-09-15 直接要「封面最好用兹维列夫二发的照片」。
    # 那条先例套宽了：封面这一问（「二发前拍20下球，算违规吗？」）的答案是
    # **不算**，它指向的是规则的空白，不是指控某个人；而照片比示意图抓人得多。
    # 大图找过：USA TODAY 当日图集 24 张逐条读过说明，兹维列夫只有三张
    # 「in action」（2024~5406px），打开看全是正反手击球，**一张发球都没有**；
    # AP 对这一场是零；WTA photo-resources 不收大满贯。所以官方接口这一档
    # 就是上限。取的那张官方说明写着 `Alexander Zverev serves`，是发球随挥的
    # 近景特写——脸、发力表情、`us open finals` 背景板都在。
    "second-serve-clock": 0.50,  # 1280x720
}
