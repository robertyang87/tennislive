...
_DRAWN_COVERS = {
    # 封面那一问是「花钱请球星来，犯规吗？」。把辛纳或阿尔卡拉斯的脸压在这句话
    # 下面，等于替读者认定了他就是收了钱的那个人——而**没有任何一手证据支持
    # 那句话**：查下来没有一手报道把推广费和这两人这两站连起来，流传的那几个
    # 数字全是网球博客和论坛的「据传」，而阿卡本人给的理由是想去个没打过的
    # 球场、且他是去卫冕。形状和 `gamesmanship` 一模一样，连理由都一样。
    # 所以封面画的是**规则本身**：四档赛事，只有 500 和 250 那两档可以。
    # 他俩的 ATP 官方实拍留在第 ①② 屏，那两屏只有赛程、比分和他们自己的原话。
    # ⚠️ 那两张都是 1920×1080，铺满封面要放大 133%，本来也过不了 1.00x 的
    # 地板——**但换掉它的理由是上一条，分辨率只是碰巧站在同一边**。
    "promotional-fees",
