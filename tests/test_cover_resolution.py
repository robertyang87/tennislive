...
_DRAWN_COVERS = {
    ...
    "gamesmanship",
    # 封面那一问是「凭什么说今天太热了？」，而它的答案**就是一张配方图**：
    # WBGT 的户外公式里湿球占七成、黑球两成、气温只占一成。一张德约的实拍
    # 只能证明「他在打球」，答不了这一问——这是「示意图的触发条件是照片讲不清」
    # 那条本身，不是找不到照片的退路（第 ① 屏就是赛事官方的当日实拍）。
    # ⚠️ **这条和上面 gamesmanship 那条的理由不一样，别混着读。** 那条是
    # 「标题带着指控，放谁的脸就是在指认谁」；这条的标题（28 度也能叫极端
    # 高温）讲的是天气和规则，**不含对任何人的负面判断**，所以德约的脸放在
    # 正文屏是合规的。换成图纯粹是因为图答得了那一问。
    # ⚠️ 顺带：那张实拍 2000×1333，铺满 1080×1440 要放大 1.08 倍，本来也过不了
    # 下面那条 1.00x 的地板——**但选图的理由是前一条，分辨率只是碰巧站在同一边**。
    "heat-rule",
}
