...
_NO_SLATE_YET = {
    ...
    # 而 CLAUDE.md 那条规矩要的正是北京时间（读者在国内）。
    # ⚠️ 它是**自动链产的**（spec 带 `_production`），而自动链直推 main
    # **不触发 CI**——这两条编辑判据只活在 pytest 里，没有像那批措辞判据一样
    # 搬进 `promote_reel_draft.waiting_reasons`，所以它从生成到发进微信一次
    # 都没被扫过。**这一类的修法是把判据搬进那道闸**，不是往这张表里加名字；
    # 那是一次单独的改动（要配自己的反向验证，还动着别的会话在改的自动链），
    # 记在这儿别忘了。
    # 挂进来不是原谅，是记账——已发的片子不为措辞重渲（音轨和字幕都烧进去了），
    # 而一条常年红的检查和没有检查是同一个毛病。
    "andreeva-gauff",
...
def test_小数要写成小数钟点不许跟着塌():
    """「百分之二十六点三」烧到屏幕上是「26%点三」——一个没人读得出来的东西。

    `arabic_numerals` 原来根本不认小数：「点」在 `_NUM_UNITS` 里当量词（「凌晨
    三点」→「3点」是对的），于是「二十一点六秒」塌成「21点六秒」；而「百分之」
    那条正则的字符类里没有「点」，run 只吃到「二十六」，剩下「点三」原样留着。
    ⚠️ **两条路都不报错**——spec 里那句话是对的，错的只有烧进画面的那一份。

    这一条的难处全在**另一头**：语料里「N点M」这个形状 86 处，绝大多数是**钟点**
    （「凌晨两点五十五」「晚上九点零六分」「上午十一点四十」），把它们渲成小数是
    屏幕上的错字，比不渲糟得多。所以两道窄闸缺一不可：

    1. **小数部分只认单个数字**——中文的小数是逐字读的（二十一点六 / 零点一），
       一旦出现十/百/千那就是钟点。语料里 42 处「点」后面跟「十」，一个小数都没有。
    2. **后面不许紧跟 `分半刻钟点`**——挡掉「九点零六分」和成语「一点一点」
       （三条采访的中文行里就有；拆掉这一条，实测它变成「1.1点」、「两点一刻」
       变成「2.1刻」、「九点零六分」变成「9.06分」）。

    ⚠️ 还有第三道，**它是回溯逼出来的，光看规则想不到**：后顾断言里还要禁掉
    数字字符本身。`[DIGIT]+` 会退一格再试——「九点零六分」先拿「零六」撞上「分」
    被拒，退成「零」，后面是「六」不在禁表里，于是当场匹配成「9.0六分」。
    拆掉这一条实测还有「凌晨两点五十五」→「2.515」、「十一点四十」→「11.410」，
    **四个钟点全在语料里**。

    ⚠️ 反向验证做了四个方向，**只有三个红**：上面这两道闸、以及百分比那条路的
    小数支持（拆掉它「百分之五十八点五」变成「100分之58.5」）。第四个方向
    ——`_decimal_value` 里那句「小数部分只认 `_DIGIT`」——**拆掉之后测试照样绿**，
    因为通用那条路的正则已经把它限死了；它只对百分比那条路有意义，而今天没有
    输入走得到。那句话在函数的 docstring 里标着「验不出来」，别把它读成验过了。

    ⚠️ 认领过的代价：`weeks-at-no1` 的「零点八分」是**真小数**（0.8 个积分），
    被第 2 道闸一起挡掉了。分辨它和「九点零六分」只能靠上下文，而那是个会误判的
    判据——两边不对等，所以取窄的那一刀。那条片子已发不重渲，对它没有回归。
    """
    from tennislive.video.explainer import arabic_numerals  # noqa: PLC0415

    小数要写成小数 = [
        # 这条线上真用到的
        ("超时的占百分之五十八点五", "超时的占58.5%"),
        ("超时降到百分之二十六点三", "超时降到26.3%"),
        ("平均间隔二十一点六秒", "平均间隔21.6秒"),
        ("男子从四十点二秒变成四十一秒", "男子从40.2秒变成41秒"),
        ("球员花的时间只差零点一秒", "球员花的时间只差0.1秒"),
        # 语料里别的片子
        ("气温是二十八点六度", "气温是28.6度"),
        ("风速每小时一点三公里", "风速每小时1.3公里"),
        ("前面十点七周，后面十八点一周", "前面10.7周，后面18.1周"),
        ("规则书第一点零八条", "规则书第1.08条"),
        ("一场两点六亿美元的改造", "一场2.6亿美元的改造"),
    ]
    钟点和成语不许跟着塌 = [
        # ① 小数部分含十/百/千的，一律是钟点
        ("凌晨两点五十五，辛辛那提", "凌晨两点55，辛辛那提"),
        ("上午十一点四十开球", "上午11点40开球"),
        ("凌晨零点十五分", "凌晨0点15分"),
        # ② 后面紧跟「分/刻」的钟点——这两条正是回溯那一刀救回来的
        ("晚上九点零六分，官方宣布顺延", "晚上9点零六分，官方宣布顺延"),
        ("第三天下午四点四十七分", "第三天下午4点47分"),
        ("下午两点一刻开打", "下午两点一刻开打"),
        # ③ 成语
        ("然后一点一点找回手感", "然后一点一点找回手感"),
        ("听起来一点都不吓人", "听起来一点都不吓人"),
        ("文案再往下挪一点", "文案再往下挪一点"),
        # ④ 「点」本来就有的两个用法，一个都不许碰
        ("决胜盘两个盘点", "决胜盘两个盘点"),
    ...
    for src, want in 小数要写成小数 + 钟点和成语不许跟着塌:
        assert arabic_numerals(src) == want, (
            f"{src!r} 换算成了 {arabic_numerals(src)!r}，应该是 {want!r}")
...
def test_赛场之上的比赛画面必须带比赛信息顶栏():
    ...
    good = {"slug": "x", "cover": {"eyebrow": "赛场之上", "winner": "萨巴伦卡",
                                   ...
                                   "topic": "WTA1000 加拿大 第三轮 · 萨巴伦卡 VS 张帅",
                                   "matchup": [{"name": "萨巴伦卡"}, {"name": "张帅"}]},
            "topbar": {"line1": "2026 WTA1000 加拿大 第三轮", "line2": "萨巴伦卡 6-3 6-4 张帅"}}
    assert reel._topbar_lines(good) == ("2026 WTA1000 加拿大 第三轮", "萨巴伦卡 6-3 6-4 张帅")
