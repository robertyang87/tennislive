...
def test_球场那个图标要画成网球场的样子(
    ...
    二值化，三头（实测值见断言里的注释，三版逐个量过）：
        ...
        宽高比   贯通全宽的横线   最上一行的墨宽
        1.75           4              0.03      ← 现在这版（真球场，35×20px）
        2.00           2              0.95      ← 第二版：只有上下两条外框
        0.91           5              0.90      ← 第一版：竖的，宽高比就不对

    ⭐⭐ 账号所有者 2026-08-31 又转了个方向：「**封面里球场的 icon 变成
    竖着的，长宽比要和实际一致**」。所以现在是**竖版**，三笔特征跟着转置
    90°，另加一头钉「和实际一致」——外框比例必须是真实球场的 36:78
    （ITF：双打宽 36 英尺 × 长 78 英尺 = 1:2.167），出处只有 viewBox 一处。

    1. **竖的**（高宽比 ≥ 1.5）——横着画就回到 08-29 那一版了
    2. **四条贯通全高的竖线**（双打边线两条 ＋ 单打边线两条）——少了单打
       边线那两笔，读出来是「一扇窗」（08-29 第一版的样子）
    3. **球网出头**（最左/最右一列的墨只有球网那一横，占高 ≤ 0.25）——
       网柱在界外，球网向两侧各伸出一点；画成贯通外框边就没有这层辨识
    4. **外框＝真实比例**：`_COURT_ICON` 的 rect 高÷宽 = 78/36 ±2%
       （渲出来的 glyph 外接框含球网出头，量不到纯外框，所以这一头
       从 SVG 源量；1~3 头证明它真的被画出来了）

    ⚠️ 前三头缺一不可（08-29 的两版各过得了其中一头）；第 4 头单独管
    「实际一致」——只有它的话，画个 1:2.167 的空框也能过。
    """
    rect = re.search(r'<rect[^>]*width="([\d.]+)" height="([\d.]+)"',
                     versus_poster._COURT_ICON)  # noqa: SLF001
    assert rect, f"图标里没有外框 rect：{versus_poster._COURT_ICON}"  # noqa: SLF001
    ratio = float(rect.group(2)) / float(rect.group(1))
    assert abs(ratio - 78 / 36) <= 78 / 36 * 0.02, (
        f"④ 外框高÷宽 = {ratio:.3f}，真实球场是 78/36 = {78 / 36:.3f}——"
        "账号所有者 2026-08-31：「长宽比要和实际一致」。")
    ...
    h, w = glyph.shape
    heights = glyph.sum(axis=0) / h

    assert h / w >= 1.5, (
        f"① 图标是 {w}×{h}，高宽比只有 {h / w:.2f}——要竖版的真实球场"
        "（含球网出头的外接框约 1.8），横着画就回到 08-29 那一版了。")

    tall = heights >= 0.8
    bands = int((tall[:-1] & ~tall[1:]).sum() + bool(tall[-1]))
    assert bands >= 4, (
        f"② 贯通全高的竖线只有 {bands} 条——要四条（双打边线两条 ＋ 单打"
        "边线两条）。少了单打那两笔，读出来是一扇窗。")

    assert heights[0] <= 0.25 and heights[-1] <= 0.25, (
        f"③ 最左/最右一列的墨占高 {heights[0]:.2f}/{heights[-1]:.2f}——"
        "网柱在界外，球网那一横要向两侧各伸出一点，所以最左和最右那一列"
        "应该只有球网、不该是贯通的外框边。")


def test_封面版式定版冻结_钩子字号和比分板坐标是全局统一的(
        monkeypatch: pytest.MonkeyPatch):
    """⭐⭐ 账号所有者 2026-08-31 定版：「后续固定这个布局设计，保证成全局
    统一要求」。

    来路：当天核过一遍——钩子字号全库收成 94 一个数（129 条新封面全是它，
    低于 94 的 49 条全是只减不增的存量豁免）、比分板外框 940/左右 70 边距、
    盘分列 96、名字格随盘数在固定外框里自适应、板的落点因为钩子只有一个
    字号而恒在 1057。他看完说定版。

    既有判据钉的是**关系**（两行对齐、预算装得下、字号上限只许往上调），
    关系判据拦不住整套值一起漂——这一条钉**数值本身**：下面这张表里任何
    一个数要变，都必须是账号所有者亲口要的，并且和这条判据一起改，让
    「改版式」永远是一次看得见的决定，不是哪次重构顺手漂掉的。

    两头缺一不可：① 常量表（值的出处）；② 渲出来的 CSS（值真的被用上——
    `_push` 键名写错、占位符没插值那两类坑，只查常量看不见）。

    反向验证过两个方向：STORYCOPY_TOP 改 750 → 红在常量表；
    `.storycopy` 的 gap 改 40 → 红在 CSS 那一头。
    """
    vp = versus_poster
    frozen = {
        "HOOK_TITLE_PX": 94,          # 钩子唯一字号（08-31「收成一个数」）
        "SOLO_HOOK_MAX_LINES": 2,     # 最多两行（第三行会把输家推出画布）
    ...
    assert not drift, (
        f"定版冻结的版式值漂了：{drift}（每项是 (现值, 定版值)）。\n"
        "这套值 2026-08-31 账号所有者定版为全局统一——要改必须是他亲口要的，"
        "并且把这张表一起改，别让版式在重构里静默漂移。")

    css = _board_css(monkeypatch)
    copy_rule = _rule(css, ".storycopy")
    for piece in ("top:790px", "gap:34px", "left:70px", "right:70px"):
        assert piece in copy_rule, (
            f"渲出来的 `.storycopy` 少了 `{piece}`：{copy_rule!r}\n"
            "常量对着、CSS 没用上（占位符没插值那一类）也是漂。")
    assert "line-height:1.24" in _rule(css, ".storytitle"), (
        "钩子行高不再是 1.24——比分板的落点账（790+round(2×94×1.24)+34=1057）"
        "跟着它走，改行高等于把板的落点也改了。")
