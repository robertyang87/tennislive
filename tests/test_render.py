...
def test_同日重跑不会冲掉上一班的候选诊断(tmp_path, monkeypatch):
    """daily.yml 同日重跑会 rm -rf knowledge/；台账写在当日目录并逐班追加。"""
    from tennislive.render import tournament_story
    ...
    tournament_story.record_story_selection(
        day_dir, digest, ranking, selected_slug="golden-slam"
    )
...
def _py_without_comments(path: Path) -> str:
    """去掉整行注释和 docstring 再扫，**但别把产物模板也一起去掉**。

    这个仓库的注释正是记教训的地方——`pushmsg.py` 那段说明里就写着
    `#ff2442` 和 `padding:13px`。连注释一起扫，「把坑记下来」会被判成
    「又踩了这个坑」（同一个错这个仓库犯过五次）。

    ⚠️ **第一版按三引号的形状去抹 docstring，当场把判据变成了一盏绿灯**：
    `knowledge.py` 那张推送卡整个是一个三引号 f-string，按钮就在里面——
    反向验证时把内联那份塞回去，测试照样过。docstring 要按 **AST 给的位置**
    精确抹，不能按引号形状抹。
    """
    ...
    try:
        tree = ast.parse(src)
    except SyntaxError:  # 扫不了的先放过，别把判据变成语法检查器
        return src
    drop: set[int] = set()
    ...
    return "\n".join(
        "" if i in drop or line.lstrip().startswith("#") else line
        for i, line in enumerate(lines, start=1)
    )


def test_推送里那颗红按钮要比主按钮矮():
    """账号所有者 2026-08-31：「红色区域高度可以更小一些」。

    指的是推送卡片底部那颗 `#ff2442` 的「分别复制标题 / 正文」——它原来和
    「▶ 打开竖版成片」**一样高（各 47px）**，而红色的视觉分量天生比深绿重，
    于是次要动作在卡底喊得比主要动作还响，层级是反的。

    真渲量过（Chromium，390px 视口）：主按钮 47px、红按钮 **37px**。
    判据钉的是**那个高度差**，不是某一个具体的数——谁把红那颗调回去，
    或者把主按钮一起压矮（差没了，层级也就没了），都当场红。
    """
    from tennislive.render import pushmsg

    assert pushmsg.PUSH_BTN_PAD_Y_COPY < pushmsg.PUSH_BTN_PAD_Y, (
        "红按钮不比主按钮矮，层级就回到「两颗一样重」那一版了"
    )
    # 上下各差 5px、渲出来差 10px。差得太少人看不出来，等于没改。
    assert pushmsg.PUSH_BTN_PAD_Y - pushmsg.PUSH_BTN_PAD_Y_COPY >= 4
    # ⚠️ 别再往下压：6px（33px 高）那档字快贴着上下边，看着是「压扁了」
    # 不是「次要」，而且它仍然是个要用手指点的链接。
    assert pushmsg.PUSH_BTN_PAD_Y_COPY >= 8

    primary = pushmsg.push_button("https://x/copy.html", "▶ 打开竖版成片",
                                  primary=True)
    copy_btn = pushmsg.push_button("https://x/copy.html", "分别复制标题 / 正文",
                                   primary=False)
    assert f"padding:{pushmsg.PUSH_BTN_PAD_Y}px 16px" in primary
    assert f"padding:{pushmsg.PUSH_BTN_PAD_Y_COPY}px 16px" in copy_btn
    assert pushmsg.PUSH_BTN_BG_COPY in copy_btn
    assert pushmsg.PUSH_BTN_BG_PRIMARY in primary

    # ⚠️ 改按钮的 markup 不许把「复制页取不到就摘掉这个 `<a>`」那条路弄断：
    # 那条正则认的是 href 里的 copy.html，不是样式。
    stripped, url = pushmsg.drop_dead_copy_button(copy_btn, probe=lambda _u: False)
    assert stripped.strip() == "" and url is None


def test_推送按钮只有一处出处():
    """三条线共用 `push_button`，不许谁再内联写一遍那颗 `<a>`。

    这颗按钮原来在**三个文件里各写了一遍**（成片推送、知识帖、已停产的日报
    模板），一模一样的内联 style。所以「红色区域矮一点」这种一句话的改动，
    要改三处——而漏一处的样子是「同一个账号发出去的推送，按钮高矮不一」，
    不报错，也没人对得上。

    判据自己从源码推，不维护名单：凡是 `src/` 和 `tools/` 里的 py 文件，
    去掉注释和 docstring 之后，都不许再出现带这两个背景色的 `<a` 标签，
    `pushmsg.py`（出处本身）除外。
    """
    from tennislive.render import pushmsg

    root = Path(__file__).resolve().parents[1]
    source = Path(pushmsg.__file__).resolve()
    hits = []
    checked = 0
    for path in sorted([*(root / "src").rglob("*.py"), *(root / "tools").rglob("*.py")]):
        if path.resolve() == source:
            continue
        checked += 1
        body = _py_without_comments(path)
        for colour in (pushmsg.PUSH_BTN_BG_COPY, pushmsg.PUSH_BTN_BG_PRIMARY):
            if re.search(r"<a\s[^>]*background-color:" + re.escape(colour), body):
                hits.append(f"{path.relative_to(root)} 内联了 {colour} 的按钮")
    # 判据自己的判据：主语没了它要出声，而不是变成一条恒真的绿灯。
    assert checked >= 50, f"只扫到 {checked} 个 py 文件，扫描范围坏了"
    assert not hits, "这几处要改走 pushmsg.push_button()：\n  " + "\n  ".join(hits)
