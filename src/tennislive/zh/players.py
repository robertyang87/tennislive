"""球员英文名 → 中文媒体通用译名.

由整理脚本生成 + 人工校对；查询逻辑见 zh/__init__.py 的 player_zh()。
"""

from __future__ import annotations

PLAYER_ZH: dict[str, str] = {
    # ===== ATP =====
    "Jannik Sinner": "辛纳",
    "Carlos Alcaraz": "阿尔卡拉斯",
    "Alexander Zverev": "兹维列夫",
    "Novak Djokovic": "德约科维奇",
    "Felix Auger-Aliassime": "阿利亚西姆",
    "Alex de Minaur": "德米纳尔",
    "Ben Shelton": "谢尔顿",
    "Taylor Fritz": "弗里茨",
    "Daniil Medvedev": "梅德韦杰夫",
    "Flavio Cobolli": "科博利",
    "Lorenzo Musetti": "穆塞蒂",
    "Alexander Bublik": "布勃利克",
    "Casper Ruud": "鲁德",
    # 戴维斯杯那条「网球有故事」（2026-09-16）点到的三位老球员，表里原来没有：
    "Victor Pecci": "佩奇",          # 巴拉圭，1985 戴维斯杯第五场胜勒孔特
    "Henri Leconte": "勒孔特",       # 法国
    "Sam Querrey": "奎雷伊",         # 美国，2008 戴维斯杯半决赛负纳达尔
    # 2026-09 戴维斯杯世界一组客场对挪威，ITF 正式名单里的两位（表里原来没有）：
    "Rigele Te": "特日格乐",          # 中国，单打；ATP/ITF 写作 Rigele Te
    "Fnu Nidunjianzan": "尼敦坚赞",   # 中国（拉萨出生），普林斯顿；百度百科词条「尼敦坚赞」
    "Andrey Rublev": "卢布列夫",
    "Jiri Lehecka": "莱赫奇卡",
    "Luciano Darderi": "达尔代里",
    "Learner Tien": "勒纳·钱",
    "Jakub Mensik": "门西克",
    "Frances Tiafoe": "蒂亚福",
    "Valentin Vacherot": "瓦舍罗",
    "Francisco Cerundolo": "塞伦多洛",
    "Arthur Fils": "菲斯",
    "Tommy Paul": "保罗",
    "Rafael Jodar": "霍达尔",
    "Karen Khachanov": "哈恰诺夫",
    "Jack Draper": "德雷珀",
    "Holger Rune": "鲁内",
    # 2018 伊斯特本资格赛头号种子（`1/Alt`），首轮负布兰兹；`fils-tokyo-qualifying` 的先例屏。
    "Jack Sock": "索克",
    "Daniel Brands": "布兰兹",
    # 保加利亚戴维斯杯队 2008 年生的小将，2026 世界一组第一轮对丹麦首日负鲁内、
    # 第五盘决胜对默勒。译名按保加利亚姓 Радулов 的通行中文写法（同姓的伊万·
    # 拉杜洛夫是保加利亚 70 年代的头号球员，中文媒体一贯写「拉杜洛夫」）。
    "Iliyan Radulov": "拉杜洛夫",
    # 2026 戴维斯杯世界一组第一轮丹麦 vs 保加利亚这条 tie 上的另外两位，
    # 写置顶评论时发现表里没有、`player_zh` 原样返回英文名，按规矩补进来。
    # ⚠️ **这两个是按音译规则定的，没找到中文媒体的既有写法**（不像拉杜洛夫那样
    # 有 70 年代的同姓前辈可依）——下次看到权威中文稿用了别的写法，以那个为准。
    # Донски 的 Дон 按「顿河 Don」的通行对法写「顿」；Ingildsen 是丹麦姓，
    # 按 In-gild-sen 三节写「因吉尔森」。
    "Alexander Donski": "顿斯基",
    "Johannes Ingildsen": "因吉尔森",
    "Joao Fonseca": "丰塞卡",
    "Stefanos Tsitsipas": "西西帕斯",
    "Grigor Dimitrov": "迪米特洛夫",
    "Tomas Machac": "马哈奇",
    "Alejandro Davidovich Fokina": "达维多维奇·福基纳",
    # 2025 年退役，进不了 top500 那张表；`chengdu-ng-kouame` 的旁白要提他
    # （「他的团队里有加斯奎特」），照 CLAUDE.md「表里没有的，定下来之后补进表里」。
    "Richard Gasquet": "加斯奎特",   # 法国
    "Denis Shapovalov": "沙波瓦洛夫",
    "Alexei Popyrin": "波皮林",
    "Alex Michelsen": "米切尔森",
    "Ugo Humbert": "安贝尔",
    "Giovanni Mpetshi Perricard": "佩里卡尔",
    "Tallon Griekspoor": "格里克斯普尔",
    "Matteo Berrettini": "贝雷蒂尼",
    "Alexander Shevchenko": "舍甫琴科",
    "Gabriel Diallo": "迪亚洛",
    "Matteo Arnaldi": "阿尔纳尔迪",
    "Luca Nardi": "纳尔迪",
    "Lorenzo Sonego": "索内戈",
    "Mattia Bellucci": "贝卢奇",
    "Arthur Rinderknech": "林德克内希",
    "Terence Atmane": "阿特马内",
    "Cameron Norrie": "诺里",
    "Benjamin Bonzi": "邦齐",
    "Alexandre Muller": "米勒",
    "Corentin Moutet": "穆泰",
    "Stephane Robert": "罗贝尔",
    "Valentin Royer": "鲁瓦耶",
    "Arthur Cazaux": "卡佐",
    "Quentin Halys": "阿利斯",
    "Hugo Gaston": "加斯东",
    "Adrian Mannarino": "马纳里诺",
    "Gael Monfils": "孟菲尔斯",
    "Luca Van Assche": "范阿舍",
    "Sebastian Korda": "科达",
    "Brandon Nakashima": "中岛布兰登",
    "Reilly Opelka": "奥佩尔卡",
    "Marcos Giron": "吉隆",
    "Nishesh Basavareddy": "巴萨瓦雷迪",
    "Tristan Boyer": "博耶",
    "Ethan Quinn": "奎因",
    "Aleksandar Kovacevic": "科瓦切维奇",
    "Jenson Brooksby": "布鲁克斯比",
    "Emilio Nava": "纳瓦",
    "Colton Smith": "科尔顿·史密斯",
    "Mackenzie McDonald": "麦克唐纳",
    "Christopher Eubanks": "尤班克斯",
    "Jordan Thompson": "汤普森",
    "Christopher O'Connell": "奥康奈尔",
    "James Duckworth": "达克沃斯",
    "Adam Walton": "沃尔顿",
    "Aleksandar Vukic": "武基奇",
    "Rinky Hijikata": "土方凛辉",
    "Tristan Schoolkate": "斯库尔凯特",
    "Nick Kyrgios": "克耶高斯",
    "Bernard Tomic": "托米奇",
    "Roman Safiullin": "萨菲乌林",
    "Jan-Lennard Struff": "斯特鲁夫",
    "Daniel Altmaier": "阿尔特迈尔",
    "Yannick Hanfmann": "汉夫曼",
    "Justin Engel": "恩格尔",
    "Dominik Koepfer": "克普费尔",
    "Jaume Munar": "穆纳尔",
    "Pedro Martinez": "马丁内斯",
    "Roberto Carballes Baena": "卡巴列斯·巴埃纳",
    "Roberto Bautista Agut": "巴蒂斯塔·阿古特",
    "Pablo Carreno Busta": "卡雷尼奥·布斯塔",
    "Martin Landaluce": "兰达卢塞",
    "Jacob Fearnley": "弗恩利",
    "Dan Evans": "埃文斯",
    "Arthur Fery": "费里",
    "Botic van de Zandschulp": "范德赞德舒尔普",
    "Jesper de Jong": "德容",
    "David Goffin": "戈芬",
    "Zizou Bergs": "贝尔格斯",
    "Raphael Collignon": "科利尼翁",
    "Alexander Blockx": "布洛克斯",
    "Stan Wawrinka": "瓦林卡",
    "Dominic Stricker": "施特里克",
    "Sebastian Ofner": "奥夫纳",
    "Filip Misolic": "米索利奇",
    "Marin Cilic": "西里奇",
    "Borna Coric": "丘里奇",
    "Dino Prizmic": "普里兹米奇",
    # 2026-09 戴维斯杯资格赛第二轮用到的六个双打球员，表里原来都没有
    # （CLAUDE.md「表里没有的，定下来之后补进表里，别只写在稿子里」）。
    "Kevin Krawietz": "克拉维茨",
    "Tim Puetz": "皮茨",
    "Tim Pütz": "皮茨",
    "Nikola Mektic": "梅克蒂奇",
    "Mate Pavic": "帕维奇",
    "Henry Patten": "帕滕",
    "Neal Skupski": "斯库普斯基",
    "Damir Dzumhur": "朱姆胡尔",
    "Laslo Djere": "杰雷",
    "Miomir Kecmanovic": "凯茨马诺维奇",
    "Dusan Lajovic": "拉约维奇",
    "Hamad Medjedovic": "梅杰多维奇",
    "Marton Fucsovics": "富乔维奇",
    "Fabian Marozsan": "马罗赞",
    "Hubert Hurkacz": "胡尔卡奇",
    "Kamil Majchrzak": "迈赫扎克",
    "Vit Kopriva": "科普日瓦",
    "Dalibor Svrcina": "斯弗尔奇纳",
    "Elmer Moller": "默勒",
    "Nuno Borges": "博尔热斯",
    "Jaime Faria": "法里亚",
    "Nicolas Jarry": "哈里",
    "Alejandro Tabilo": "塔比洛",
    "Cristian Garin": "加林",
    "Sebastian Baez": "巴埃斯",
    "Tomas Martin Etcheverry": "埃切维里",
    "Mariano Navone": "纳沃内",
    "Francisco Comesana": "科梅萨尼亚",
    "Camilo Ugo Carabelli": "乌戈·卡拉贝利",
    "Facundo Diaz Acosta": "迪亚斯·阿科斯塔",
    "Roman Burruchaga": "布鲁查加",
    "Thiago Seyboth Wild": "塞博特·维尔德",
    "Thiago Monteiro": "蒙泰罗",
    "Ignacio Buse": "布塞",
    "Daniel Elahi Galan": "加兰",
    "Nikoloz Basilashvili": "巴西拉什维利",
    "Emil Ruusuvuori": "鲁苏武里",
    "Milos Raonic": "拉奥尼奇",
    "Federico Cina": "奇纳",
    "Matteo Gigante": "吉甘特",
    "Francesco Passaro": "帕萨罗",
    "Kei Nishikori": "锦织圭",
    "Yoshihito Nishioka": "西冈良仁",
    "Shintaro Mochizuki": "望月慎太郎",
    "Taro Daniel": "丹尼尔太郎",
    "Yosuke Watanuki": "绵贯阳介",
    "Soonwoo Kwon": "权纯雨",
    "Sumit Nagal": "纳加尔",
    "Chun-Hsin Tseng": "曾俊欣",
    "Coleman Wong": "黄泽林",
    "Zhizhen Zhang": "张之臻",
    "Juncheng Shang": "商竣程",
    "Yunchaokete Bu": "布云朝克特",
    "Yibing Wu": "吴易昺",
    "Yi Zhou": "周意",
    "Fajing Sun": "孙发京",
    # 机器音译只切出一个姓字「卢」，姓本身还写错了：英文维基 Lu Jiajing
    # 写明是「逯佳境」，排名史（生涯最高 162、2026 年 4 月第 425）与我们
    # 7 月 27 日快照里的第 417 对得上。卡片上印一个单字姓是坏输出。
    "Jia-Jing Lu": "逯佳境",
    # ===== WTA =====
    "Aryna Sabalenka": "萨巴伦卡",
    "Iga Swiatek": "斯瓦泰克",
    "Angelique Kerber": "科贝尔",
    "Elena Rybakina": "莱巴金娜",
    "Coco Gauff": "高芙",
    "Jessica Pegula": "佩古拉",
    "Amanda Anisimova": "阿尼西莫娃",
    "Mirra Andreeva": "米拉·安德烈耶娃",
    "Madison Keys": "凯斯",
    "Sloane Stephens": "斯蒂芬斯",
    "Jasmine Paolini": "保利尼",
    "Ekaterina Alexandrova": "亚历山德罗娃",
    "Karolina Muchova": "穆霍娃",
    "Linda Noskova": "诺斯科娃",
    "Clara Tauson": "陶森",
    "Elina Svitolina": "斯维托丽娜",
    "Naomi Osaka": "大坂直美",
    "Belinda Bencic": "本西奇",
    "Liudmila Samsonova": "萨姆索诺娃",
    "Jelena Ostapenko": "奥斯塔彭科",
    "Victoria Mboko": "姆博科",
    "Diana Shnaider": "施奈德",
    "Emma Navarro": "纳瓦罗",
    "Daria Kasatkina": "卡萨金娜",
    "Beatriz Haddad Maia": "哈达德·玛雅",
    "Elise Mertens": "梅尔滕斯",
    "Sofia Kenin": "肯宁",
    "Emma Raducanu": "拉杜卡努",
    "Marta Kostyuk": "科斯秋克",
    "Dayana Yastremska": "亚斯特雷姆斯卡",
    "Barbora Krejcikova": "克雷吉茨科娃",
    "Veronika Kudermetova": "维罗妮卡·库德梅托娃",
    "Anna Kalinskaya": "卡林斯卡娅",
    # 表里没有的名字，定下来就要补进来，别只写在稿子里——2026-07-29 写萨姆索诺娃
    # 2022 年华盛顿夺冠那条时用到，当时 `player_zh("Kaia Kanepi")` 原样返回英文。
    "Kaia Kanepi": "卡内皮",
    "Leylah Fernandez": "费尔南德斯",
    "Maya Joint": "乔因特",
    "Iva Jovic": "约维奇",
    "Ashlyn Krueger": "克鲁格",
    "McCartney Kessler": "凯斯勒",
    "Marketa Vondrousova": "万卓索娃",
    "Paula Badosa": "巴多萨",
    "Donna Vekic": "维基奇",
    "Danielle Collins": "柯林斯",
    "Magdalena Frech": "弗雷赫",
    "Magda Linette": "林内特",
    "Maja Chwalinska": "赫瓦林斯卡",
    "Olga Danilovic": "达尼洛维奇",
    "Alexandra Eala": "伊埃拉",
    "Cho I-hsuan": "卓宜萱",
    "I-hsuan Cho": "卓宜萱",
    "Cho Yi-tsen": "卓宜岑",
    "Yi-Tsen Cho": "卓宜岑",
    "Aidan Mayo": "梅奥",
    "Aldila Sutjiadi": "苏特贾迪",
    "Anna Danilina": "达尼利娜",
    "Cruz Hewitt": "克鲁兹·休伊特",
    "Desirae Krawczyk": "克拉夫奇克",
    "Eri Hozumi": "穗积绘莉",
    "Lyudmyla Kichenok": "柳德米拉·基琴诺克",
    "Nadiia Kichenok": "娜迪娅·基琴诺克",
    "Nicole Melichar-Martinez": "梅利哈尔-马丁内斯",
    "Lois Boisson": "布瓦松",
    "Janice Tjen": "曾丽美",
    "Qinwen Zheng": "郑钦文",
    "Xinyu Wang": "王欣瑜",
    "Shuai Zhang": "张帅",
    "Xiyu Wang": "王曦雨",
    "Yue Yuan": "袁悦",
    "Lin Zhu": "朱琳",
    "Yafan Wang": "王雅繁",
    "Sijia Wei": "韦思佳",
    "Xinyu Gao": "高馨妤",
    "Saisai Zheng": "郑赛赛",
    "Xinyun Han": "韩馨蕴",
    "Zhaoxuan Yang": "杨钊煊",
    "Xinyu Jiang": "蒋欣玗",
    "Qianhui Tang": "汤千慧",
    "Feng Shuo": "冯硕",
    # ESPN 数据源把她的名字拆成三段（姓 Ye + 名 Qiu/Yu 各占一段），
    # 与本文件"名 姓"两段式惯例不同，需按源字符串原样收录才能命中。
    "Ye Qiu Yu": "叶秋语",
    # ⚠️ **WTA 官方给的是姓在后那一版**（球员接口 id=321023 的 `fullName`
    # 就是 `Qiu Yu Ye`，双打排名榜写 `YE, QIU YU`），而 `player_zh` 的词序
    # 反转对三段名是整串倒过来（`Ye Qiu Yu` → `Yu Qiu Ye`）、去中间名是取
    # 首尾（`Qiu Ye`），**两条都对不上上面那一行**。也就是说 WTA 自己那条源
    # 一直解析不出她——而失败的样子是卡上多出一个英文名，跟"表里没有这个人"
    # 长得一样。2026-08-14 补，两种写法各收一行。
    "Qiu Yu Ye": "叶秋语",
    "Hanyu Guo": "郭涵煜",
    "Yifan Xu": "徐一璠",
    "Yexin Ma": "马烨欣",
    "Xinran Sun": "孙心然",
    "Su-Wei Hsieh": "谢淑薇",
    "Fang-Hsien Wu": "吴芳娴",
    "Hao-Ching Chan": "詹皓晴",
    "Latisha Chan": "詹咏然",
    "Jaqueline Cristian": "克里斯蒂安",
    "Sorana Cirstea": "科斯蒂亚",
    "Elena-Gabriela Ruse": "鲁塞",
    "Anca Todoni": "托多尼",
    "Irina-Camelia Begu": "贝古",
    "Anastasia Pavlyuchenkova": "帕夫柳琴科娃",
    "Anastasia Potapova": "波塔波娃",
    "Kamilla Rakhimova": "拉希莫娃",
    "Anna Blinkova": "布林科娃",
    "Polina Kudermetova": "波利娜·库德梅托娃",
    "Anastasia Zakharova": "扎哈罗娃",
    "Erika Andreeva": "埃里卡·安德烈耶娃",
    "Oksana Selekhmeteva": "谢列赫梅捷娃",
    "Victoria Azarenka": "阿扎伦卡",
    "Aliaksandra Sasnovich": "萨斯诺维奇",
    "Yulia Putintseva": "普汀塞娃",
    "Elina Avanesyan": "阿瓦涅相",
    "Rebecca Sramkova": "什拉姆科娃",
    "Anna Karolina Schmiedlova": "施米德洛娃",
    "Viktorija Golubic": "戈卢比奇",
    "Jil Teichmann": "泰希曼",
    "Marie Bouzkova": "鲍兹科娃",
    "Katerina Siniakova": "西尼亚科娃",
    "Tereza Valentova": "瓦伦托娃",
    "Sara Bejlek": "贝莱克",
    "Laura Samson": "萨姆森",
    "Linda Fruhvirtova": "琳达·弗鲁维尔托娃",
    "Petra Kvitova": "科维托娃",
    "Kimberly Birrell": "比勒尔",
    "Ajla Tomljanovic": "汤姆贾诺维奇",
    "Talia Gibson": "吉布森",
    # prozorova-concussion-withdrawal-2026（2026-09-26）定下的译名
    "Eugenie Bouchard": "布沙尔",
    "Dominika Cibulkova": "齐布尔科娃",
    "Miyu Kato": "加藤未唯",
    "Sofya Lansere": "兰瑟雷",
    "Ellen Perez": "佩雷斯",
    "Priscilla Hon": "韩天遇",
    "Emerson Jones": "埃默生·琼斯",
    "Katie Boulter": "布尔特",
    "Sonay Kartal": "卡尔塔尔",
    "Francesca Jones": "弗朗西斯卡·琼斯",
    "Peyton Stearns": "斯特恩斯",
    "Alycia Parks": "帕克斯",
    "Bernarda Pera": "佩拉",
    "Caroline Dolehide": "多勒海德",
    "Taylor Townsend": "汤森德",
    "Hailey Baptiste": "巴普蒂斯特",
    "Ann Li": "安·李",
    "Katie Volynets": "沃利内茨",
    "Robin Montgomery": "蒙哥马利",
    "Venus Williams": "大威廉姆斯",
    "Serena Williams": "小威廉姆斯",
    "Varvara Gracheva": "格拉乔娃",
    "Diane Parry": "帕里",
    "Elsa Jacquemot": "雅克莫",
    "Caroline Garcia": "加西亚",
    "Kristina Mladenovic": "姆拉德诺维奇",
    "Laura Siegemund": "西格蒙德",
    "Tatjana Maria": "玛丽亚",
    "Eva Lys": "利斯",
    "Jule Niemeier": "尼迈尔",
    "Ella Seidel": "赛德尔",
    "Cristina Bucsa": "布克沙",
    "Jessica Bouzas Maneiro": "博萨斯·马内罗",
    "Sonja Zhiyenbayeva": "日延巴耶娃",
    "Sara Sorribes Tormo": "索里贝斯·托尔莫",
    "Lucia Bronzetti": "布龙泽蒂",
    "Elisabetta Cocciaretto": "科恰雷托",
    "Sara Errani": "埃拉尼",
    "Camila Osorio": "奥索里奥",
    "Emiliana Arango": "阿朗戈",
    "Nadia Podoroska": "波多罗斯卡",
    "Renata Zarazua": "萨拉苏亚",
    "Solana Sierra": "谢拉",
    "Suzan Lamens": "拉门斯",
    "Arantxa Rus": "鲁斯",
    "Greet Minnen": "米宁",
    "Anna Bondar": "邦达尔",
    "Dalma Galfi": "加尔菲",
    "Viktoriya Tomova": "托莫娃",
    "Tamara Zidansek": "齐丹舍克",
    "Antonia Ruzic": "鲁日奇",
    "Zeynep Sonmez": "森梅兹",
    "Mayar Sherif": "谢里夫",
    "Maria Sakkari": "萨卡里",
    "Anhelina Kalinina": "卡利尼娜",
    "Yuliia Starodubtseva": "斯塔罗杜布采娃",
    "Daria Snigur": "斯尼古尔",
    "Moyuka Uchijima": "内岛萌夏",
    "Aoi Ito": "伊藤葵",
    "Nao Hibino": "日比野菜绪",
    "Ena Shibahara": "柴原瑛菜",
    "Wakana Sonobe": "园部八奏",
    "Bianca Andreescu": "安德莱斯库",
    "Marina Stakusic": "斯塔库西奇",
    "Gabriela Dabrowski": "达布罗夫斯基",
    "Lulu Sun": "孙璐璐",
    "Erin Routliffe": "劳特利夫",
    "Ons Jabeur": "贾巴尔",
    "Simona Halep": "哈勒普",
    # 2026-09-17 补：`zhang-shuai-story` 讲 2016 澳网，八强对手是她。
    # CLAUDE.md「表里没有的，定下来之后补进表里，别只写在稿子里」。
    "Johanna Konta": "孔塔",
    # ===== 补充批次：巡回赛常客 / 挑战赛升组球员（新华社式音译） =====
    # ATP
    "Alex Molcan": "莫尔坎",
    "Andrea Pellegrino": "佩莱格里诺",
    "Marco Trungelliti": "特伦杰利蒂",
    "Daniel Merida": "梅里达",
    "Titouan Droguet": "德罗盖",
    "Stefano Travaglia": "特拉瓦利亚",
    "Adolfo Daniel Vallejo": "巴列霍",
    "Hugo Dellien": "德利恩",
    "Federico Coria": "科里亚",
    "Albert Ramos-Vinolas": "拉莫斯-维尼奥拉斯",
    "Otto Virtanen": "维尔塔宁",
    "August Holmgren": "霍尔姆格伦",
    "Tomas Barrios Vera": "巴里奥斯·贝拉",
    "Juan Manuel Cerundolo": "胡安·塞伦多洛",
    "Harold Mayot": "马约",
    "Pierre-Hugues Herbert": "赫伯特",
    "Gregoire Barrere": "巴雷尔",
    "Kyrian Jacquet": "雅凯",
    # WTA
    "Vendula Valdmannova": "瓦尔德曼诺娃",
    "Alina Korneeva": "科尔涅娃",
    "Clara Burel": "布雷尔",
    "Elena Micic": "米契奇",
    "Gina Feistel": "费斯特尔",
    "Lilli Tagger": "塔格尔",
    "Lisa Pigato": "皮加托",
    "Claire Liu": "刘婧文",
    "Nikola Bartunkova": "巴尔通科娃",
    "Barbora Palicova": "帕利佐娃",
    "Dominika Salkova": "沙尔科娃",
    "Darja Vidmanova": "维德曼诺娃",
    "Julia Grabher": "格拉布赫尔",
    "Sinja Kraus": "克劳斯",
    "Panna Udvardy": "乌德瓦尔迪",
    "Celine Naef": "内夫",
    "Simona Waltert": "瓦尔特特",
    "Katarzyna Kawa": "卡瓦",
    "Linda Klimovicova": "克利莫维乔娃",
    "Anastasia Tikhonova": "吉洪诺娃",
    "Maria Timofeeva": "季莫费耶娃",
    "Victoria Jimenez Kasintseva": "希门尼斯·卡辛采娃",
    "Marina Bassols Ribera": "巴索尔斯·里韦拉",
    "Leyre Romero Gormaz": "罗梅罗·戈尔马斯",
    "Nuria Parrizas Diaz": "帕里萨斯·迪亚斯",
    "Tyra Caterina Grant": "格兰特",
    "Julia Riera": "里埃拉",
    "Nuria Brancaccio": "布兰卡乔",
    "Georgia Pedone": "佩多内",

    # ===== 已退役 / 历史人物 =====
    # 知识帖里经常要提上一个年代的人，而两张表都只收现役球员——写稿时
    # grep 不到就只能手打，正是「人名不要手打」那条踩过两次的坑。定下来
    # 就补进表里，让 player_zh() 和 test_人名要以译名表为准 都查得到。
    "Goran Ivanisevic": "伊万尼塞维奇",
    "Patrick Rafter": "拉夫特",
    "Kim Clijsters": "克里斯特尔斯",
    "Caroline Wozniacki": "沃兹尼亚奇",
    "Evonne Goolagong": "古拉贡",
    "Martina Navratilova": "纳芙拉蒂洛娃",
    "Na Li": "李娜",

    # ↓ 2026-09-06 补。触发的是「赛场之上」伊埃拉美网第三轮那条：官方的出场
    # 视频里，通道尽头那块屏上印着 `PRESSURE IS A PRIVILEGE / BILLIE JEAN KING`，
    # 旁白要念这个名字。**仓库里已经在用了，表里却没有**——
    # `zh/tournaments.py` 的 `billie jean king cup → 比利·简·金杯`、
    # `render/tournament_story.py` 里那句「推动者正是比利·简·金」，
    # 两处都是这个写法，所以照它写，不另起一个「比莉」。
    # 正是「人名不要手打，先查仓库里的译名表」那条踩过两次的坑，只是这回
    # 错在**表里根本没有可查的**：`player_zh("Billie Jean King")` 原样返回英文。
    "Billie Jean King": "比利·简·金",

    # ↓ 2026-07-29 补。触发的是两件事，都指向同一个漏：
    #
    # 1. **仓库里已经在用了，表里却没有**——`render/tournament_story.py` 和
    #    `render/knowledge.py` 里「费德勒」出现在 8 个文件、「纳达尔」7 个、
    #    「穆雷」7 个，全是手打的。正是「人名不要手打，先查仓库里的译名表」
    #    那条踩过两次的坑，只是这回错在**表里根本没有可查的**。
    # 2. **中文热搜要拿它当关键词**（`research/zh_trends.py`）。表里没有，
    #    微博上一条「费德勒回应」就会被判成和网球无关。
    "Roger Federer": "费德勒",
    "Rafael Nadal": "纳达尔",
    "Andy Murray": "穆雷",
    "Andre Agassi": "阿加西",
    "Pete Sampras": "桑普拉斯",
    "Boris Becker": "贝克尔",
    "Bjorn Borg": "博格",
    "John McEnroe": "麦肯罗",
    "Jimmy Connors": "康纳斯",
    "Andy Roddick": "罗迪克",
    "Ivan Lendl": "伦德尔",
    "Lleyton Hewitt": "休伊特",
    "Marat Safin": "萨芬",
    "Juan Martin del Potro": "德尔波特罗",
    "Dominic Thiem": "蒂姆",
    "Steffi Graf": "格拉芙",
    "Monica Seles": "塞莱斯",
    "Chris Evert": "埃弗特",
    # ===== 比利·简·金杯介绍片（bjk-cup-story，2026-09-18）补的历史人物 =====
    "Arantxa Sanchez Vicario": "桑切斯·维卡里奥",
    "Conchita Martinez": "康奇塔·马丁内斯",
    "Francesca Schiavone": "斯齐亚沃尼",
    "Flavia Pennetta": "佩内塔",
    "Roberta Vinci": "文奇",
    "Barbora Strycova": "斯特里科娃",
    "Bobby Riggs": "里格斯",
    "Martina Hingis": "辛吉斯",
    "Justine Henin": "海宁",
    "Agnieszka Radwanska": "拉德万斯卡",
    "Agnieszka Radwańska": "拉德万斯卡",
    "Johanna Larsson": "拉尔松",
    "Daniela Hantuchova": "汉图楚娃",
    "Daniela Hantuchová": "汉图楚娃",
    "Maria Sharapova": "莎拉波娃",
    "Ana Ivanovic": "伊万诺维奇",
    # 2026-08-15 补：世界第一在位周数那条片子要念到她（连续 114 周排第四）。
    # ⚠️ 表里原来只有「巴蒂斯塔·阿古特」——**「巴蒂」是它的前缀**，grep 得到的是
    # 另一个人。这正是「人名不要手打，先查仓库里的译名表」那条要防的形状，
    # 只不过这次查得到的那条是个假命中。
    "Ashleigh Barty": "巴蒂",
    # 「纳达尔学院」那条选题要念到的三个**非球员**。他们不在任何一份前 500 快照里
    # （教练和退役球员都不在），所以家在这儿。⚠️ 三个都写**全名**：
    # 「鲁德」在表里已经是卡斯珀·鲁德，父子同姓，只写姓会指错人；「纳达尔」同理。
    "Christian Ruud": "克里斯蒂安·鲁德",   # 卡斯珀·鲁德的父亲，至今是他的主教练
    "Toni Nadal": "托尼·纳达尔",           # 纳达尔的叔叔兼启蒙教练，学院院长
    "Carlos Moya": "卡洛斯·莫亚",          # 前世界第一，学院技术总监
    # 「幸运落败者」那条选题里要念到的人，全部已退役或已改名——**前 500 的快照
    # 永远同步不到他们**（`update_player_names.py` 只抓现役前 500），所以家在
    # 这儿，不在 `data/player_name_overrides.json`：那个文件只喂快照生成器。
    # 译名一律取中文维基条目的姓，写全名的惯例只留给需要区分的人。
    "Paolo Lorenzi": "洛伦齐",          # 保罗·洛伦齐，2017 乌马格决赛负于卢布列夫
    "Kiki Bertens": "贝尔腾斯",         # 基基·贝尔腾斯——是「腾」不是「滕」
    "Andrea Petkovic": "佩特科维奇",
    "Stefanie Voegele": "沃格勒",       # 史蒂芬妮·沃格勒
    "Stefanie Vogele": "沃格勒",        # 带变音的写法数据源里两种都出现过
    "Nicole Vaidisova": "瓦伊迪索娃",   # 妮科莱·瓦伊迪索娃
    # 同一个人：2019 年林茨时叫 Kozlova，后改姓 Baindl。两个都要认得——
    # 她先在林茨输给幸运落败者高芙，四年后又在布达佩斯决赛输给幸运落败者
    # 季莫费耶娃，这条选题里两次都点到她。
    "Kateryna Kozlova": "科兹洛娃",
    "Kateryna Baindl": "班德尔",
    # 1995 年温网以幸运落败者身份打进第四轮的比利时人，男子那边至今的天花板。
    # 名字**画在示意图上**，「示意图也要扫」那条同样管——不能只写在稿子里。
    "Dick Norman": "诺曼",
    # 「中段长得一模一样」那条要念到的四个人，德约科维奇 2018 年那半年的对手。
    # 同样进不了前 500 快照（安德森、郑泫、克利赞都已退役，埃德蒙德早不在榜），
    # 所以家也在这儿。
    "Kevin Anderson": "安德森",         # 2018 温网决赛负于德约科维奇
    "Hyeon Chung": "郑泫",              # 정현，韩国人，2018 澳网第四轮淘汰德约科维奇
    "Martin Klizan": "克利赞",          # 2018 巴塞罗那第二轮淘汰德约科维奇
    "Kyle Edmund": "埃德蒙德",          # 2018 马德里第二轮淘汰德约科维奇
    # 现在不打球、但会以别的身份出现在稿子里的前球员。补进来的理由和球员一样：
    # 写稿时凡是外国人名先查这张表，查不到就会现编——这一条是加拿大大师赛
    # 赛事总监（前 WTA 球员，生涯最高第 112 位），mandatory-1000 那条要引她的话。
    "Valerie Tetreault": "泰特罗",
    "Valérie Tétreault": "泰特罗",
    "Ken Rosewall": "罗斯沃尔",           # 1974 年温网四强，公开赛年代最年长的那个纪录
    # 「资格赛最远能打到哪儿」那条冷知识片要点名的六个人。补进表的理由和罗斯沃尔
    # 一样：写稿时凡是外国人名先查这张表，查不到就会现编。六个人都是公开赛年代
    # 从资格赛打进大满贯半决赛的（男四女二），生涯早就结束，不会进 top500。
    # ⚠️ Gilles Müller（2008 美网资格赛八强）**故意不补**：他的通行译法「米勒」
    # 在这张表里已经归 Alexandre Muller 了，同一个账号不能把两个人叫同一个名字。
    # 那条片子的图卡因此在美网那一格只写年份，不点名。
    "Aslan Karatsev": "卡拉采夫",          # 2021 澳网，公开赛年代最近一个资格赛男单四强
    "Filip Dewulf": "德沃夫",              # 1997 法网
    "Vladimir Voltchkov": "沃尔奇科夫",     # 2000 温网
    "Bob Giltinan": "吉尔蒂南",            # 1977 年 12 月那届澳网
    "Christine Matison": "马蒂森",         # 1978 澳网，女子第一个
    "Alexandra Stevenson": "史蒂文森",      # 1999 温网
    # 中国线：这几个是本人的名字，不是音译
    "Michael Chang": "张德培",
    "Shuai Peng": "彭帅",
    "Jie Zheng": "郑洁",
    # 2026 名古屋亚运会中国网球队那份名单上的人，表里原来没有（前四位不在 top500，
    # 所以 player_names_top500.json 也兜不住）。前四位是本人的名字、不是音译，
    # 英文写法按报名表的「名 姓」序；王蔷是 2018 雅加达亚运会女单金牌得主。
    "Qiang Wang": "王蔷",
    "Weiyi Kong": "孔维一",
    "Tianhui Zhang": "张天汇",
    "Fanming Meng": "孟凡茗",
    "Fumin Jiang": "姜福民",
    "Denis Istomin": "伊斯托明",   # 乌兹别克斯坦，2018 雅加达亚运会男单金牌
}
