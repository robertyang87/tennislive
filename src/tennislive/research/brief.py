...
_DEEP_SYSTEM = (
    ...
    "⚠️ 尤其**不许说这个人是怎么进签表的**"
    "（外卡／资格赛／保护排名／特殊豁免／直入），除非正文里写了。"
    "讲那条规则本身没问题，说「他用了这条」必须有出处。"
    "确实想不出对得上的规则就留空字符串。\n"
    "全部用简体中文。"
)
...
#
# 喂进去之后（run 30992159194，走 `push=false` 验的，没发微信）**十条里九条有
# 内容**，而且真的贴着那条线走：退赛与截止日那两条把 late withdrawal 的罚则
# 写出来了，混双改制那条把辛辛那提的外卡和美网的改制并起来比。
#
# ⚠️ **但同一趟里「编情节」这一类出现了三次，全是同一个形状——断言这个人是
# 怎么进的签表**：德雷珀「持外卡而非保护排名进入签表」（他是直入的前世界
# 第四）、孟菲尔斯「外卡选手」、柳托娃「凭资格赛外卡」，正文里一个都没写。
#
# 这一类特别容易犯，因为角度表里那几条常青线（保护排名／特殊豁免／外卡／
# 资格赛）**本身讲的就是入场方式**——喂了那条线，模型顺手就替这个人认领了
# 一种。所以那句禁令要点名这一类，不能只写「不许编情节」：
# **讲规则本身可以，说「他用了这条」必须有出处。**
#
# 「失败之后要防住那一类，不是那一个」（CLAUDE.md）。判据是下一趟 `push=false`
# 的 run——这条线现在验一次不用发微信了。
