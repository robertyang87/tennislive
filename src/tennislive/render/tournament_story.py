...
STORIES = STORIES + (
    ...
    _trivia_story(
        ...
        facts=(
            ...
            "**他 2025 年是怎么进的正赛——官方签表原件说了算**：protennislive "
            "`posting/2025/747/mds.pdf` 上，外卡标 `WC`（4 张之外的 3 个位置："
            "ZHANG Zhizhen / BU Yunchaokete / SHANG Juncheng）、资格赛标 `Q`、"
            "幸运落败者标 `LL`（RINDERKNECH），而 **19 号位 `TIEN, Learner` "
            "前面一个标记都没有** —— 直接入围。同页 `Last Direct Acceptance: "
            "Etcheverry, Tomas Martin - 59`，那一年的直入线画在世界第 59。"
            "⚠️ 有搜索摘要说他「以幸运落败者顶替西西帕斯进正赛」，**是错的**："
            "那年的 LL 是林德克内希，顶的是伤退的埃切维里，西西帕斯根本不在这张表上。"
            "维基原始 wikitext 的 `RD1-seed03=` 为空，和官方签表对得上。",
            "**那张卡要是最后作废了，正赛不会少人**：7.12 底下的判例 `Unused Wild Cards` "
            "写着 —— 资格赛还没开打，`the next player on the acceptance list is "
            "moved into the main draw`；资格赛已经开打，`a lucky loser is to be "
            "inserted`。**入围名单比正赛线长**，这是签表原件印着的事实而不是推论："
            "2025 年正赛那张表上 `Withdrawals: T. Etcheverry (Left rib)` ＋ "
            "`Alternates/Lucky Losers: A. Rinderknech (LL)`，直入线最后一位伤退，"
            "位置就这么补上了；资格赛签表 16 人抢 4 个名额（`Last Direct "
            "Acceptance: N. Basavareddy - 104`），落败者 12 个，表里还有 3 个 `Alt` "
            "是替补队列顶上来的。**结构上不可能缺人。**",
            "**他为什么没在 9/1 那份名单上：双方都没解释，只有日程可查**。"
            "9/18 戴维斯杯资格赛第二轮捷克 vs 美国在布拉格（本仓库 "
            "`specs/reels/mensik-tien-davis-cup-2026-q2.json` 三源核过，他 6-2 6-4 "
            "胜门西克）；9/25-27 拉沃杯在伦敦 O2，他首次入选世界队，而 lavercup.com "
            "的名单公告是 **2026-06-25**，比中网报名截止（赛事周周一往前推约 28 天，"
            "即 8 月底）早两个多月；9/30 中网在北京开打。"
            "⚠️ 「所以他才没报名」是**推论不是事实**，片子里只摆日程，不替任何人给理由。",
