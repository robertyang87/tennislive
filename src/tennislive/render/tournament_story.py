...
STORIES = STORIES + (
    ...
    _trivia_story(
        ...
        hero=(
            "莱巴金娜在多伦多打进决赛又输掉，仍是世界第二——那个位置她一周都没坐过。"
            "设榜五十年只有 29 个人坐上去过：连续最久的格拉芙和小威廉姆斯各 186 周；"
        ...
        facts=(
            ...
            "「我们早年的记录工作并不完善」。",
            "⚠️ 片子里的排名差用的是**官方已经发布的 354 分**"
            "（wtatennis.com 排名接口，`rankedAt=2026-08-10`：萨巴伦卡 8670、莱巴金娜 8316），"
            "**两个媒体数字都没有采用**：cincinnatiopen.com 08-12 推的「夺冠也还差 4 分」是"
            "决赛前的假设句；WTA 官方 08-14 Rankings Watch 写的「narrowed the gap to just "
            "54 points」也站不住——账号所有者一句「不止差 54 分吧，如果夺冠才多得 50 分"
            "这不符合逻辑啊」点破，算下来是 214："
            "莱巴金娜掉 2025 蒙特利尔四强 390、拿多伦多亚军 650 → 8576"
            "（和赛前推的「夺冠 8926」对得上，8926−350＝8576）；"
            "萨巴伦卡 2025 因疲劳退出蒙特利尔、一分不用掉，2026 输在第四轮 +120 → 8790。"
            "54 需要她净掉 40 分，而她无分可掉。"
            "**官方发的推算和官方发的排名不是一回事**，前者会算错——查产物，不查信号。",
            "⚠️ 片子里**不写「史上最小分差」**：0.8 分（平均分制）、354 分（52 周滚动累计）、"
        ...
        moments=(
            ...
            ChampionMoment(
                ...
                detail=(
                    "官方 08-10 排名：萨巴伦卡 8670、莱巴金娜 8316，差 354 分。"
                    "她生涯从未当过世界第一——29 人的名单里没有她。"
                ),
                source_url=(
