...
def pin_asset_revision(html_content: str, revision: str) -> str:
    ...
    钉进 URL 的是 sha 的**前 `ASSET_REVISION_PIN_LEN` 位**，不是整整 40 位——
    来路和那个数怎么定的写在 `cdn.pin_ref`（2026-09-14：整串会把图多的推送顶过
    PushPlus 的 2 万字上限）。

    ⚠️ **截断只走 `pin_ref`，别在这儿再写一遍。** 钉 sha 在这个仓库里有**两处**
    （这儿的正则替换 ＋ `pushplus._pages_image_url` 重拼 URL），而后者随后按整个
    URL 字符串全文替换，**会把这儿的结果整个盖掉**——只改一处的样子是「改对了
    的那一半看不出来」。
    """
