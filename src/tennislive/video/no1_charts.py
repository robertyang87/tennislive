...
#
# ⚠️ 第一行改过两次，两次都是同一个毛病的两半，记下来别再犯：
#
# 1. 原来写「4 分 · 赢下多伦多冠军也还差 4 分」——**假设句**（账号所有者
#    2026-08-03：「不要用写这些假设，就用实际举例」），而且和真实数字硬矛盾。
# 2. 改成「54 分」（WTA 官方 08-14 Rankings Watch 的原话）之后，账号所有者顶回来：
#    「不止差 54 分吧，如果夺冠才多得 50 分这不符合逻辑啊」。**他是对的**，算得出来：
#      · 官方 08-10 排名 萨巴伦卡 8670 / 莱巴金娜 8316，差 354（多伦多决赛 08-13 才打）
#      · 莱巴金娜掉 2025 蒙特利尔四强的 390，拿多伦多亚军 650 → 8576
#        （这个数和 tennisuptodate 赛前推的「夺冠 8926」对得上：8926−350＝8576）
#      · 萨巴伦卡 2025 因疲劳退出蒙特利尔，**一分都不用掉**；2026 输在第四轮 → +120
#        → 8790
#      · 8790 − 8576 = **214**。而 54 需要萨巴伦卡净掉 40 分，她无分可掉。
#    所以 54 也站不住。**两个媒体数字都不用**，只印官方已经发布的那个：354 分
#    （wtatennis.com 排名接口 rankedAt=2026-08-10，谁都点得开）。
#
# 教训：官方发的**推算**（Rankings Watch 是对下周排名的预告）和官方发的**排名**
# 不是一回事，前者会算错。查产物，不查信号——排名接口才是产物。
MARGINS: tuple[tuple[str, str, str], ...] = (
    ("354 分", "2026-08-10 官方排名", "莱巴金娜和萨巴伦卡的差距"),
...
def margin_ladder() -> str:
    """354 分 → 0.8 分 → 0 分。最后一行是并列，所以描边和垫底都换成强调色。
