...
#: 一年下来，参赛国是怎么从 157 收到 8 的。照片拍不出「漏斗」这件事本身，
#: 而这条片子的第 ① 屏就是在回答「八张门票哪儿来的」。
...
AUTO_PUSH_SLUGS: frozenset[str] = frozenset({
    ...
    # 在分支上渲的 **run 36250119375**（explainer.yml push=false）落库成功，微信那步没跑。
    # 成片从 Release 拉回本地量过——**236.87 秒、1080×1440、6,680,460 字节**
    # （和 `render.json` 逐位相同）；音画差 **0.04 秒**（236.867 / 236.829）；
    # 全片 mean −24.0 / max −4.1 dB，七个采样窗峰值 −6.6~−4.1 dB，**没有哑场**。
    # `check_explainer_voice` 确认云健 `zh-CN-YunjianNeural +22%`、9 段。
    # 九屏本地逐屏渲出来看过；字幕逐行回读，排名全是阿拉伯数字
    # （「阿尔卡拉斯世界第3」「500赛的常规外卡有3个名额」）。
    # ⚠️ 2026-09-26 同日重做（补「先例」屏、写明问题出在选手这一边）再验一次：
    # run 36251845082 落库成功；成片 **263.47 秒、7,235,616 字节**（和 render.json
    # 逐位相同），音画差 0.04 秒，六个采样窗峰值 −8.4~−5.0 dB 无哑场，10 段；
    # 字幕回读「问题出在选手这一边的报名手续上」整句一行。同日目录，
    # `pushed.json` 删掉让合并重推（CLAUDE.md 2026-09-22「重渲默认重推」）。
    ...
    # 成片从 Release 拉回本地量过——**363.43 秒、1080×1440、30fps、12,127,945 字节**；
    # ⚠️ 前一趟 run 35507491281 量的是同一条片子的另一次编码（12,132,134 字节，
    # 时长／音画差／dB 全部相同）——**字节数会因编码不确定性变几千**，
    # 别拿它当「是不是同一版」的判据，判据是 `narration.json` 和成片本身。
    # 音画等长（视频 363.433 / 音频 363.385，差 **0.048 秒**）；
    # 全片 mean −24.2 / max −3.9 dB，十个采样窗峰值 **−7.5~−5.2 dB，没有哑场**
    # （数字静音是 −91）。`check_explainer_voice` 确认云健
    # `zh-CN-YunjianNeural +22% +0Hz`、**11 段**（封面 ＋ 10 屏）。
    # `check_explainer_landed` 逐词查过（「18 个必算」「满 18 岁」「同一个结构」全部
    # 已落地），⚠️ **并拿「羽毛球世界排名」做过负对照**，它报「一个文件都没有」。
    # 十一屏本地逐屏渲出来看过，成片另抽 4 帧确认**示意图标题和字幕真的烧进了画面**
    # （第 ⑦ 屏「一年最多能打几站职业赛」、第 ③ 屏「必算的 18 个格子，两边一样多」，
    # 字幕是阿拉伯数字：「15岁10站 16岁12站 17岁16站」「11个钉死 剩7个自己挑」）。
    # ⚠️ **这一版修掉的最重一条是事实错**：原来第 ⑧ 屏说「年终总决赛两边的算法正好
    # 相反」——回两本规则书原文核，**两边都是 18 个必算 ＋ 总决赛额外加一个**，
    # 差别只在 ATP 把它算进了对外报的那个数。账在 docs/ranking-rules-research.md。
    # ⚠️ 留一条给下一个人：**画布是 1080×1440**，不是 `finals-venues` 那条记的 1920
    # ——`canvas_height()` 默认返回 `CARD_H`(1440)，只有 `_OPENINGS` 里显式写
    # `"canvas": "9:16"` 才是 1920，而活着的那批一条都没写。
    # ⚠️ 语速按 **5.94 字/秒** 走（2157 字 / 363.4 秒）——比 `finals-venues` 实测的
    # 4.90 快一档，拿 4.90 去估这条会高估一分多钟。
    "ranking-math",
    ...
    "second-serve-clock",
    # 2026-09-15 验过才加进来的（同上那句「加进来之前先问一句：这条片子验过了吗」）：
    # 在分支上渲的那趟落库成功；成片从 Release 拉回本地量过——120.13 秒、1080×1920、
    # 4,161,309 字节和 render.json 的 video_bytes 逐位相同；六屏抽帧墙逐格看过
    # （三张示意图的数都对得上：条长按周数线性 600/912＝282/428＝156/237＝0.658，
    # 末屏那张 viewBox 收到 500 之后序号药丸不再压住「12 德约科维奇」那一行）；
    # `check_explainer_landed --local` 报「已落地」，五条小标全命中；
    # 小红书正文 964 字，过得了 #899 那道 1000 字的闸。
    #
    # ⚠️ **全量测试是在本地跑的，而且必须在本地跑**：渲染产物那条提交是 bot 用
    # GITHUB_TOKEN 推的，CI 给它的 run（34933367750）是 `action_required`、**0 秒空跑**
    # ——也就是这棵树从没拿到过 CI。本地跑在**渲染之后那棵树**上：3326 passed / 0 failed
    # （判据是那一行统计，不是退出码）。
    #
    # ⚠️ 哑场照 `second-serve-clock` 那条读，别照抄更上面那几条的「全片 0 处」：
    # `silencedetect -60dB:d=0.4` 报的长空当只有两处（113.76 +2.35、118.91 +1.20），
    # **两处都在品牌片尾那一段**，正是 LEAD 0.6 ＋ TAIL 1.5 那个形状；逐秒扫下来
    # 低于 −60 dB 的只有 114/115/119/120 这四秒，全片 mean −24.3 / max −4.6 dB。
    #
    # ⚠️ **同名的 `big-three` 知识帖 2026-09-06 发过一条**（`story_state.json` 记着），
    # 讲的是「三巨头的数字」（66 冠 / 18 年年终第一 / 2008 温网 / 2012 澳网）。
    # 这条讲的是 9-14 那期排名把三个人一起挤出前十，材料零重叠——底下那张今日前十
    # 的示意图里「24 个大满贯」是唯一一处碰到旧片子的数，一句带过，不是铺开重讲。
    "big-three",
})
...
from .qualifier_charts import (
    ceiling_cover,
    draw_format,
    men_semifinal_timeline,
    qualifier_board,
)

#: 「资格赛最远打到哪儿」的封面。**这条选题没有诚实的封面照片**——
#: 它讲的是八个格子摆在一起时的样子，而任何一张球员实拍都只讲得了一个格子。
#: 画的是最硬的那一个对比：女子那条到冠军，男子那条停在半决赛。
_QUALIFIER_COVER_DIAGRAM = ceiling_cover()

from .serve_clock_cards import (
    gap_timeline,
    next_gen_lab,
    rule_quotes,
    slow_play_history,
)
from .promo_fee_cards import (
    audit_power,
    penalty_ladder,
    who_may_pay,
)
...
# ⚠️ **法网输给纳达尔不止决赛那一次**（账号所有者 2026-08-27 追问）：2013 法网
# **八强**纳达尔也赢过他，而且比分和 2017 决赛**一模一样**都是 6-2 6-3 6-1
#   2013 男单签表  https://en.wikipedia.org/wiki/2013_French_Open_%E2%80%93_Men%27s_singles
#                  原始数字 6,6,6 / 2,3,1；瓦林卡词条同页写「首进八强，直落三盘负于纳达尔」
# ⚠️ 但那是**八强不是决赛**，所以这张「四次决赛」的表不收它——收进来就把
# 这一屏从「决赛」改成了「交手记录」，是另一件事。旁白里带一句就够。
...
_FILS_PRECEDENT_DIAGRAM = """\
<svg viewBox="0 0 900 600" xmlns="http://www.w3.org/2000/svg">
  <text x="450" y="52" text-anchor="middle" fill="#f4fbf7"
        font-size="38" font-weight="800">前 20 打资格赛，以前有过</text>
  <text x="450" y="98" text-anchor="middle" fill="#cfe6d8"
        font-size="28" font-weight="700">两次都是资格赛头号种子</text>

  <rect x="60" y="134" width="780" height="104" rx="14" fill="#8fd6a8" opacity="0.12"/>
  <text x="90" y="178" fill="#cfe6d8" font-size="27" font-weight="700">2018 伊斯特本</text>
  <text x="90" y="220" fill="#f4fbf7" font-size="30" font-weight="800">索克：资格赛第一轮就输了</text>
  <rect x="60" y="254" width="780" height="150" rx="14" fill="#8fd6a8" opacity="0.12"/>
  <text x="90" y="298" fill="#cfe6d8" font-size="27" font-weight="700">2023 女王杯</text>
  <text x="90" y="340" fill="#f4fbf7" font-size="30" font-weight="800">保罗：连赢 2 场，打进正赛</text>
  <text x="90" y="382" fill="#cfe6d8" font-size="27" font-weight="700">同一张签表的 4 号种子：19 岁的菲斯</text>
  <rect x="60" y="420" width="780" height="104" rx="14" fill="#c6f65a" opacity="0.14" stroke="#c6f65a" stroke-width="3"/>
  <text x="90" y="464" fill="#cfe6d8" font-size="27" font-weight="700">2026 东京</text>
  <text x="90" y="506" fill="#c6f65a" font-size="30" font-weight="800">菲斯：世界第 11</text>
  <text x="450" y="566" text-anchor="middle" fill="#cfe6d8" font-size="26" font-weight="700">TennisTemple：2007 年以来排名最高的一次</text>
</svg>
"""
...
_SCRIPTS: dict[str, tuple[tuple, ...]] = {
    ...
    (
        ...
        "先说结论",
        "东京的名单上，没有世界第 11",
        "先说结论。东京站的正赛名单，排名按八月三十一号那一期算：阿尔卡拉斯世界第三，"
        "弗里茨世界第十，蒂亚福世界第十二。世界第十一的菲斯本该排在第三个，名单上却没有他。"
    ...
    (
        ...
        "至于为什么没赶上，官方原话说，问题出在选手这一边的报名手续上——"
        "也就是说，这一次是他们自己漏掉的。"
        "具体漏在哪儿，网上传得最广的说法，是团队在同一周的北京和东京之间犹豫到了最后一刻——"
    ...
    (
        ...
        "第一扇",
        "4 张外卡，没有一张给菲斯",
        "第一扇是外卡。五百赛的常规外卡有三个名额，给谁全凭赛事自己决定："
    ...
    (
        # ⚠️ 2026-09-26 账号所有者追问「历史上是否有类似的案例，高排位的人去打资格赛」
        # 之后补的一屏。两例都查到了签表原件（维基原始 wikitext）：
        #   · 2018 伊斯特本资格赛 Seeds 第 1 位 `Jack Sock (first round)`，签位写着
        #     `1/Alt`（替补身份报进来的），首轮 4-6 6-7(4) 负布兰兹。
        #   · 2023 女王杯资格赛 Seeds：`1. Tommy Paul (qualified)`、`2. Grigor Dimitrov
        #     (qualified)`、`4. Arthur Fils (qualified, withdrew)`——**菲斯本人就在那张表上**
        #     （19 岁，生于 2004-06-12），打上来之后退赛，位置给了林德克内希。
        # ⚠️ 两人当周的**确切**排名两个源对不上（索克 14 / 15、保罗 15 / 17），所以只说
        # 「世界前 20」，不报具体名次。「2007 年以来最高」只有 TennisTemple 一个源，点名出处。
        # ⚠️ 两例**为什么**去打资格赛没查到可靠出处，片子里不替他们给理由。
        "today",
        "先例",
        "以前也有前 20 打资格赛",
        "这种事以前有过，但很少。二〇一八年伊斯特本，世界前二十的索克以资格赛头号种子的身份"
        "去打资格赛，第一轮就输给了布兰兹，连正赛都没进。"
        "二〇二三年女王杯，资格赛的头号种子是同样在前二十的保罗，他连赢两场打了上来。"
        "而那张资格赛签表的四号种子，是十九岁的菲斯——他也打了上来。"
        "法国网球网站 TennisTemple 翻了记录：二〇〇七年以来，"
        "还没有人以比这次更高的排名打过 ATP 的资格赛。",
        "",
        "示意图 · 网球时差绘制",
        (
            "2018 索克 资格赛首轮就输",
    ...
    # ＋ 9.03 Note 3 那扇门）；这一条讲的是**积分怎么算出来的**，零分只在
    # 第 ④ 屏**一句带过**、且落点是「两边绑的人不一样」（ATP 只绑前 30，
    # WTA 够得着正赛就绑）——那一层 `mandatory-1000` 一个字都没讲，
...
""",
        ...
        (
            ...
            "从保送变成了竞争。顺带说一句：多哈和迪拜以前是逐年在一千赛和五百赛之间"
            "轮换的，二〇二四年起两站同时固定成了一千赛，女子独办这一桶也是从那一年"
            "才定型成今天这三站。",
            "",
            "示意图 · 网球时差绘制",
            (
...
""",
    ...
    # 2026-09-16 选题：辛纳去北京、阿卡去东京，同周同级别。账号所有者问「是否
    # 可以说是走出场费」——**不能**，查下来没有任何一手报道把出场费和这两人
    # 这两站连起来，流传的那几个数字全是网球博客和论坛的「据传」，而阿卡本人
    # 给过的理由是「想去一个没打过的球场」，他还是去卫冕 2025 年东京那个冠军。
    # CLAUDE.md「『排除了 A 和 B』不等于『就是 C』」正是这个形状。
    # **所以这条片子的主语是规则，不是这两个人**——他们只是同一周出现在两个
    # 城市的事实。全片一次都不断言谁拿了钱。
    #
    # 条文和罚则的一手出处：ATP 2026 规则书第一章 1.16（2024 版是 1.15，
    # 节号变了是因为 2026 在前面插了一节 Special Events）。两版逐字比过，
    # 828 字符对 828 字符零差异。全套账在 research/promotional-fees-2026.md。
    "promotional-fees": (
        (
            "now",
            "这一周",
            "两个人，去了两个城市",
            "先看这一周。九月三十号到十月六号，辛纳在北京，阿尔卡拉斯在东京。"
            "两站同一天开打，同一天结束，级别一模一样，都是 ATP500。"
            "而这不是第一次。去年这一周也是这样："
            "辛纳在北京六比二、六比二拿下冠军，"
            "阿尔卡拉斯在东京六比四、六比四拿下冠军。"
            "同一个星期，两座奖杯，隔着两千公里。",
            "assets/explainer/promotional-fees/sinner-beijing-2025.jpg",
            "ATP 官方图 · 2025 年 10 月 1 日，北京，辛纳捧起中网男单冠军奖杯",
            (
                "9 月 30 日 两站同周开打",
                "北京和东京 都是 ATP500",
                "去年这一周 他们也是分开的",
        ...
        (
            ...
            "另一边",
            "想去一个没打过的球场",
            "另一边是东京。阿尔卡拉斯二〇二四年在北京拿过冠军，"
            "二〇二五年改去东京，也拿了冠军，今年他回东京卫冕。"
            "他自己给过一个理由：我想去一个不同的地方，一个我从来没打过的球场。"
            "而北京这边今年来了七个世界前十——辛纳是卫冕冠军，"
            "兹维列夫刚拿下美网，德约科维奇时隔十一年回来。"
            "两站同一个级别，同一个星期，都想把人请到自己这儿。",
            "assets/explainer/promotional-fees/alcaraz-tokyo-2025.jpg",
            "ATP 官方图 · 2025 年 9 月 30 日，东京，"
            "阿尔卡拉斯亲吻日本公开赛男单冠军奖杯",
            (
                "2024 在北京夺冠 2025 改去东京",
                "今年回东京卫冕",
                "北京来了 7 个世界前十",
            ),
        ),
        (
            ...
            "规则书",
            "它不叫出场费，叫推广费",
            "所以翻开规则书。ATP 二〇二六年那本，第一章第一点一六条，"
            "标题是推广费。"
            # ⚠️ 不能写「直接或者间接**给**的钱」：切词器把「给」黏进来，
            # 切成「间 ｜ 接给」，而「间」单独站着念 jiān（「间接」是 jiàn）。
            # 逗号合不拢一个该合的词，所以是把「给」挪走，不是加标点。
            "第一款写着：球员不得接受任何来源、直接或者间接的钱，"
            "或者任何有价值的东西，用来影响或者确保他参赛，奖金除外。"
            "第二款给了一个例外，整本书里只有这一个。"
            # ⚠️ 「一个都没有」是全称断言，出处是 2024 和 2026 两本全文各搜一遍
            # （`appearance fee` 两版都是 0 处），记在 research 那份里。
            "顺带一提，大家都管它叫出场费，"
            "而规则书里从头到尾没有出场费这三个字。",
            "assets/explainer/promotional-fees/rulebook-116.png",
            "ATP 2026 官方规则书 第一章 1.16 · 网球时差截取并高亮",
            (
                "1.16 条 标题是「推广费」",
                "默认：一分都不许收",
                "全书没有「出场费」这个词",
        ...
        (
            ...
            "那个例外",
            "级别越高，反而越不许",
            "那个例外是这么划的：ATP500 和 ATP250 这两档赛事，"
            "可以支付推广服务费。"
            "其余任何一档，赛事方、运营方、赞助商、代理人，一律不许给。"
            "也就是说，大满贯不许，大师赛不许，而五百赛和二百五十赛可以。"
            "级别越高反而越不许，听起来是反的，"
            "可原因很直接：那几档是强制参赛的，人本来就得来，不用买。"
            "而这两档不强制，来不来是球员自己定——"
            "规则书留给这两档的，是它们唯一能抢人的工具。",
            "",
            "示意图 · 网球时差绘制",
            (
                "500 和 250 可以给",
        ...
        (
            ...
            "有多认真",
            "罚最重的，是不许给的那一档",
            "这条规矩不是写着好看的。违规给钱，球员这一头的罚款上限按级别递增："
            # ⚠️ 级别数后面必须接个「赛」字，否则「二百五十」和「三万」连读成「二百五十三万」
            # ——TTS 会把它念成 253 万。仓库既有旁白写的就是「挑战赛、五百赛、二百五十赛」。
            "挑战赛两万美元，二百五十赛三万，五百赛四万，大师赛六万，"
            "另外还要把那笔钱本身追缴回来，最多可以停赛三年，"
            "连续违规，罚款上限每次翻一倍。"
            "注意最高那一档——大师赛，罚得最狠，"
            "而大师赛本来就一分都不许给。",
            "",
            "示意图 · 网球时差绘制",
            (
                # ⚠️ 卡上四个金额一律阿拉伯数字——旁白那边写「二百五十赛三万」
                # 是为了挡住 TTS 连读，卡片不过 TTS，跟着旁白写就成了
                # 「250 三万」，同一张卡里两种数字体系，而且读不通。
                "挑战赛 2 万 · 250 赛 3 万",
                "500 赛 4 万 · 大师赛 6 万",
                "另追缴那笔钱 最多停赛三年",
            ),
        ...
        (
            ...
            "查得动吗",
            "查得动，可金额从来不公布",
            # ⚠️ 不能写「规则书写着」：切成「规则 ｜ 书写 ｜ 着」，「书写」是个真词，
            # 念 shūxiě，意思全变（tennis-video-craft 那张假词表的第一条）。
            # 补一个「里」把两个字撑开。
            "赛事那一头更重。规则书里写着：ATP 只要认为某一站可能违规，"
            "赛事方必须应要求交出它掌握的全部相关记录；"
            "没有记录的，要出具一份详述事实的宣誓书。"
            "不交，罚款最高十万美元，并且可以终止会籍。"
            "所以这条规矩是有牙的。"
            "可是把整本规则书翻完，有一个数字自始至终没有出现——"
            "到底给了多少。那一栏，从来不公布。",
            "",
            "示意图 · 网球时差绘制",
            (
        ...
        (
            ...
            "回到看台",
            "你买那张票，是为了谁",
            "所以回到这一周。辛纳在北京，阿尔卡拉斯在东京，"
            "两站同一天开打。"
            # ⚠️ 到这儿为止，全片没有说过这两个人拿了钱——也不许说。
            # 能说的只有规则允许，而这一问问的正是那条规则本身。
            "他们为什么去了各自那一站，规则书不会告诉你，"
            "他们自己说的是赛程和球场。"
            "但规则书确实写着：这两档赛事，是可以花钱请人的。"
            "而坐在看台上的那些人，买票的时候是冲着某一个人来的。",
            "assets/explainer/promotional-fees/ariake-crowd.jpg",
            "ATP 官方图 · 东京有明竞技场，日本公开赛主场馆（2023 年赛事主视觉）",
            (
                "能不能给 规则书说了",
                "给没给 给了多少 没人公布",
                "而票是你买的",
    ...
    "second-serve-clock": (
        (
            "rule", "规则", "一发有钟，二发没有",
            "先把规则摆出来。ATP、大满贯、国际网联和 WTA，"
            "共用同一份发球计时程序。"
            "里面第二条只有一句话：计时器只对一发生效。"
            "一发触网要重来，没有钟；二发，也没有钟。"
            "而二〇二六年的大满贯规则书，"
            "对二发的全部要求只有两个词——不得延误。"
            "至于拍几下球，规则里一个数都没有。"
            "注意，没有钟不等于没人管："
            "拖得算不算太久，由主裁自己判断——"
            "半决赛那场，兹维列夫就是这么吃到一次时间违例的。",
            "", "示意图 · 网球时差绘制",
            ("计时器只对一发生效", "拍几下球，规则里没有上限",
             "拖得算不算太久，主裁说了算"),
            rule_quotes(),
        ),
        (
            "now", "这一次", "谢尔顿说，二发不该是无限的",
            "提出这件事的是谢尔顿。"
            "九月十三号的美网决赛，他一比三输给兹维列夫，"
            "没能拿下美国男子等了二十三年的那个冠军。"
            "赛后他说：一发你想怎么用都行，二十五秒用满也没关系；"
            "但二发应该有个限制，不该是无限的。"
            "他说对方二发之前从来不少于十下球，多的时候要拍二十到二十五下。"
            "而网上有人把这届美网的前五场数了一遍——一共一万多下。"
            "兹维列夫自己也提到过这个数字，说挺好笑的。",
            "assets/explainer/second-serve-clock/net-embrace.jpg",
            "USA TODAY 每日图集 · Geoff Burke, IMAGN IMAGES via Reuters · "
            "2026 美网男单决赛，两人在网前拥抱",
            ("9.13 美网决赛，谢尔顿 1-3 负", "他要的是给二发设限",
             "「从来不少于十下球」"),
        ),
        (
            "why", "为什么", "终点画在了一发上",
            "这个空档不是谁忘了写，是计时方式带出来的。"
            "国际网联的规则第二十九条写得很清楚："
            "这二十五秒，从上一分结束开始算，到一发被击出为止。"
            "也就是说，一发打出去那一刻，这二十五秒就已经走完了。"
            "后面无论是擦汗、走动，还是拍二十下球，"
            "都不在任何一段计时里。",
            "", "示意图 · 网球时差绘制",
            ("ITF 规则 29a：算到「一发被击出」", "所以一发触网重发，同样没有钟",
             "那一截不属于任何一个钟"),
            gap_timeline(),
        ),
        (
            "receiver", "接发的人", "被打断的是那个小跳",
            "那这件事，对站在对面的人有影响吗？"
            "半决赛输给兹维列夫的哈恰诺夫是这么说的："
            "你已经准备好了，他又拍十二下，"
            "你只好重新准备，重新做一次分腿垫步，几乎每一分都这样。"
            "分腿垫步是接发之前那个小跳，"
            "落地的时机决定你能不能在第一时间启动，"
            "被打断一次，就得重做一次。"
            "而同一份计时程序里还写着第四条："
            "接发球员仍然有责任，配合发球方的合理节奏。"
            "谢尔顿在场上就撞上了这一条。"
            "主裁提醒他要早点站到底线，他回了一句："
            "他想什么时候开始都行，"
            "我保证不会有他发球、而我没准备好的时候。"
            "他把那条规则称作愚蠢的规则。",
            "assets/explainer/second-serve-clock/waiting.jpg",
            "usopen.org 官方图 · Darren Carroll/USTA · 2026 美网男单决赛，谢尔顿在等",
            ("哈恰诺夫：得重新做一次分腿垫步", "规则第 4 条：接发方要配合发球方",
             "谢尔顿称那条是「愚蠢的规则」"),
        ),
        (
            "crowd", "还有一样", "拍球越久，全场越安静",
            "还有一样东西被一起按住了。"
            "按惯例，球员准备发球的时候，观众要安静下来，"
            "直到这一分打完。"
            "所以拍球的时间越长，全场安静的时间也越长。"
            "那天几乎所有人都在为谢尔顿喊，"
            "而每一次二发，这份声音都要被按住一段。"
            "第二盘三比三、四十平，他中止了抛球，看台上一片嘘声，"
            "主裁不得不提醒全场：一发和二发之间，也不要出声。"
            "到了第四盘二比一、三十平，他自己吃了一次时间违例，"
            "他当场跟主裁说：观众太吵，他没法开始发球动作。",
            "assets/explainer/second-serve-clock/crowd.jpg",
            "usopen.org 官方图 · Garrett Ellwood/USTA · "
            "2026 美网男单决赛，谢尔顿赢下一盘后全场起立",
            ("球员准备发球时，全场要安静",
             "主裁提醒：一发二发之间也别出声",
             "第四盘 2-1 30-30，他吃了时间违例"),
        ),
        (
            "history", "往回看", "拿时间做文章，一直都有",
            "往回看，拿时间做文章这件事，网球里一直都有。"
            "一九八九年法网第四轮，十七岁的张德培对伦德尔。"
            "他在一次换边喝水喝得太久，被判了时间违例，然后接着喝。"
            "同一场他还发了一记下手发球。"
            "最后伦德尔在两个赛点上一发失误、二发擦网出界——"
            "那一分，伦德尔输在了一个二发上，"
            "而张德培成了历史上最年轻的大满贯男单冠军。"
            "二〇一二年澳网决赛，德约科维奇和纳达尔打了五小时五十三分，"
            "是大满贯决赛史上最长的一场。"
            "当年大满贯的规定是分与分之间二十秒，"
            "而 ESPN 在场边量到的是三十秒和三十三秒——"
            "那还是主裁警告过两个人之后的数。"
            "第二年巡回赛开始严格执法。"
            "纳达尔当时的原话是：在潮湿的地方，这条规则完全是灾难；"
            "打完一个长回合，下一分你还打得动一个长回合吗？",
            "", "示意图 · 网球时差绘制",
            ("1989 那场，他还发了一记下手发球", "规定 20 秒，场边量到 30 和 33 秒",
             "纳达尔：打完长回合，下一分还打得动吗"),
            slow_play_history(),
        ),
        (
            "data", "顺带一问", "钟装上了，比赛变快了吗",
            # ⚠️ 这一屏的每个数都从原文核过，别按摘要抄（两个数量级的教训见
            # CLAUDE.md「12040」那条）：
            #   58.5% / 21.5s / 3475 个发球 —— Kolbinger, Großmann, Lames,
            #     《A closer look at the prevalence of time rule violations…》，
            #     Journal of Sports Analytics 2018，量的是 **2016 澳网男单**，
            #     当时的限制是 20 秒。同一篇里「主裁只判了两次（0.1%）」。
            #   26.3% / 21.6s / 2135 分 —— Mühlberger & Kolbinger，
            #     《The serve clock reduced rule violations, but did not speed
            #     up the game》，Journal of Human Sport and Exercise 16(3)，
            #     doi:10.14198/jhse.2021.163.05，量的是 **2018 美网男单**，
            #     25 秒的钟第一次进大满贯。摘要原话：the share of rule
            #     violations decreased (26.3%), but the average time did not
            #     (21.6s)。⚠️ 科尔宾格是两篇的共同作者，所以「前后量过两次」
            #     是实话，不是把两拨人凑成一句统称。
            #   40.2 → 41.0 秒 —— Jeff Sackmann，Tennis Abstract
            #     《Another Slam, Another Pointless Serve Clock》2019-01-31，
            #     澳网男单 2018 对 2019（女子是 40.4 → 40.3）。⚠️ 它量的是
            #     **整分**（含回合），和上面那个 21 秒的**间隔**不是一个口径，
            #     所以旁白里必须点明，不然读者会觉得两个数在打架。
            # ⚠️ 「钟装上之后」不能这么写：edge-tts 把它切成「钟装 ｜ 上」
            # （「钟装」不是词），下面那句原来的「违规的定义松了」切成
            # 「定义松 ｜ 了」，同一个毛病。两处都换了句式撑开，不是加逗号——
            # 标点进不了词边界事件，撑不开该分的两个字（CLAUDE.md 记过）。
            "还有一个容易被跳过的问题：装上钟之后，比赛真的变快了吗？"
            "统计学者科瓦尔奇克在二〇一八年就提醒过："
            "那次改动其实是放宽，不是收紧。"
            "科尔宾格前后量过两次。"
            "限制还是二十秒的时候，澳网男单三千四百多个发球，"
            "超时的占百分之五十八点五，"
            "平均间隔二十一点五秒。"
            "换上二十五秒的钟，再量美网男单："
            "超时降到百分之二十六点三，"
            "平均间隔二十一点六秒。"
            "松的是违规的定义，球员花的时间只差零点一秒。"
            "萨克曼那边量的是整分，连打球那几秒一起算。"
            "澳网前后两年，男子从四十点二秒变成四十一秒，"
            "不但没快，还慢了一点。",
            "assets/explainer/second-serve-clock/from-the-stands.jpg",
            "usopen.org 官方图 · Mike Lawrence/USTA · "
            "2026 美网男单决赛，从看台望向球场",
            ("20 秒的限制：超时 58.5%，平均 21.5 秒",
             "25 秒的钟：超时 26.3%，平均 21.6 秒",
             "澳网整分：40.2 秒 → 41.0 秒"),
        ),
        (
            "closer", "所以", "这条路，已经走过一次",
            "那有没有改进的空间？其实已经试过了。"
            "二〇二三年起，ATP 在自己的新生代总决赛上加了一条："
            "一发和二发之间最多八秒，屏幕上照样有钟。"
            "二〇二四和二〇二五两届，都还在用。"
            "而今天这块二十五秒的钟，"
            "本身就是从同一块试验田出来的——"
            "二〇一七年在那儿第一次亮相，第二年进了大满贯，"
            "第三年铺到整个巡回赛。"
            "也就是说，谢尔顿要的那条规则，"
            "不是没人想过，是已经在跑了。"
            "这一次站出来的是罗迪克。"
    ...
    # 四张当站捧杯照按时间顺序揭晓。最后一屏主动给出澳网反例，把传播标题
    # 收回到“法网起连续四次”的准确范围，避免把巧合包装成因果定律。
    "kostyuk-champion-test": (
        (
            "paris",
            "第一次 · 法网",
            "终结 17 连胜，然后夺冠",
            "第一次，法网半决赛。带着红土十七连胜的科斯秋克，被十九岁的"
            "米拉·安德烈耶娃六比一、六比三终结。两天后，安德烈耶娃又以"
            "六比三、六比二击败赫瓦林斯卡，捧起生涯第一座大满贯。第一次，应验。",
            "assets/explainer/kostyuk-champion-test/andreeva_roland_garros_2026_trophy.jpg",
            "Jimmie48/WTA · 2026 法网女单冠军安德烈耶娃捧杯",
            (
                "半决赛 6-1 6-3 胜科斯秋克",
                "决赛 6-3 6-2 胜赫瓦林斯卡",
                "19 岁 · 生涯首座大满贯",
        ...
        (
            ...
            "第二次 · 温网",
            "首进大满贯决赛，就捧杯",
            "第二次，温网半决赛。诺斯科娃六比四、六比四淘汰科斯秋克；"
            "决赛再打满三盘击败穆霍娃，第一次打进大满贯决赛，就把"
            "维纳斯玫瑰露水盘抱回家。第二次，又应验。",
            "assets/explainer/ten-champions/noskova.jpg",
            "AELTC/Thomas Lovelock · 2026 温网女单冠军诺斯科娃捧杯",
            (
                "半决赛 6-4 6-4 胜科斯秋克",
                "决赛 6-2 5-7 6-3 胜穆霍娃",
                "第一次进大满贯决赛即夺冠",
        ...
        (
            ...
            "第三次 · 多伦多",
            "先丢一盘，随后只丢三局",
            "第三次，多伦多第四轮。斯瓦泰克先丢一盘，随后只让科斯秋克拿到三局，"
            "三比六、六比一、六比二逆转。决赛，她直落两盘击败莱巴金娜，"
            "结束近一年的冠军荒。第三次，还是应验。",
            "assets/explainer/golden-masters/swiatek.jpg",
            "WTA 官方图库 · 2026 多伦多女单冠军斯瓦泰克捧杯",
            (
                "第四轮 3-6 6-1 6-2 胜科斯秋克",
                "决赛 6-2 6-3 胜莱巴金娜",
                "结束近一年的冠军荒",
            ),
        ),
        (
            ...
            "第四次 · 辛辛那提",
            "高芙让这条规律再次应验",
            "第四次，就是刚刚结束的辛辛那提。高芙在八强只丢四局，"
            "六比二、六比二带走科斯秋克；决赛再以六比二、六比四击败佩古拉，"
            "拿到本赛季第一冠。四站、四个人、四座奖杯。"
            "但别把巧合说成全年定律：澳网首轮击败她的雅克莫并没有夺冠。"
            "准确说，这是从法网开始连续四次应验。下一次击败科斯秋克的人，还会捧杯吗？",
            "assets/explainer/kostyuk-champion-test/gauff_cincinnati_2026_trophy.jpg",
            "WTA 官方图库 · 2026 辛辛那提女单冠军高芙捧杯",
            (
                "八强 6-2 6-2 胜科斯秋克",
                "决赛 6-2 6-4 胜佩古拉",
                "准确范围：法网起连续四次",
    ...
    # ⚠️ 这条片子讲的是一张表，而**表最容易画成假的**：口径混一条，八个格子就全错。
    # 三条钉死在 `qualifier_charts` 的模块 docstring 里（公开赛年代 / 不含幸运落败者 /
    # 单打），每一格的两个独立源也在那儿逐条记着。
    #
    # ⚠️ 末屏**只摆规则，不下因果**。「男子走不了那么远是因为要多打两盘」说得通，
    # 而没有任何一个源这么说过——所以旁白最后一句是「这两件事有没有关系，
    # 规则书没有说」，那一问问的正是这个。
    "qualifier-ceiling": (
        (
            "now", "今天", "赢了三场，才拿到入场券",
            # ⚠️ 这里**不写「今天开打」**。美网正赛第一轮跨三天（美东 8/30–9/1，
            # tennismajors 和 olympics.com 的赛程页都这么写），日场美东 11:00
            # ＝北京时间前一天 23:00——CLAUDE.md 那条「北京日历会把美洲赛事的
            # 比赛日拦腰切断」正是这个形状。按北京时间说「今天开打」，对刷到
            # 这条片子的人有一半时候是错的；而「网球有故事」是常青栏目，
            # 相对时间词过一天就作废。
            "美网正赛开打了，第一轮要打三天。郑钦文这一次是从资格赛打进来的。"
            "首轮六比三、六比二赢下尤晓迪，次轮六比一、七比五赢下布雷尔。"
            "决胜轮被大雨切成两天：她先拿下第一盘六比四，"
            "隔夜续赛三比六丢掉第二盘，决胜盘三十六分钟回敬六比零。"
            "肘部手术之后十三个月，她重新回到大满贯正赛。",
            "assets/explainer/qualifier-ceiling/zheng_uso_q3.jpg",
            "WTA 官方图库（摄影 Jimmie48/WTA）· 2026 年 8 月 28 日美网资格赛决胜轮，郑钦文",
            ("三场资格赛，一场没输", "决胜轮跨两天，决胜盘 6-0", "肘部手术之后 13 个月"),
        ),
        (
            "board", "那张表", "女子拿过冠军，男子只到半决赛",
            "所以把这张表摆出来。公开赛年代五十八年，四大满贯，"
            "从资格赛打进正赛的人，最远走到过哪一轮。"
            "女子这一行：澳网半决赛，法网决赛，温网半决赛，美网冠军。"
            "男子这一行：澳网、法网、温网都是半决赛；"
            "美网那一格，四分之一决赛。"
            "八个格子里最亮的和最暗的，都在美网。"
            "最亮那一格是拉杜卡努，二零二一年，"
            "三场资格赛加七场正赛，十场全胜，一盘没丢。",
            "", "示意图 · 网球时差绘制",
            ("女子四站全部进过半决赛", "美网出过冠军，法网出过亚军",
             "拉杜卡努 2021 美网 10 场全胜"),
            qualifier_board(),
        ),
        (
            # ⚠️ 标题第一版写的是「十场全胜，一盘没丢」——那句说的是**拉杜卡努**，
            # 而这一屏的画面是赫瓦林斯卡捧着亚军盘。渲出来一看，读者读到的是
            # 「她十场全胜」，可她刚输掉决赛。CLAUDE.md 那条「替她说话的那句
            # 压在对手身上」就是这个形状，只不过这次压错的是同一边的另一个人。
            # 拉杜卡努那一句挪去了 ② 屏（那儿的画面正是那张表）。
            "women", "最近那一次", "第一场就赢了郑钦文",
            "第二亮的那一格是今年六月。"
            "赫瓦林斯卡赛前世界第一百一十四位，法网正赛还是第一次打。"
            "三场资格赛打上来，正赛第一场就六比四、六比零赢了五号种子——"
            "那是郑钦文。"
            "赫瓦林斯卡一直赢到决赛，三比六、二比六负于安德列耶娃。"
            "公开赛年代打进过大满贯决赛的资格赛球员，只有她们两个。",
            "assets/explainer/qualifier-ceiling/chwalinska_rg_final.jpg",
            "WTA 官方图库 · 2026 年 6 月 6 日法网决赛日，赫瓦林斯卡捧着亚军盘",
            ("赫瓦林斯卡从资格赛打进决赛", "正赛第一场赢的是 5 号种子",
             "打进过决赛的资格赛球员只有 2 个"),
        ),
        (
            "men", "男子这边", "五十八年，五个人",
            "男子这边，五十八年只有五个人。"
            "一九七七年澳网的吉尔蒂南，同一年温网的麦肯罗——"
            "那年麦肯罗十八岁，还是业余身份，"
            "三场资格赛加五场正赛，八场球打进半决赛。"
            "然后是一九九七年法网的德沃夫，二零零零年温网的沃尔奇科夫，"
            "二零二一年澳网的卡拉采夫。"
            "五个人，全部止步半决赛，一个决赛都没有。"
            "而美网那一格到今天还是空的：三次四分之一决赛，就是最好成绩。",
            "", "示意图 · 网球时差绘制",
            ("五个人全部止步半决赛", "麦肯罗那年 18 岁，业余身份", "美网一次半决赛都没有"),
            men_semifinal_timeline(),
        ),
        (
            "format", "有一条规则", "男子在中途换了赛制",
            "有一条规则值得摆出来。翻开大满贯规则书，赛制那一节只有一句话："
            "只有男子正赛是五盘，其余全部是三盘。"
            "也就是说，男女的资格赛都是先赢两盘就过；"
            "进了正赛，女子还是赢两盘，男子要赢三盘。"
            "同一条路，男子在中途换了赛制。"
            "这两件事有没有关系，规则书没有说。",
            "", "示意图 · 网球时差绘制",
            ("资格赛男女都是先赢两盘", "正赛女子赢两盘，男子赢三盘",
             "128 人抢 16 个正赛名额"),
            draw_format(),
            "男子那条线停在半决赛五十八年，你觉得谁会先越过去？",
        ),
    ),
    "gamesmanship": (
        (
    ...
    "wawrinka-wildcard": (
        ...
        (
            # ⚠️ 「一九七八年以来第一个」是**全称断言**，两个独立源核过
            # （2026-08-27 补，此前一处出处都没写）：
            #   NBC Sports  Wawrinka, 40, makes Grand Slam history by reaching the
            #               third round in Australia —「first man 40 or older to reach
            #               the third round of a Grand Slam since the 1978 Australian Open」
            #   澳网官网    Marathon Stan snaps 48-year drought at AO 2026
            #               （2026−1978 ＝ 48，标题自己把年数写出来了）
            # 上一个是 1978 年 44 岁的罗斯维尔；R2 他 4-6 6-3 3-6 7-5 7-6(3) 胜热亚，
            # 4 小时 33 分，是 AO2026 最长的一场；R3 负于弗里茨。
            "australian-open",
            "一月墨尔本",
            "赛事总监上场，陪瓦林卡碰了杯啤酒",
        ...
        (
            # ⚠️ 全场比分 **6-7(9) 7-6(16) 7-6(7) 7-6(5)**，两个独立源核过
            # （2026-08-27 更正：原来写成 6-7(7) … 7-6(9)，**第一盘和第三盘的抢七
            #  数字对调了**——两处都是「7 和 9」，看着都像对的，只有并排比才看得出来）：
            #   Tennis Majors 正文  「6-7 (9), 7-6 (16), 7-6 (7), 7-6 (5)」，4 小时 20 分
            #   维基 2026 温网男单签表  瓦林卡那一行 7⁹ 6¹⁶ 6⁷ 6⁵
            #     （上标是**这一盘抢七里输的那个人**拿到的分：第二盘他 16 分，输 16-18 ✓；
            #      第三盘 7 分、第四盘 5 分；第一盘他赢，那个 9 是贝雷蒂尼的）
            # 六个盘点两个源都写着；旁白只说「四盘全抢七」「十八比十六」「六个盘点」，
            # 完整比分只出现在 tournament_story 的 hero 里，改的是那一处。
            "wimbledon",
            "六月伦敦",
            "四盘全是抢七，全场站着送瓦林卡",
            "六月的伦敦，他在一号球场碰上贝雷蒂尼，"
...
_CAPTIONS: dict[str, dict] = {
    ...
    "promotional-fees": {
        "hook": (
            "九月三十号那一周，辛纳在北京，阿尔卡拉斯在东京——"
            "同一天开打，同一天结束，都是 ATP500。\n"
            "翻开 ATP 二〇二六年的规则书，第一章一点一六条，标题是「推广费」。"
            "默认是一分都不许收；而整本书只给了一个例外："
            "ATP500 和 ATP250 这两档赛事，可以支付推广服务费。\n"
            "也就是说大满贯不许，大师赛不许——不是因为它们更干净，"
            "是因为那几档强制参赛，人本来就得来，不用买。"
            "而这两档不强制，这把工具是它们唯一能抢人的东西。\n"
            "规矩有牙：违规给钱，球员那头罚到六万美元并追缴，最多停赛三年；"
            "赛事那头要交出全部记录接受查账，不交罚十万、可以终止会籍。\n"
            "可整本书里有一个数字自始至终没出现——到底给了多少。那一栏从来不公布。\n"
            # ⚠️ 这一段是整条文案的安全阀，**不许删**：全片没有、也不能有任何
            # 一句说这两个人拿了钱。查下来没有一手报道把出场费和这两人这两站
            # 连起来；阿卡本人给的理由是想去个没打过的球场，而且他是去卫冕。
            "最后说清楚一句：这条片子讲的是规则本身。"
            "辛纳和阿尔卡拉斯为什么各去一站，他们自己给的理由是赛程和球场，"
            "没有任何公开记录说这里面有推广费。"
        ),
    ...
    "second-serve-clock": {
        "hook": (
            "谢尔顿在美网决赛后要求给二发设个时限——"
            "而规则书里，二发那一段本来就没有钟。\n"
            "发球计时器只对一发生效：25 秒算到「一发被击出」为止，"
            "一发失误之后的擦汗、走动、拍二十下球，"
            "都不落在任何一个计时窗口里。\n"
            "但没有钟不等于没人管：拖得算不算太久由主裁自由裁量，"
            "兹维列夫本届半决赛和决赛各吃过一次时间违例。\n"
            "罗迪克在自己的播客里说：二发也需要一块钟——"
            "你不能在一发和二发之间拍二十几下球，"
            "然后在凌晨三点收场的时候一脸震惊。\n"
            # ⚠️ 这两件事原来占了两行 225 字，把第 ⑦⑧ 屏的内容整段搬进了
            # 正文，正文因此涨到 1118 字——**超过小红书那一格的 1000 字上限，
            # 粘不进去**（账号所有者 2026-09-15：「可复制的正文超出 1000 字
            # 了，后续要改进」）。收成一行之后 973 字。
            # ⚠️ 三个统计数字**不在这儿重印**：第 ⑦ 屏的要点里本来就逐条印着
            # （20 秒 58.5%/21.5s、25 秒 26.3%/21.6s、澳网整分 40.2→41.0），
            # 同一组数在同一份正文里说两遍正是超标的直接原因。hook 的职责是
            # 钩子不是摘要——全库别的 hook 都是 2~4 行，这条原来 6 行是异常值。
            # 出处写在 `_SCRIPTS` 那一屏的注释里（两篇论文 ＋ Tennis
            # Abstract）；**正文里不挂「数据来源：」注脚**（CLAUDE.md
            # 2026-09-03 那条，来源写进注解不写进文案）。
            # ⚠️「试了三届」＝ 2023 首试 ＋ 2024、2025 沿用，出处同在第 ⑧ 屏。
            "不过钟并没有让比赛变快：20 秒换成 25 秒，"
            "松的是违规的定义，球员花的时间只差 0.1 秒"
            "——而给二发设限这条路，ATP 在新生代总决赛已经试了三届。"
        ),
        "tags": ("网球", "网球时差", "谢尔顿", "网球规则", "美网"),
    },
    "qualifier-ceiling": {
        "hook": (
            "郑钦文从美网资格赛打回正赛。那从资格赛出发的人，最远能走到哪儿？\n"
            "公开赛年代 58 年：女子出过 1 个冠军（拉杜卡努 2021 美网）"
            "和 1 个亚军（赫瓦林斯卡今年法网）；\n"
            "男子只有 5 个人进过半决赛，一个决赛都没有——美网那一格到今天还是空的。"
        ),
        "tags": ("网球", "网球时差", "郑钦文", "资格赛", "网球冷知识"),
    },
    "wawrinka-wildcard": {
        "hook": (
...
_OPENINGS: dict[str, dict] = {
    ...
    "ranking-math": {
        ...
        # 不是照片找不到」），所以封面改画这 19 个 / 18 个格子本身：标题问
        # 「为什么只算 19 个」，图里就是那 19 个格。
        ...
        # 3 选 1、落选的转进自选池），**是结构不是动作**：没有任何一段比赛画面
        # 拍得出「这 6 个位置钉死、那 7 个要抢」，而两张示意图一眼就说清了。
        # 和 `mandatory-1000` / `pr-allowance` 同族（规则原文 ＋ 数据对比）。
        "cards_why": "天然图表题材：讲的是两套积分公式的构成（19 项 / 18 项、"
        "7 选 6、3 选 1、落选转进自选池），是结构不是动作，比赛画面拍不出来；"
        "两张示意图承担主要信息，照片只负责让人看见当事人。",
    },
    "finals-venues": {
    ...
    "promotional-fees": {
        # ⚠️ 这一行**每一屏的台头上都挂着**，所以它得自己说得清。原来写的是
        # 「请得动球星的那条规则」——一个抽象标签，不看完不知道在说什么。
        # 仓库里同类的台头都是直接把那个反常的事实摆出来（`一发有钟，二发没有`、
        # `28 度也能叫极端高温`、`规则书里真有「盘外招」这个词`），照这个写。
        "topic": "只有两档赛事可以花钱请人",
        # ⚠️⚠️ **封面放他俩的脸，那这一问就不能带指控**——这两件事是一对，
        # 改一个必须动另一个。账号所有者 2026-09-16 要「封面图用两个人拼接在
        # 一起」，原来那一问「花钱请球星来，**犯规**吗？」就不能再用了：
        # 「犯规」是负面判断，两张脸压在它下面，合成语义是「这两个人收了钱」，
        # 而**没有任何一手证据支持那句话**（见 `_SCRIPTS` 那条注释）。
        #
        # 现在这一问是账号所有者自己给的方向，而它**恰好落在分界的另一边**：
        # CLAUDE.md 那条的判据是「这句话指向事情本身，还是指向画面上这个人」
        # ——「**请得来**吗」问的是这钱管不管用（答案：管，但只有两档赛事能用），
        # 不含任何负面判断，也没说这两个人收过。脸承载得起。
        # ⚠️ 9 个字排一行 129px，过 `_COVER_MIN_ONE_LINE_PX = 84`。
        "question": "花钱请得来球星吗？",
        "narration": "花钱请得来球星吗？请得来。"
                     "但整本规则书里，只有两档赛事可以这么做——"
                     "大满贯和大师赛，一分都不许给。",
    ...
    "second-serve-clock": {
        ...
        # 「绝大部分人不知道 12040 是啥啊」退回来——一个裸的大数没有语境，
        # 读者第一眼不知道它在说什么，而封面唯一的职责就是让人一眼知道内容。
        # 换成的这个数是**人人秒懂而且不需要任何限定**的：ESPN 决赛稿正文
        # 「Zverev bounces the ball many times -- sometimes as many as 20」，
        # 谢尔顿原话也是「20 to 25 times」「never does less than 10 bounces」。
        # ⚠️ 中文数字那版「二发前拍二十下球」排一行只有 82px，**差 84 这道线
        # 两个像素**，渲出来断成「二发前拍二十下 / 球，算违规吗？」——把
        # 「二十下球」劈开了。换成半角「20」是 88px，一行排得下，而且它和
        # 图上「25 秒 · 有钟」那一格正好形成对照。配音仍念中文（见 narration）。
        "question": "二发前拍20下球，算违规吗？",
        "narration": "二发前拍二十下球，算违规吗？不算。"
                     "因为发球的钟只管一发——"
                     "二发那一段，规则里一个数都没有。",
        "image": "assets/explainer/second-serve-clock/zverev-serve.jpg",
        "credit": "usopen.org 官方图 · Simon Bruty/USTA · "
                  "2026 美网男单决赛，兹维列夫发球",
        "tags": ["网球", "网球时差", "网球规则", "美网", "网球冷知识"],
    },
    "wawrinka-wildcard": {
        "topic": "美网外卡的这出戏",
    ...
    "kostyuk-champion-test": {
        "topic": "科斯秋克：冠军试金石",
        "question": "击败科斯秋克的人，都夺冠？",
        "narration": (
            "击败科斯秋克的人，都夺冠？高芙刚刚在辛辛那提捧杯，"
            "也让 WTA 今年一条离谱规律第四次应验："
            "科斯秋克最近四次输球，赢她的人最后全成了冠军。"
        ),
        "image": "assets/reel/kostyuk-cincinnati-2026-r16.jpg",
        "credit": "WTA 官方赛后稿 / Getty Images · 2026 辛辛那提，科斯秋克",
        "tags": ("网球", "网球时差", "科斯秋克", "高芙", "WTA"),
    },
    "equal-pay": {
    ...
    "qualifier-ceiling": {
        "topic": "资格赛最远打到哪儿",
        # ⚠️ 封面这一问指向**天花板在哪儿**（女子冠军、男子半决赛），末屏那一问指向
        # **谁会先越过男子那条线**——两件事。回声那道判据按字集重合度算。
        "question": "资格赛最远能打到哪儿？",
        "narration": "资格赛最远能打到哪儿？"
        "女子那边有人拿过冠军，男子那边五十八年只到过半决赛。",
        # ⚠️ **封面故意不用球员实拍。** 这条片子的答案是两个数摆在一起，
        # 而任何一张球员照片都会把它缩回到某一个人身上——读者读到的会变成
        # 「这个人最远走到哪儿」，那是另一个问题。和 `equal-pay` 同一个理由。
        "diagram": _QUALIFIER_COVER_DIAGRAM,
        "credit": "示意图 · 网球时差绘制",
        # ⚠️ 标签必须专属：默认那五个和别的冷知识片一字不差，
        # `test_文案的开场和标签属于它自己的选题` 会当场红。
        "tags": ("网球", "网球时差", "郑钦文", "资格赛", "网球冷知识"),
...
def speakable(text: str) -> str:
    ...
    **两边字数一样**，所以按字位算出来的字幕时间轴对两份都成立——这是
    ...
    ### 年份里的 〇 换成 零——`〇` 会挡住合成器的「这是个年份」识别

    账号所有者 2026-08-27：「tts 里 2002 年读音错误」。

    旁白按仓库惯例写「二〇〇二年」（`〇` 是 U+3007 IDEOGRAPHIC NUMBER ZERO）。
    量出来问题不在写法对不对，在**合成器认不认得它是个年份**——同一把嗓子、
    同一语速，只换这一个字，切词完全变了（edge-tts 的 WordBoundary，实测）：

        二〇〇二年   二｜〇｜〇｜二年      ← **「二年」被黏成一个词**   二零零二年   整体
        二〇〇五年   二｜〇｜〇｜五｜年                              二零零五年   整体
        二〇一七年   二｜〇｜一七｜年                               二零一七｜年
        二〇二六年   二｜〇｜二六｜年                               二零二六年   整体
        二〇二四     二｜〇｜二｜四       ← 四个孤立字                二零二四     整体
        一九九九年   一九九九年（本来就整体）——**它没有 〇**

    末行是反证：唯一不带 `〇` 的那个年份，本来就被整体识别。也就是说
    **`〇` 一律把年份打散成孤立字**，而孤立的字只能逐个猜读音。

    ⚠️ **2002 是唯一一个把「二年」黏成一个词的**——那正是「两年」那个量词短语的
    形状（其余年份的末位数字和「年」是分开的）。账号所有者点的正是这一年。

    换 `零` 不动屏幕、**字数 1:1**（上面那条「两边字数一样」的前提照旧成立），
    时长只短 0~0.03 秒（往宽里差，不会把哪一段挤成超长）。非年份的用法也验过
    没有回归：`六比〇`→`六比零` 字节数一模一样。

    ⚠️ **生产走的是 Azure，而这台沙箱只够得着 edge-tts**（CLAUDE.md 记着这两者
...
# 「点」后面跟着的东西：这几个字一出现，前面那个「N点M」就是**钟点或成语**，
# 不是小数。逐条都是从语料里挖出来的，不是想出来的：
#   分 —— 「晚上九点零六分」（special-exempt）「下午两点零五分」（longest-match）
#   刻 —— 「下午两点一刻开打」（heat-rule）
#   点 —— 「然后一点一点」（三条采访的中文行）——不挡的话它变成「1.1点」
#   半/钟 —— 「八点半」「三点钟方向」，这两个今天在语料里够不着，防的是以后
# ⚠️ 代价认领过：`weeks-at-no1` 的「零点八分」是**真小数**（0.8 个积分），
# 被「分」这一刀一起挡掉了，屏幕上仍是中文。分辨它和「九点零六分」要靠上下文
# （前面有没有「晚上／凌晨」），而那是个会误判的判据——**把钟点渲成小数是
# 屏幕上的错字，把小数留成中文只是维持现状**，两边不对等，所以取窄的那一刀。
# 那条片子已发不重渲，这一刀对它没有回归。
_DECIMAL_STOP = "分半刻钟点"


def _decimal_value(run: str) -> str | None:
    """「二十一点六」→ 21.6。读不出来返回 None。

    小数部分在中文里是**逐字读**的（二十一点六 / 五十八点五 / 零点一），所以它
    只能由单个数字字符组成。**一旦出现十/百/千，那就不是小数**——「两点十五」
    「九点五十」「零点三十五分」全是钟点，语料里 42 处这个形状一个小数都没有。

    ⚠️ 下面那句 `tail` 只认 `_DIGIT` 的判断，**在通用那条路上是冗余的**——
    那边的正则已经把小数部分限死成 `[_DIGIT]+`，拆掉它测试照样绿（验过）。
    它真正管着的是**百分比那条路**：那儿的字符类是 `[_NUM_CHARS点]+`，
    「百分之两点十五」这种能走到这儿。今天语料里没有这种输入，所以这一条
    **反向验证不出来**——按 CLAUDE.md 那条，验不出来的守卫不许写成「验过了」。
    """
    if run.count("点") != 1:
        return None
    head, tail = run.split("点")
    whole = _num_value(head)
    if whole is None or not tail:
        return None
    if any(ch not in _DIGIT for ch in tail):
        return None
    return f"{whole}.{''.join(_DIGIT[c] for c in tail)}"
...
def arabic_numerals(text: str) -> str:
    ...
    def percent(m: re.Match) -> str:
        ...
        if run == "百":
            ...
            # ⚠️ 先试小数：「百分之五十八点五」的 run 是「五十八点五」。不试的话
            # `_num_value` 在「点」上返回 None，而**上面那条正则的字符类里没有
            # 「点」时它压根匹配不到小数部分**——屏幕上出来的是「58%点五」，
            # 一个没人读得出来的东西（这条片子第一版就是这么渲的）。
            value = _decimal_value(run) or _num_value(run)
    ...
    # 而判据宁可窄不可宽。
    #
    # ⚠️ 「百分点」没有「之」，进不了这一条，走的是下面 `one()` 那一轮——
    # 而「百」命中那里的 `_STRUCTURED`，换出来是「100分点」。
    #
    # ⚠️⚠️ 这里原来写着「量过：全仓库三处全在注解和小红书正文里，一处都没进
    # `narration`」，据此判定它是个**没被触发过的坑**。**那个量过期了**：
    # `tiafoe-nakashima-cincinnati-2026-sf` 的第 10 段旁白就写着「比对手高出
    # 十五个百分点」，那条片子的字幕**已经带着「15个100分点」发出去了**
    # （已发不重渲）；`chwalinska-townsend-us-open-2026-r1` 渲完抽帧又撞了一次，
    # 第 50 秒印着「22个100分点」。带数字的注释会过期，而它过期的时候不吭声。
    #
    # 现在「百分点」在 `one()` 里单独挡掉了，判据
    # `test_百分点是一个词不是数` 钉着它。
    #
    ...
    # 轮次：四分之一决赛 → 1/4决赛、八分之一决赛 → 1/8决赛。
    #
    # 旁白给合成器写「四分之一决赛」，屏幕上要的是轮次的正式写法「1/4决赛」
    # （CLAUDE.md「轮次写 1/8决赛 / 1/4决赛」＋「给人看的字一律阿拉伯数字」）。
    # 不单独接的话，下面几轮对裸的「四」「一」都不动，屏幕上原样印出「四分之一决赛」
    # ——2026-09-26 `bu-majchrzak-hangzhou-2026-r2` 渲完抽帧（第 120 秒）才看见。
    # ⚠️ 只认后面紧跟「决赛」的这几个分母，别的「X分之一」一律不碰，判据宁可窄。
    text = re.sub(
        r"(二|四|八|十六|三十二|六十四)分之一决赛",
        lambda m: f"1/{ {'二': 2, '四': 4, '八': 8, '十六': 16, '三十二': 32, '六十四': 64}[m.group(1)] }决赛",
        text,
    )
    ...
    #
    # **排在这儿**：要在比分和 `one()` 之前——`one()` 把「点」当量词
    # （「凌晨三点」→「3点」是对的），轮到它的时候「二十一点六秒」已经塌成
    # 「21点六秒」了，再也拼不回来。
    #
    # ⚠️ 后顾断言里 `_NUM_CHARS` 那一半**不是冗余，是防回溯的**。第一版只写了
    # `(?![_DECIMAL_STOP])`，而 `[DIGIT]+` 会**退一格再试**：「九点零六分」先拿
    # 「零六」撞上「分」被拒，退成「零」，后面是「六」不在禁表里——于是当场匹配，
    # 渲出来是「9.0六分」。「凌晨两点五十五」同样塌成「2.515」、「十一点四十」
    # 塌成「11.410」。**四个钟点全在语料里，而它们一个字都不报错。**
    # 把数字字符也列进禁表，退一格那条路就堵死了：小数后面只能跟非数字。
    #
    # ⚠️ 两道闸各挡一类，缺一类就漏一类（两个方向都反向验证过，数是量出来的）：
    #   拆掉 `_DECIMAL_STOP` → 「一点一点」变成「1.1点」、「两点一刻」变成
    #     「2.1刻」、「九点零六分」变成「9.06分」
    #   拆掉上面那半个防回溯的 → 「凌晨两点五十五」变成「2.515」、
    #     「十一点四十」变成「11.410」、「九点零六分」变成「9.0六分」
    text = re.sub(
        rf"([{''.join(_NUM_CHARS)}]+点[{''.join(_DIGIT)}]+)"
        rf"(?![{_DECIMAL_STOP}{''.join(_NUM_CHARS)}])",
        lambda m: _decimal_value(m.group(1)) or m.group(0),
        text,
    )
    # 比分照数字写：六比四 → 6比4。
    ...
    def one(m: re.Match) -> str:
        ...
        if run == "百" and nxt == "分" and text[m.end():m.end() + 1] == "点":
            # 「百分点」是一个词，不是数。不挡的话「百」命中下面那条
            # `_STRUCTURED`（含十/百/千）被换成 100，屏幕上是「22个100分点」。
            # 卡死在「百分点」三个字上，和上面「一号种」那条同一个形状——
            # 裸的「百」在别处照旧换（「一百三十九公里」不受影响）。
            return m.group(0)
...
def _best_break(text: str) -> int:
    ...
    ⚠️⚠️ **`+ i` 别换成「越均分越好」——2026-08-31 试过，当场把词劈开。**
    看着很有道理：`+ i` 是贪心塞满，同档里永远挑最靠右的断点，于是尾巴被挤成
    两三个字（「本西奇熬了3小时24分钟才拿下 ｜ 汤森德」）。换成一个 [0,90] 的
    均分项之后，全库孤屏从 69 再降到 59（多消掉 10 处），代价是**四条把词劈开**（全库量的，
    不是挑的）：
    ...
    根子是**这几个分数是一个调好的平衡，均分项的量级把它打破了**：孤行罚分
    −95 会把 `bonus=1` 的候选从 100 拉到 5，此时它和 `bonus=0` 的候选只差
    5 分——而 `+ i` 最多 20（宽度上限 16 格，中文一字一格）、恰好跨不过一个
    边界档；90 分的均分项跨得过去。

    **孤行好过把词劈开**（docstring 开头那条，是反过来做过一版才定下来的），
    所以那笔账是亏的：10 处孤屏换 4 处劈词。⚠️ 那次改动**只拿四个例子对比过
    就合并了**，四条劈词一条都不在那四个里——真正抓到它的是事后全库跑一遍
    before/after。所以要动这个分数，先跑这两条：
    `test_断点不许让非词边界赢过词边界`（钉这条平衡本身）和
    `test_断行的硬切不许悄悄挪位置`（把那次手工全库 diff 变成机器做的）。
    """
