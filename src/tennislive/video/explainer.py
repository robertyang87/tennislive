...
_CAPTIONS: dict[str, dict] = {
    "second-serve-clock": {
        "hook": (
            ...
            "你不能在一发和二发之间拍二十几下球，"
            "然后在凌晨三点收场的时候一脸震惊。\n"
            # ⚠️ 这三个数和视频第 ⑦ 屏同源，出处写在 `_SCRIPTS` 那一屏的注释里
            # （两篇论文 ＋ Tennis Abstract）。**正文里不挂「数据来源：」注脚**
            # ——CLAUDE.md 2026-09-03 那条，来源写进注解不写进文案。
            "不过钟本身并没有让比赛变快：20 秒限制的年代，"
            "澳网男单超时率 58.5%、分间平均 21.5 秒，而主裁全程只判罚过 2 次；"
            "换上 25 秒的钟之后，美网男单超时率降到 26.3%，分间平均还是 21.6 秒"
            "——松的是违规的定义，不是球员花的时间。\n"
            "这条路 ATP 自己走过一次——新生代总决赛 2023 年起试行"
            "「一发到二发最多 8 秒」，2024、2025 两届沿用；"
            "而今天这块 25 秒的钟，本身就是 2017 年从同一块试验田出来的。"
        ),
