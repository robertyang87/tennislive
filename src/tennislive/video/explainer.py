...
_SCRIPTS: dict[str, tuple[tuple, ...]] = {
    ...
    "heat-rule": (
        ...
        (
            ...
            "", "示意图 · 网球时差绘制",
            ("当地 15 点 气温 28.6 度", "湿度 76% 风速 1.3", "要不要停赛 不看第一个数"),
            that_afternoon(),
        ),
        (
...
_OPENINGS: dict[str, dict] = {
    ...
    "heat-rule": {
        ...
        "新规则看的那个数里，气温只占一成。",
        # ⚠️ **封面用配方那张图，不用德约的脸。** 不是因为这条片子有指控——它
        # 没有（标题讲的是天气和规则，不含对任何人的负面判断），所以他的实拍
        # 放在第 ① 屏是合规的。换成图纯粹是因为**配方那张图直接回答了封面那
        # 一问**，而一张脸只是把人认出来。（顺带：那张图 2000×1333，铺满
        # 1080×1440 要放大 1.08 倍，本来也过不了 `test_封面图不许被放大`
        # 的 1.00x 地板。**但选它的理由是前一条，分辨率只是碰巧站在同一边。**）
        "diagram": wbgt_recipe(),
        "credit": "示意图 · 网球时差绘制",
