...
COLUMNS: dict[str, Column] = {
    ...
    # 讲一个人的两个栏目，按**第一次还是又一次**分。
    #
    # 「First Serve」单看有点平——英文里它就是「一发」而已。它的意思是靠挨着
    # 「Second Serve」才立起来的，两个名字合起来把一段生涯的两端占住了，正如
    # 「开赛之前」和「握手之后」占住一场比赛的两端。这是明知道的取舍，不是疏忽。
    "First Serve": Column(
        name="First Serve",
        promise="一个人第一次让所有人记住他——是第一次，不是又一次；"
                "片子讲的是他从哪儿来、这一下有多难。",
        # 和 Second Serve 一样：不会过期，但仍然该写清引子是哪一天。
        perishable=False,
    ),
    # 四个栏目按「讲的是谁／什么」分工：现象归「网球有故事」，一场比赛归「开赛之前」
    # 和「握手之后」，一个人归这里。
    #
    # 名字是网球术语：一发失了，第二发必须进——**第二次机会，而且没有退路**。
    # 用英文是因为它在英文里就是这个意思，不用解释；卡上印成「网球时差 · Second
    # Serve」，标题行的显示字体（得意黑）和封面绿标签的字距本来就是按西文设计的，
    # 渲出来比中文更贴。试过「人物志」，太普通——每个号都有一个，而且它只是个分类
    # 标签：现有三个名字里，「开赛之前」「握手之后」有画面，「网球有故事」有承诺，
    # 「人物志」两样都没有。
    #
    # promise 里三种「掉下去」的写法（跌、停、被忘）是故意的：只写「低谷」会把
    # 伤停和被时间盖过去的人排除在外，而这个栏目要装得下几十个人。
    "Second Serve": Column(
        name="Second Serve",
        promise="一个人的第二次机会——跌下去、停下来、或者被忘掉之后，他又赢了一次；"
                "片子讲的是中间那几年。",
        # 不易逝：那几年明年也还是那几年。但**仍然该写清引子是哪一天**，理由和
        # 「握手之后」一样——片子会在几天后被翻出来转，读者要知道说的是哪一次。
        perishable=False,
    ),
    "握手之后": Column(
        name="握手之后",
