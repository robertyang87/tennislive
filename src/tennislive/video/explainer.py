...
# 这一屏回答的是「128 个人里只有 16 个能出线，这意味着什么」——一张照片
# 讲不出「单败淘汰、连赢三场、晋级率八分之一」这类比例关系，示意图是唯一
# 诚实的画法（CLAUDE.md「示意图的触发条件是照片讲不清，不是照片找不到」）。
# 条长＝这一轮开打前的人数，只有最后一格（16 人晋级正赛）用品牌绿标出——
# 一屏一个强调色；数字写在条外，不写在条上。
#
# ⚠️ 这个高度是**量出来的，不是按 CLAUDE.md 记的老参考线（640/608）推的**。
# 两版都撞过：第一版 viewBox 620、内容到 y=598；砍掉冗余文字后收到 560、
# 内容到 y=482，本地渲出来**仍然**压进卡片底部 `.copy` 徽章里。
# 拿 Playwright 量 `.diagram-wrap svg` 和 `.chip`（序号徽章）的真实
# `getBoundingClientRect()` 才看清楚问题不在"内容留白多少"，
# 在于**整个 viewBox 的高度都会被渲成一个等比缩放的实体框**，
# 哪怕底部是空的：`svg{width:920px;height:auto}` 按 900:viewBox高 的比例
# 把整个 viewBox 拉成一个矩形占位，不是只占到"最后一笔画在哪儿"。
# 量到的数：`.diagram-wrap` 顶部 210px，这一屏 `.copy`（含徽章）顶部在
# 660px——留给 svg 的框只有 450px，换算回 viewBox 单位是 450÷(920/900)≈440。
# 现在 viewBox 390，渲成 399px 高，210+399=609，离 660 还留 51px——这条边界
# 只对**这一屏**成立（`.copy` 的高度取决于这一屏自己的标题行数和要点条数），
# 换一屏文案要重新量，不能照抄这个 390。
_ZHENG_YOU_DRAW_DIAGRAM = """
<svg viewBox="0 0 900 390" xmlns="http://www.w3.org/2000/svg">
  <text x="450" y="34" text-anchor="middle" fill="#f4fbf7"
        font-size="28" font-weight="800">128 人，16 个正赛名额</text>
  <text x="450" y="60" text-anchor="middle" fill="#cfe6d8"
        font-size="16" font-weight="700">条长＝这一轮开打前还剩多少人，单败淘汰</text>

  <g font-size="20" font-weight="800" fill="#f4fbf7" text-anchor="end">
    <text x="210" y="115">第一轮</text>
    <text x="210" y="191">第二轮</text>
    <text x="210" y="267">第三轮</text>
    <text x="210" y="343">正赛名额</text>
  </g>

  <rect x="230" y="90" width="580" height="36" rx="6" fill="#8fd6a8"/>
  <text x="824" y="115" fill="#f4fbf7" font-size="24" font-weight="800">128</text>

  <rect x="230" y="166" width="290" height="36" rx="6" fill="#8fd6a8"/>
  <text x="534" y="191" fill="#f4fbf7" font-size="24" font-weight="800">64</text>

  <rect x="230" y="242" width="145" height="36" rx="6" fill="#8fd6a8"/>
  <text x="389" y="267" fill="#f4fbf7" font-size="24" font-weight="800">32</text>

  <rect x="230" y="318" width="73" height="36" rx="6" fill="#c6f65a"/>
  <text x="317" y="343" fill="#f4fbf7" font-size="24" font-weight="800">16</text>
</svg>
"""
...
_SCRIPTS: dict[str, tuple[tuple, ...]] = {
    ...
    # 郑钦文 VS 尤晓迪，2026 美网女单资格赛第一轮（Q1）。
    #
    # 事实两个源核过：SCMP＋维基百科「2026 US Open – Women's singles
    # qualifying」互相印证「这是她 2022 年澳网之后第一次打大满贯资格赛」——
    # 不是「生涯第一次」（她 2022 年澳网就连赢三场首次踏进大满贯正赛），是
    # 「隔了四年再打」，两个说法混起来就是一句假话（CLAUDE.md「判历史事件
    # 用当年那本规则书」那条的同族坑）。排名（127 / 198）用 WTA 官方
    # `players/ranked?playerId=` 接口核过，和 app 截图的即时数字逐位对上。
    #
    # 封面不走 versus_poster.py 合成——尤晓迪这个级别没有官方半身抠图
    # （Torso cutout），硬凑会撞上「没有官方抠图就退回照片版，别拿头像凑」
    # 那条：一个全身、一个只有头，摆一起是「浮头站在半身像旁边」。封面直接
    # 复用郑钦文本人的实拍（和 wang-sabalenka 同一个做法），credit 自动借用
    # 第①屏。
    "zheng-you": (
        (
            "peak",
            "巅峰",
            "两年前，她是奥运冠军",
            "两年前，她是奥运冠军。二〇二四年八月三号，巴黎奥运会女单决赛，"
            "郑钦文六比二、六比三击败维基奇，拿下中国网球史上第一枚奥运会"
            "网球单打金牌。去年六月十六号，她升到生涯最高的世界第四。"
            "一个月后，右肘的游离碎骨让她做了手术——"
            "二〇二五年的下半个赛季，她一场比赛都没有打完。",
            "assets/explainer/zheng-you/zheng_athens_r1_2026.jpg",
            "WTA 官方图库 · 2026 年 7 月 13 日雅典 WTA250 女单第一轮，郑钦文",
            (
                "2024.8.3 奥运会女单夺冠",
                "2025.6.16 生涯最高第 4",
                "一个月后，右肘手术",
        ...
        (
            ...
            "半年",
            "教练换了几轮，排名跌到一百六",
            "手术之后，她二〇二六年二月才重新回到巡回赛，那之后排名一路往下——"
            "六月的草地赛季，跌到过第一百六十。教练也跟着换：里巴出车祸伤了"
            "颈椎，缺战期间她先后请来巴格达蒂斯、又在七月加了纳瓦罗。"
            "七月中旬的雅典站，她先后淘汰博萨斯·马内罗和米契奇，打进十三个月"
            "来第一个八强——那一场她第一盘一度三比零领先，随后连丢五局，"
            "最终四比六、四比六告负。三周后，多伦多资格赛第一轮，她全场一个"
            "破发点都没拿到，三比六、四比六再次首轮出局。美网直通正赛的门槛，"
            "她也没有够到。",
            "assets/explainer/zheng-you/zheng_athens_r2_2026.jpg",
            "WTA 官方图库 · 2026 年 7 月 15 日雅典 WTA250 女单第二轮，郑钦文正手",
            (
                "本赛季至今 6 次首轮出局",
                "7 月雅典打进 13 个月来首个八强",
                "8.2 多伦多资格赛首轮出局",
        ...
        (
            ...
            "打过一次",
            "2018 年，她赢过郑钦文",
            "两个人不是第一次交手。二〇一八年，南京——尤晓迪的家乡——"
            "一站 ITF 赛事的八强战，尤晓迪六比四、六比一击败了郑钦文，"
            "那时候郑钦文还只是个十几岁的青少年球员。八年过去，"
            "尤晓迪，中国南京人，今年三十岁，生涯最高排名是二〇二〇年三月的"
            "世界第一百六十六，本周世界排名第一百九十八——这些年，她常年在"
            "大满贯资格赛里打拼，澳网、法网、温网、美网，都留过她的名字。",
            "assets/explainer/zheng-you/you_xiaodi_headshot.png",
            "WTA 官方头像 · 尤晓迪",
            (
                "2018 南京 ITF 八强，尤晓迪 6-4 6-1 胜出",
                "生涯最高第 166（2020.3）",
                "澳网/法网/温网/美网资格赛都打过",
            ),
        ),
        (
            ...
            "这一场",
            "赢的人，往前走一步",
            "北京时间八月二十四号晚上十一点，郑钦文对尤晓迪，两个人在纽约"
            "打响资格赛第一轮。美网资格赛一百二十八个签位，单败淘汰，"
            "三盘两胜制——只有十六个人能笑着走出去，晋级率八分之一。"
            "这一签，郑钦文是十四号种子。上一次她打大满贯资格赛，是二〇二二年"
            "的澳网，那一次她连赢三场，第一次踏进大满贯正赛。",
            "",
            "网球时差绘制",
            (
                "8 月 24 日 资格赛第一轮开打",
            ...
            _ZHENG_YOU_DRAW_DIAGRAM,
            "四年前，她靠三连胜走进了大满贯正赛。这一次，你觉得她还需要几场？",
        ),
    ),
...
_CAPTIONS: dict[str, dict] = {
    ...
    "zheng-you": {
        "hook": (
            "2024 年奥运会女单金牌，2025 年 6 月生涯最高世界第 4——\n"
            "一个月后右肘手术，2026 年美网，她要从资格赛第一轮打起。"
        ),
        "tags": ("网球", "网球时差", "郑钦文", "尤晓迪", "美网"),
    },
    "eala-anisimova": {
        "hook": (
...
_OPENINGS: dict[str, dict] = {
    ...
    "zheng-you": {
        "column": "开球之前",
        "topic": "郑钦文 VS 尤晓迪：美网资格赛第一轮",
        # 这一问指向**过程/身份反差**（从大满贯正赛常客到资格赛第一轮），
        # 不是排名——排名已经印在下面的对阵行旁边（渲染时挂国旗+即时排名）。
        # 「世界第 4」是核过的真数字，落点是「为何打资格赛」这个反差，
        # 不是身份罗列（CLAUDE.md「钩子要有剧情的跌宕」那条）。
        # ⚠️ 原来写「世界第四，怎么打起了资格赛？」，`test_封面那一问要能排进
        # 一行` 量出来 82px，差 84px 那道线 2 像素——这两个像素只有渲出来才
        # 看得见。砍掉「起了」两字换成「为何」，96px，一行排得下。
        "question": "世界第四，为何打资格赛？",
        "narration": "世界第四，为何打资格赛？二〇二六年八月二十四号，"
                     "郑钦文站上资格赛第一轮的赛场——距离她上一次打大满贯"
                     "资格赛，已经过去四年。",
        "fixture": {
            "date": "8.24",
            # ⚠️ 单源、带星号。来源是账号所有者截的 app 实时赛程卡（TNNS），
            # 给的是这一场具体的开球时刻（北京 23:00／纽约当地 11:00），
            # 不是那一节的场次开场时间；但拿不到第二个独立源交叉核对
            # （usopen.org 的官方资格赛 Order of Play 在写这条时还没查到），
            # 按「一个时刻有三种口径，别压成一种」，归 single-source 那一档。
            "time": "23:00*",
            "level": "GS",
            "site": "纽约",
            "round": "资格赛第一轮",
        ...
        # 封面复用第①屏郑钦文本人的实拍（雅典 WTA250 首轮握拳庆祝），
        # `_opening_segment` 自动借用那一屏的 credit。尤晓迪这个级别没有
        # 官方半身抠图，versus_poster.py 的 cutout 版式在这条上走不通——
        # 见 credits.json 的 `why`。
        "image": "assets/explainer/zheng-you/zheng_athens_r1_2026.jpg",
    },
    "eala-anisimova": {
        "column": "开球之前",
