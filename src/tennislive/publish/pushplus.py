...
def check_content_length(html_content: str) -> None:
    ...
    ⚠️ **口径按字符（`len`），不按 UTF-8 字节——这条是量出来的，2026-09-14 定案。**
    官方文档只有一张错误码表，没写清「2万字」数的是什么；而同一条片子两趟实测
    把它夹死了：

        被拒 run 34843211265    21404 字符 ❌ / 24032 字节 ❌   code 999
        **收下 run 34849158901  19154 字符 ✅ / 21782 字节 ❌**  流水号 e3804b2d40a0…

    第二行那一趟**字节越线 1782 而照样发出去了**，所以口径是字符。⚠️ 日志里仍然
    两个数都打——哪天服务端改口径，那一行就是现成的反证，不用再拿一条推送去试。
    """
    ...
    raise PushPlusError(
        ...
        "实测 **25 张图 ＝ 19154 字符**（钉 10 位 sha 之后），离上限只剩 846；"
        "要发得先减图或缩短 URL。"
    )
