...
def check_content_length(html_content: str) -> None:
    ...
    ⚠️ **口径按字符（`len`），不按 UTF-8 字节——这不再是"照它措辞的读法"，
    是量出来的。** 同一条片子两趟把它夹死了：

    | run | 字符 | 字节 | 结果 |
    |---|---|---|---|
    | 34843211265 | 21404 ❌ | 24032 ❌ | `code 999` 拒收 |
    | **34849158901** | **19154 ✅** | **21782 ❌** | **发出去了** |

    第二行那一趟**字节越线 1782 而照样收下**，所以数的是字符。在这之前只有
    第一行一个样本、两个数都越线，**分不出口径**——闸当时按字符写、日志两个数
    都打，正是为了让下一次给出答案。现在给出来了。

    ⚠️ 日志**仍然两个数都打**：哪天服务端改口径，那一行就是现成的反证。

    ⚠️ 顺带一条容易拿错的：CLAUDE.md 里「字节 ≈ 5767 + 图数 × 700」那条经验
    公式量的是**字节**，而这道闸数的是**字符**（中文一个字三字节），
    **只能用来排序，不能拿去对上限**。按字符的实测点是 **25 张图 ＝ 19154 字符**，
    离 20000 只剩 846。

    （2026-09-15 从 #896 移植：那条 PR 基于历史重写之前的 main，不能 merge。）
    """
