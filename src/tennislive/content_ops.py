...
# **排期一出来就能定，不用等到快开赛。** 账号所有者 2026-08-05：
# 「赛前焦点补位 最好是在日程排期出来后就决定，不要等到快开赛了」。
#
# 原来的上界是 **210 分钟**——也就是「开赛前三个半小时之内才进候选」。
# 那个数是**给一天 72 趟的定时排的**：窗口只要比轮询间隔宽，就不会漏掉
# 任何一场。定时 2026-08-02 归零之后，它的作用整个反过来了——手动跑一趟，
# 只能看见未来三个半小时里开赛的那几场，**明天的签表全在视野之外**。
#
# 现在的判据是「**排期出没出来**」，而数据里早就写着了：`start_utc is not None`
# 就是官方给了时间。CLAUDE.md 那条「时间从无到有会改变归属日」说的是同一件事。
# 所以上界只需要宽到不挡路——`cmd_content` 抓的是昨天/今天/明天三天，
# 48 小时正好盖住这个取数半径，**真正的边界回到数据本身**，而不是一个拍出来的数。
#
# ⚠️ **下界 45 分钟不能去掉，它是另一件事**：不是「等到快开赛」，是
# 「还来得及把包做完发出去」（渲 4~5 张卡要几分钟）。开赛前 10 分钟才选中，
# 做完人也来不及发。
#
# ⚠️ 副作用是**比较的范围变大了**：原来是「未来三个半小时里最好的一场」，
# 现在是「未来两天里最好的一场」。这正是「排期出来就决定」的意思——
# 但也意味着更早定下的那条，可能在开赛前退赛。包是给人过目的草稿，
# 人工那一关照旧。
...
def preview_candidates(
    ...
    *,
    ...
    「进入窗口」现在的意思是**排期已经出来、而且还来得及做完发出去**，
    不再是「马上要开赛了」。
    """
