...
original_result=json.loads(raw)
aliases={'Sabalenka':'萨巴伦卡','Aryna Sabalenka':'萨巴伦卡','阿丽娜·萨巴伦卡':'萨巴伦卡'}
for obj,key in ((result.get('interviewee',{}),'value'),(result.get('cover',{}),'subject')):
    if obj.get(key) in aliases: obj[key]=aliases[obj[key]]
if result.get('interviewee',{}).get('speaking_or_receiving_trophy')=='speaking':
    result['interviewee']['speaking_or_receiving_trophy']=True
(out/'minimax_visual_review_raw.json').write_text(json.dumps(original_result,ensure_ascii=False,indent=2)+'\n')
