...
prompt = '\n'.join(mapping) + '''
你是赛后开麦的视觉审核员。给定事实用于比对，不能冒充视觉证据：美网八强赛，萨巴伦卡对诺斯科娃；受访者萨巴伦卡。
只用图片4–6判定采访正文类型；图片1–3对比4–6判断同场；只用图片7判断封面。
返回JSON：content_type:{value:on_court|ceremony|press|studio|unknown,reason,confidence},interviewee:{value,visible,speaking_or_receiving_trophy,reason,confidence},same_match_lead_in:{value:boolean,reason,confidence},mirrored:{value:boolean,reason,confidence},bilingual_subtitles:{value:boolean,reason,confidence},cover:{subject,same_program,frontal,eyes_open,clear,reason,confidence}。
