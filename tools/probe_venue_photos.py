...
# ⚠️ 渲染**新闻站文章页**时，DOM 里最大的那几张多半**不是这篇文章的配图**，
# 而是侧栏 / 页尾推荐位（"猜你还想看"）里的图。这类模块的图往往比正文配图还大，
# 按尺寸倒序排一路排在最前面。
#
# 判据是"对不对题"，不是"大不大"：2026-07 找阿卡普尔科时，
# elfinanciero.com.mx 那篇决赛直播文渲出来 15 张 ≥1000px，排前面的依次是
# 客厅里的电视机、皇马对曼城、一枚勋章、一个警戒线现场——只有第 2、3 张
# 是本站的球员特写，一张场馆全景都没有。
#
# 所以新闻站这条路要么按**文章正文容器**取图（`article img` 之类），
# 要么干脆放弃列表、直接找带图注的现场报道（见 CLAUDE.md「最好的出处，
# 是图注自己把时刻写死了」）。
