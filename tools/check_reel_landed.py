...
def seg_film_seconds(s: dict) -> float:
    ...
    """
    if s.get("image") or s.get("stat_card") or s.get("title_card"):
        # stat_card / title_card 段是 render 现渲的整屏段，形状同 image（seconds 定长）
...
def evidence_windows(spec: dict, cover: float) -> list[tuple[float, float]]:
    ...
    for seg in spec["segments"]:
        length = seg_film_seconds(seg)
        if seg.get("image") or seg.get("stat_card") or seg.get("title_card"):
