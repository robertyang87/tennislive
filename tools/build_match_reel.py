...
**那一轮里成片那一步（当时是 preset slow / crf 18）一个字没动**——省时间先从
中间产物和重复解码上省，别一上来就动最后交出去的那一份。

⚠️ 后来这句话只剩一半成立，别照着它推：**`crf 18` 至今一个字没动**，而
**preset 2026-08-14 按实测从 `slow` 换成了 `medium`**（那一步是整趟 render 里
最大的一块：423.7s / 862s；量下来快 28~38%、大 1.6~6.1%、PSNR 差 0.04~0.18 dB）。
两张对照表和为什么当年只能是 `slow`，都写在 `FINAL_PRESET` 上面。
...
# 给坐标）**按原比例**贴回左下，纵坐标不动，逐帧天然同步。⚠️ 原比例贴必然比
# 窗口天然含住的那一条宽（这条片子多 173px，盖的是真球场）——2026-08-28 试过
# 缩小和不贴两条路，都被否掉（一条读不出字、一条名字被裁），最后按账号所有者
# 定的「按板的实际大小裁、原比例贴、纵坐标不变」回到这里。全部实测和被否掉的
# 两条记在 cut_segment 的回贴那一段。
...
#: 比分数字的字体和字号。**两条线共用同一支**——账号所有者 2026-08-18 拿
...
#:
#: **换哪一支是量出来的**：把参考图那个粗「6」和细「3」的墨迹二值化、归一到
#: 200×200，跟手上每一支字体逐个算 IoU——
#:     粗 6：Noto Sans CJK SC **0.719** ｜ 我们封面在用的 NotoSansSC-sub 0.716
#:           ｜ Barlow Condensed SemiBold 0.689
#:     细 3：Noto Sans CJK SC **0.640** ｜ NotoSansSC-sub 0.641
#:           ｜ Barlow Condensed SemiBold **0.440**
#: 也就是说 **Noto Sans CJK SC 和封面那支是同一套设计**（差 0.003），
#: 顶栏和封面从此不会再是两种数字。
#:
#: ⚠️ **不用 `Noto Sans`（fonts-noto-core 那支）**，尽管它在粗 6 上更高
#: （0.790）：① 细 3 只有 0.581，两档合起来不如 CJK SC；② `match-reel.yml`
#: **只装了 `fonts-noto-cjk`，没装 `fonts-noto-core`**，写上去 libass 会静静
#: 换一支——而"换掉了"和"就是这么渲的"在成片上长得一模一样。
#:
...
# ⚠️⚠️ **而这条路默认不走**：账号所有者 2026-08-29 在两条路里选了第二条——
# 成片不带音乐，上传视频号／抖音之后在**平台自己的曲库**里加（不用重渲、
# 不用自己找授权）。烧进片子（`spec.music`）是**认领过才走的例外**，
# 判据 test_背景音乐默认走平台曲库不烧进片子。实现留着不删，它是那条例外的
# 现成能力。整段账在 CLAUDE.md「背景音乐走平台曲库，不烧进片子」。
MUSIC_GAIN_PCT = 4.0
...
#     ⚠️ 这张表整张是那一轮量的，末行那个 `slow/crf18` 记的是**当时**成片那一步
#     的参数；成片 preset 2026-08-14 已经换成 `medium`（见 `FINAL_PRESET`）。
#     这里不重量：这一行只是当年给中间段定档时用的上界参照，结论（ultrafast/12
#     最好）跟成片走哪个 preset 无关。
#
...
# 成片那一步的参数。**crf 一个字都不许动；preset 是另一件事，2026-08-14 按实测
# 调过一次（slow → medium）。**
#
# 两件事各管各的，别混成「编码参数」一坨：**preset 决定编码器花多少时间去找
# 省比特的编法，crf 决定留多少画质**。所以动 crf 是动画质，动 preset 是拿体积
# 换时间——下面分开记。
#
# ## crf 18：不许动
#
...
#
# ## preset：slow → medium（2026-08-14）
#
# 来路：一趟 render 862s，其中 render 步 747s（87%），而**「烧字幕+成片」一步
# 就是 423.7s——占整趟 49%**，这条线上最大的一块，偏偏它从来没被量过。
# `slow` 当年不是拍的，它有一个真实约束：成片要塞进 git（100 MiB 服务端硬拒），
# 比特省一点就多一分进仓库的机会。**2026-08-13 起成片一律走 Release 附件
# （单个 2 GB、公开仓库下载不计流量额度），那个约束没了**，所以这笔账重算一遍。
#
# 量出来的。四核沙箱、**真素材**，照真实流水线的形状走：源片 → 裁 3:4 → 放大到
# 1080×1440 → `ultrafast/crf 12` 出中间产物 → 烧一份真 ASS 字幕 + crf 18 出成片。
# 时间取三趟（样本 ② 两趟）最小值；SSIM/PSNR 拿**成片对它自己那份输入**量
# （也就是加了字幕的中间产物——比对源片更干净地量出这一步丢了什么）：
#
#   样本 ① 多伦多看台实拍 16s / 25fps —— 人群，高细节高运动，最难编的那一头
#     preset      编码       体积      vs slow     SSIM       PSNR
#     slow      32.18s   14.32 MiB       —      0.990284   46.580   ← 改前
#     medium    19.81s   14.55 MiB    +1.6%     0.990254   46.536   ← 改后
#     fast      16.28s   15.08 MiB    +5.3%     0.990364   46.539
#     veryfast   7.91s   13.85 MiB    −3.3%     0.989273   45.731   ← 陷阱，见下
#
#   样本 ② 品牌片尾母版 12.9s / 60fps —— 图形、低运动量，另一头
#     slow      18.90s    2.34 MiB       —      0.998063   52.461   ← 改前
#     medium    13.66s    2.49 MiB    +6.1%     0.998016   52.278   ← 改后
#     fast      10.96s    2.41 MiB    +2.7%     0.997973   51.979
#     veryfast   6.26s    1.76 MiB   −24.9%     0.997346   49.705   ← 陷阱，见下
#
# 解码 + 字幕滤镜的地板只有 1.26s/16s（样本 ①），所以上面这些时间**几乎全是
# 编码本身**——比的就是 preset，没被别的东西稀释。
#
...
# 体积代价落到一条 80 MiB 的片子上是 +1.3~2.5 MiB。
#
# ⚠️ **不要再往快里推，`veryfast` 是个陷阱，而它伪装成「又快又省」**：同样
# crf 18，它出来的文件比 `slow` **还小**（−3.3% / −24.9%）。同一个 crf 下 preset
# 越快只会越大——**它反而变小，就说明它根本没在同一个画质上编**，PSNR 跟着掉了
# 0.85 / 2.76 dB。**光看时间和体积会把这读成两头都赚，只有量 SSIM/PSNR 才看得
# 见**。`fast` 是这条坡的起点（样本 ② 已经掉 0.48 dB，体积却只省 2.7%），
# `medium` 是最后一档「快了但量不出画质差」的。
# 判据在 test_成片preset按实测定在medium不许再往快里推。
#
# ⚠️ 这一档只管**成片**这一次编码。分段和封面那两个中间产物走 `PART_*`
# （`ultrafast`/`crf 12`），两者不是一回事，别互相搬——那边的道理写在 `PART_*`
# 上面，是「马上要被重编，所以省比特没用」，跟这里的账完全不同。
FINAL_PRESET = "medium"
FINAL_CRF = "18"
...
# `test_估体积的码率只许按实测往上调`。
#
# ⚠️ **上面这一串码率全是 `preset slow` 时代量的**，而 2026-08-14 起成片走
# `medium`（见 `FINAL_PRESET`）——同样的 crf，medium 比 slow 大 1.6~6.1%，所以
# 这个常量对新片子会**偏低 2~6%**。**故意不跟着往上调**：往上调要按「实测」，
# 而我手上只有按样本推出来的比例，不是任何一条真成片量出来的数——这个仓库为
# 「没量过的推断写进注释，之后每个人都拿它当判据」栽过。下一条 medium 成片
# 渲出来，照上面那串的口径量一次再决定动不动它。眼下不动是安全的：估低了
# 已经不会再炸 push（路只有一条：Release）。
#
    ...
    if scorebox is not None:
        ...
        if _scorebox4(scorebox) is None:
            raise ReelError(
                ...
                f"y0<y1、全为非负数——probe 的 scorebox_guess 直接抄），"
                f"拿到的是 {scorebox!r}。")
...
def cut_segment(source: Path, seg: Segment, dest: Path, source_w: int,
    ...
    if seg.fit == "square":
        ...
        box = seg.score_inset
        if box and x <= box[0]:
            # 窗口左缘没越过板左缘 → 整条板本来就在画面里，回贴反而会
            # 叠一层重影。跳过要出声：不然「贴了」和「不用贴」在日志上一样。
            print(f"    [score] {seg.start:.1f}s 段窗口左缘 {x} ≤ 板左缘 "
                  f"{box[0]}，整条板本来就在画面里，跳过回贴")
            box = None
        if path:
            ...
            if box:
                # **记分条回贴**（带式专用，parse_segments 把非带式拦在门外）：
                # 同一帧劈两路——[wm] 照常裁居中窗口铺进画面带；[wb] 按
                # scorebox 抠整条板、乘同一个缩放比（VIDEO_W/CROP_W）、
                # 贴回画面带左下。竖向不用换算落点以外的任何东西：CROP_Y=0
                # 且两路同比缩放，贴上去的每一行和主窗口的同一行**逐像素
                # 对齐**（y=BAND_TOP+round(y0×比)），横向残条则被整条板盖住
                # （窗口左缘 x > 板左缘 x0 时，残宽 (x1−x) < 贴宽 (x1−x0)，
                # 恒成立）。取自同一帧，比分逐帧天然同步。
                # 慢放的 setpts 放在 overlay 之后：两路都还是源片时间轴，
                ...
                #
                #   ① 缩到残条宽（不多占一寸球场）→ 板只有画面宽的 27%，
                #      按手机尺寸（360px 宽）渲出来是一条读不出字的深色块，
                #      账号所有者管它叫「马赛克」
                #   ② 干脆不贴（让转播的板按窗口原样露出来）→ 比分是清楚了，
                #      但名字被画面左缘裁掉，「比分板没展示全」
                #
                # 所以回到原比例。⚠️ 它必然比残条宽 `(x - x0) × 比例`
                # （这条片子是 173px＝画面宽的 16%），那一截盖的是真球场——
                # **这是认领过的代价**，不是没量过。能少盖的只有一处，就是
                # `x1` 要按**这一段板的真实右缘**写（美网每打完一盘 +38px，
...
# ⭐ 2026-09-03（review 路线 ⑥ 第二刀）：TTS 只有一份出处 `tennislive.video.tts`
# ——edge-tts 重试的那整套账（2026-08-13）和它的注释原样搬过去了，这儿只留
# 名字：判据（tests/test_reel_narration.py）按 `reel._EDGE_BACKOFF` /
# `reel.tts_one` / `reel._tts_one_uncached` 认，别处也这么调。
from tennislive.video import tts as _tts  # noqa: E402

_EDGE_BACKOFF = _tts.EDGE_BACKOFF
_EDGE_FATAL = _tts._EDGE_FATAL
_EDGE_RETRY = _tts._EDGE_RETRY
_exc_chain = _tts._exc_chain
edge_tts_verdict = _tts.edge_tts_verdict
_tts_content_key = _tts.tts_content_key


def _tts_one_uncached(text: str, path: Path, voice: str, rate: str,
                      pitch: str = "+0Hz", style: str = "", styledegree: str = "",
                      lead_pause: float = 0.0, *, error=None) -> list[dict]:
    """真正跑合成的那半截（`tts_one` 的缓存外逻辑）——失败抛 ReelError。"""
    return _tts._tts_one_uncached(text, path, voice, rate, pitch, style,
                                  styledegree, lead_pause, error=ReelError)
...
def tts_one(text: str, path: Path, voice: str, rate: str,
    ...
    ⚠️ `synth=_tts_one_uncached` 按**名字**在调用那一刻取——判据打桩的就是这个名字。
    """
    return _tts.tts_one(text, path, voice, rate, pitch, style, styledegree,
...
#
# ⭐⭐ 2026-08-15：**系数按 Azure 重拟合了一次，而漂的不是模型，是后端。**
# 下面那串「⚠️ 又低估了 1.5/1.63/1.75/2.07 秒」的加注一路把 `SPEECH_EST_ERR`
# 从 1.44 顶到 2.07，每一条都写着「中位数仍贴近 0，不是模型系统性偏差」——
# 这天中位数**走到了 −0.40**，正好撞上判据那条 `abs(mid) <= 0.4`。
#
# 按 `render.json` 的 `narration_backend` 把 718 段实测拆开，答案一眼可见：
#
#     backend=azure    n=580   中位 **−0.48**   均值 −0.52   最坏 1.93
#     没记 backend     n=138   中位 **+0.04**   均值 +0.05   最坏 1.42
#
# 也就是说**老系数对 edge-tts 一直是准的**（中位 +0.04），它只是从来没对
# Azure 校过；而 Azure 现在占了 81%，所以池化的中位数被拖过了线。
#
# ⚠️ **不是音色的锅，是后端的。** 同一把 `YunjianNeural`、同样 `+6%`：
#
#     edge-tts  YunjianNeural  n= 37  中位 **+0.13**
#     azure     YunjianNeural  n=542  中位 **−0.49**
#     azure     YunxiNeural    n= 38  中位 −0.43
#
# 同一个音色跨后端差 0.6 秒，而跨音色只差 0.06 秒——**控制变量做完了才敢动
# 系数**。这条正是上面 2026-08-11 那段猜到的（「Azure 天然比 edge-tts 快，
# 落差会随句子变长而放大」），只是那时只有一条样本，现在有 580 条。
#
# **拟合口径**：只取 `narration_backend == "azure"` 的 580 段最小二乘，
# 解出 `0.1655·字 + 0.3213·句读 + 0.4543`，取整成下面三个数之后
# **中位 −0.004、最坏 1.64**（老系数是 −0.48 / 1.93）。
#
# ⚠️ **为什么不是「按两个后端各留一套系数」**：`speech_seconds` 只有一个用途
# ——`--dry-run` 预判**这一段到了 runner 上装不装得下**，而 runner 走的就是
# Azure（`match-reel.yml` 注了 AZURE_SPEECH_*，`tts_one` 第一句就是
# `if azure_tts.available()`）。本地 `--check-narration` 跑的是**真** edge-tts，
# 不读这个估算。也就是说 **edge-tts 那套系数没有消费者**，留着只是好看。
# ⚠️ 而这次白烧过一趟 render 的正是这个方向：本地估「余量 3.74s、门槛 4.0s、
# 过」，Azure 上是 4.20s 当场撞哑场闸（`wangxiyu-timofeeva`，2026-08-15）。
# 系数偏长 → 以为说得满 → **少报哑场**，那是错的那一头。
#
...
# 本意小幅调宽，不是把闸松掉。
# ⚠️ 2026-08-09：`swiatek-kostyuk` 第 3 段（39 字 5 处标点）真语音只说 7.46s，
...
# 所以同样按闸的本意小幅调宽。
# ⭐ 2026-08-15：**这个数第一次往下走**（2.07 → 1.64）。上面那一串每次调宽都
# 是在给一个没对 Azure 校过的模型让路；系数按 Azure 重拟合之后（见它上面那段），
# 580 段实测最坏就是 **1.64**，带子按它收回来。**这不是把闸调松的反向操作，
# 是模型准了之后带子自然收窄**——「悬」那一档窄了，说明可信的区间大了。
# ⚠️ 2026-08-11（同一天，同一条片子，改写之后）：`cincinnati-story` 重做成
# 「125年历史全景」时第 3 段整句改写（补充了新的赞助方留驻25年那半句），
# 生产渲染真撞过一次哑场闸——本地`--check-narration`走的是 edge-tts（沙箱没
# Azure 密钥），而生产渲染实际走 Azure（`[TTS] Azure（风格 无）`），同一音色
# 同一语速，Azure 真实合成比 edge-tts 快 3%~7%（这个仓库第一次系统性记录到
# 这个后端差异，不是「音色名写错」那类旧坑）。补长这句话重渲之后（60字8处
# 标点），真语音 11.64s，比离线估的 13.71s 短 2.07s——比上一条记录又宽了
# 0.32s。这不是同一段话变糟了，是**系数本来就是按 edge-tts 校的**，Azure
# 天然比它快，落差会随着句子变长而放大。493 段实测中位数仍贴近 0（不是模型
# 系统性偏差），按闸的本意调到刚好盖住这条。
# ⭐⭐ 2026-08-31：**这次调宽的原因和以上每一次都不同——不是后端差，是模型缺
# 一维。** `chwalinska-townsend-us-open-2026-r1` 第 10 段（66 字 8 处标点）真语音
# **16.15s**，离线估 13.97s，差 **+2.18s**。而同一批 1979 段实测的中位误差是
# **+0.006s**——模型整体准得不能再准，超出带子的**只有这一段**（第二坏 1.64，
# 正是上一次定带子用的那条）。
#
# 差在哪：这一句里堆了九个数（三十六次／二十六分／十一次／六分／二十九比十／
# 三比四），实测 **0.245 秒每字**，而模型用的是 0.166。**`speech_seconds` 只看
# 字数和句读数，看不见「这一段有多少个数字」**——数字念出来比同样字数的白话慢
# 得多，而这一维不在模型里，**重新拟合系数抓不住它**（拟合只会把所有句子一起
# 往慢里挪，把 1978 段准的拖偏去迁就 1 段）。
#
# ⚠️ 没有改那句旁白，虽然那样也能让它落回带子：这几个数正是这条片子的落点
# （封面钩子讲的就是上网那一组）。上面 2026-08-11 那条写过同一件事——
# 「为了迁就一个离线估算去改一句没毛病的旁白，方向反了」。
#
# ⚠️ 代价照实说：带子宽了 34%，dry-run 判「估得再乐观也装不下」的门槛跟着松，
# 更多段会落进「悬」那一档。**而「悬」正是这次该出现的提示**——这一段当时报的
# 是余量 2.03s（在 1.64 的带子外，不提示），于是我没去跑 `--check-narration`，
# 白烧了一趟 render 才知道它超。带子盖住真实误差，那趟本来可以省下。
SPEECH_EST_ERR = 2.20
...
APPROVED_LOW_RES_SOURCES: dict[str, int] = {
    ...
    # 网球为什么要安静（crowd-noise-hindrance）：账号所有者 2026-09-25「分辨率不够没关系的，
...
def _is_chinese_row(row: str) -> bool:
    """这一行是中文字幕行吗——汉字占到非空格字数的一半以上就算。

    分中英是因为**「有空格」在两种行里根本不是一回事**：英文行的空格是词与
    词之间的分界，libass 在那儿断开读起来是对的；中文行的空格多半是
    `_quote_display` 把中文标点换来的（见那儿的注释：换成空格而不是删掉，
    否则两句会糊成一坨），在那儿断开是**把一句中文从中间劈开**，谁都没要求过。
    """
    solid = [ch for ch in row if not ch.isspace()]
    if not solid:
        return False
    return sum(1 for ch in solid if "㐀" <= ch <= "鿿") * 2 >= len(solid)


def quote_overflow_rows(spec: dict) -> list[tuple[int, str, float, int, bool]]:
    """`quote` 里一行装不下的那些行：`(第几段, 那一行, 宽度, 字号, 是不是中文行)`。

    **不看豁免表**——豁免表自己的自检要建在它上面，看了就成了一盏恒真的绿灯。
    """
    out: list[tuple[int, str, float, int, bool]] = []
    for i, seg in enumerate(spec.get("segments") or []):
        ...
        for item in raw:
            ...
            for index, row in enumerate(shown.split("\n")):
                ...
                if not row:
                    continue
                size = ass_row_size(shown, index)
                chinese = _is_chinese_row(row)
                # `_sub_width` 是按主读行标定的（汉字 68、数字和西文 78）；
                # 双语的英文参照行整行 46，按字号等比折算到同一个单位上。
                width = _sub_width(row) * size / _ASS_SIZE
                if width <= _SUB_MAX:
                    continue
                if " " in row and not chinese:
                    continue
                out.append((i + 1, row, width, size, chinese))
    return out


def _quote_lines_fit_the_frame(spec: dict) -> None:
    """`quote` 里手写的每一行，都要装得下一行——中文行尤其。

    **第一回（2026-08-22，fils-cobolli）：横着溢出。** 冷开场双语字幕的中文行
    手写成 24 字（`_sub_width` 23.4，上限 16），libass 对没有空格的中文没有
    断点，整行贴着左右 150px 安全边距画出去，而且**一处报错都没有**——
    `--dry-run`、`--check-narration`、`check_reel_landed`、全量测试全绿，
    是把成片从 Release 拉回来逐帧核对才看见的，白付一整趟渲染。

    **第二回（2026-09-20，jovic-stearns）：竖着溢出，而第一回那道闸正好放它过去。**
    账号所有者看完片子说「**两行英文字母、两行中文字幕的最下面的中文字幕没有
    显示全**」。那一行中文是 `伊娃 约维奇卫冕了 12 个月前的这座冠军`
    （宽 19.4，同样超着），可它**带空格**——`_quote_display` 把 `·` 换成了
    空格——于是第一回写下的那句「带空格的行 libass 自己断得开，不拦」把它
    放行了。断是断开了，**断出来的第二行把整块字幕顶到了画布外面**：四行从
    1240 往下长到 1488，画布只有 1440（烧帧量的：末行墨迹 1421–1439，本该
    到 1466）。

    ⚠️ **「libass 断得开」这句话本身没错，错在它对中文行不成立。**
    英文行的空格是词与词之间的分界，断在那儿是对的；中文行的空格是标点换来的，
    断在那儿是把一句话从中间劈开，还多长一行。所以豁免收窄成「**不是中文行**
    的带空格行」，见 `_is_chinese_row`。

    ⚠️ **同一轮在渲染那头也补了一道**（`explainer.bilingual_bottom_margin`）：
    双语字幕改成下锚，行数只往上长，底下那行中文钉死在原地——**再多几行也切
    不掉它**。两道一起才算解决：闸管「别写成这样」，下锚管「万一还是这样了，
    也不许把字切掉」。存量 spec 和自动产的 spec 走不到闸，靠的就是下锚那一道。

    ⚠️ **已发的不重渲**（消息发出去收不回来），挂进 `_LEGACY_QUOTE_OVERFLOW`，
    只许减不许加。
    """
    slug = str(spec.get("slug", ""))
    if slug in _LEGACY_QUOTE_OVERFLOW:
        return
    for seg, row, width, size, chinese in quote_overflow_rows(spec):
        why = ("中文行里的空格是 `_quote_display` 把标点换来的，libass 会在"
               "那儿折行——折出来的那一行往下长，把整块字幕顶出画布底边，"
               "最下面那行中文就被切掉了"
               if " " in row else
               "libass 对没有空格的行没法自动换行——超宽的行会贴着左右 "
               f"{_ASS_MARGIN_H}px 安全边距整行画出去")
        raise ReelError(
            f"第 {seg} 段 quote 里手写的这一行装不下一行：\n"
            f"  「{row}」（{len(row)} 字，按 {size}px 折算宽度 {width:.1f}，"
            f"上限 {_SUB_MAX}）\n"
            f"{why}，而且**渲染不报错**，要把成片拉回来逐帧看才发现。\n"
            "两条改法：把这一条拆成两条 `{\"at\": ..., \"text\": ...}`"
            "（各自钉在真实时刻上，本来就更同步），或者把这一行写短。")


#: 已经发布、不会重渲的 8 条——**只许减不许加**，自检在
#: `test_quote超宽豁免表只许减不许加`：表里每个 slug 必须真的存在、
#: 而且真的还超着，写错一个名字豁免就成了一盏恒真的绿灯。
#: ⚠️ 后 4 条是 2026-09-20 闸收窄到「中文行的空格不算断点」之后扫出来的
#: （见 `_quote_lines_fit_the_frame` 那段「第二回」）——它们一直这么发着，
#: 只是之前那道闸看不见。成片里中文被折成两行，靠同一轮改的下锚兜住不被切掉。
_LEGACY_QUOTE_OVERFLOW = frozenset({
    "alexandrova-sabalenka", "gauff-kostyuk-cincinnati-2026-qf",
    "osaka-mertens", "swiatek-kostyuk",
    "jovic-stearns-guadalajara-2026-final",
...
#:   要放大 2.00 倍。
#: - `zhiyenbayeva-bouzas-bjk-cup-2026`：2026-09-23 比利·简·金杯总决赛 1/4决赛
#:   第一场单打。推片窗口内官方图**还没发**：金杯官网直播页头图换成了第二场的
#:   普京采娃，WTA photo-resources 和 AP 都查空，西语媒体转载的一张是马德里站
...
#: - `wang-prozorova-singapore-2026-qf`：2026-09-25 新加坡 1/4决赛。推片后账号
#:   所有者要换封面，而她这一场的官方实拍还没发（WTA 图库、AP 都只有赢家
#:   普罗佐罗娃那张），本站前两轮那两张官方图都已用过（「这张用过了」「这张
#:   太丑了」）。我把赢家本场官方图、她美网官方图、本场抽帧、等图四条渲成
#:   预览摆出来，账号所有者选了抽帧，并点名要「输球后的表情图」（289.0s
#:   走向网前握手那一帧）。源片 1920×1080，放大 1.33 倍。
...
def cover_photo_problem(spec: dict) -> str | None:
    ...
    if not image:
        ...
        return (
            ...
            "⚠️ **拦住不是目的，目的是给读者一张好图**（账号所有者 2026-08-17："
            "「不开工不是目的，目的是要有好的图片给人好的体验，"
            "**所以想办法解决而不是不做**」）。查空之后按这个顺序办，"
            "**别直接跳过这场**：\n"
            "  ① **确认是「还没发」还是「真没有」**——夜场的图要等约 24 小时"
            "（打完已过午夜 → 归次日那一辑 → 次日晚上才发）。还没发就**等**："
            "片子可以先渲，封面最后定，图一落地换上去再推\n"
            "  ② **换个查法再来一遍**——同一个赛事有好几位摄影师、好几套文件名"
            "（`CincinnatiOpen_<YYYYMMDD>_` / `<MMDDYY>_DAY-N_` / "
            "`CincyOpen<M.D.YY>`），只认一套会漏掉三分之二；姓能撞的时候"
            "`--player` 要给文件名里的全名写法\n"
...
def music_problem(spec: dict, base: Path = Path(".")) -> str | None:
    ...
    ⚠️ **默认根本不该走到这儿**：账号所有者 2026-08-29 选的是「平台曲库」
    那条路——成片不带音乐，上传视频号／抖音之后在它们自己的曲库里加。
    所以缺 `_license` 的报错要**先把那条路说出来**，别只教人怎么补字段：
    十有八九补字段是错的答案，不加才是。
    """
    ...
    if not str(music.get("_license") or "").strip():
        return ("spec.music 要写 `_license` 认领这首曲子的授权来路"
                "（哪儿来的、什么授权、能不能用在这个号上）。\n"
                "⚠️ 先想一下要不要走到这一步：这条线的默认是**成片不带音乐**，"
                "上传视频号／抖音之后在**平台自己的曲库**里加 BGM"
                "（账号所有者 2026-08-29 选的那条路——不用重渲、不用自己找授权）。\n"
                "真要烧进片子，背景音乐会被平台的音频指纹扫到，"
                "来路必须留在 spec 里。")
...
def render(spec: dict, outdir: Path, *, voice: str, rate: str,
    ...
    with stage("拼接"):
        ...
        # 「中间产物要花的是比特，不是时间」），成片那一步另走 `FINAL_*`
        # （`medium`/`crf 18`，2026-08-14 起 preset 从 `slow` 换过来）。
        #
    ...
    # 片尾页上印着「网球时差」四个大字 + 「时差归我，好球归你」，
    # 而口播是「关注网球时差，时差归我，好球归你。」——多出来的
...
TOPBAR_SCORE_FONT_RESET_ASS = (
    rf"{{\b0\fn{TOPBAR_BODY_FONT}\fs{TOPBAR_BODY_SIZE}}}")
#: 赢下那一盘的数字加粗——账号所有者 2026-08-29「赢的一盘的加粗」。
#:
#: ⚠️ **这是真渲量过的，不是照着 `TOPBAR_WINNER_ASS` 那段注释推的。** 那段写着
#: 「libass 既解不到完整的 Bold face 也不给合成」——**那句话对中文名成立，对数字
#: 不成立**。同一条命令（带 `fontsdir=assets/fonts`，和 `topbar_filtergraph`
#: 一样）分别量：
...
#: 抢七小分：**裸数字、印在大分的右上角**（账号所有者 2026-08-31 连着三句：
#: 「不要带()」→「小分不带括号」→「小分在大分的右上角」）。ASS 没有行内上标
#: 标签——`\fs` 小字坐在基线上是右**下**角——所以「小而高」做进了字形本身：
#: `TL Score` 里的上标码位（U+2070/00B9/00B2/00B3/2074–2079）是
#: `tools/build_fonts.py` 自己合成的（0.65 缩放、顶对齐抬升 261 单位），
#: 顶栏只要写 `⁵` 这个字符，字体自己把它摆到右上角。
#: ⚠️ **只能写合成过的那十个码位**：libass 碰到缺字会走 fontconfig 回退到
#: 随便哪支有这个码位的字体（DejaVu 就有 ⁵），不报错、只是样子悄悄换掉——
#: 判据 `test_TLScore的上标数字真的又小又高` 量的就是仓库里那三份 TTF。
#: 采访线 `build_interview_clip._SUP_DIGITS` 是同一张表（Unicode 标准映射，
#: 1/2/3 在 Latin-1 区不在 207x 区，别想当然连排）。
...
def colorize_topbar_score(line: str) -> str:
    ...
    for token in tokens[lead:]:
        ...
        if tb:
            ...
            # 的附注，不是比分本身；标签粘连，不显式复位的话同一个数字会
            # 时粗时细——**同一个元素按谁赢了渲成两个样子**，比一直粗或一直细
            # 都糟。⚠️ 加了 Light 之后这一下也不能省。
            # ⚠️ **裸数字、右上角**（2026-08-31，见 `SUP_DIGITS` 的注释）：
            # 写的是字体里合成好的上标码位，小和高都在字形里，这儿不用再
            # 动字号或颜色——颜色跟着前面 `TOPBAR_RESET_ASS` 的正文色走，
            # 和封面比分板 `<sup>` 继承数字颜色是同一个口径。
            piece += f"{TOPBAR_SCORE_PLAIN_ASS}{tb.translate(SUP_DIGITS)}"
