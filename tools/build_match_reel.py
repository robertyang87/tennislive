...
#: 抢七小分的字号——**裸数字不带括号**（账号所有者 2026-08-31，两遍：先指着
#: 美网官方图说「不要带()」，我在顶栏保留括号并说明了理由（ASS 没有行内上标），
#: 他又来一句「小分不带括号」——重申就是决定）。ASS 做不了上标，靠**字号小一
#: 大截 ＋ 压暗到连字符那一档**让它读成注脚：40px 的比分旁边一个 26px 的暗
#: 数字，和下一盘满号的数字分得开。26 是渲了 40（括号版）/28/26/24 四档在
#: 深色条上并排比出来的：24 起在 H.264 压完开始发虚。
TOPBAR_TB_SIZE = 26
#: 小分整段的起始标签：常规字重 ＋ 小一档 ＋ 连字符那支暗色。之后要跟
#: `TOPBAR_RESET_ASS`（把颜色还给正文）再 `TOPBAR_SCORE_FONT_RESET_ASS`
#: （把字体字号还给正文）——ASS 标签粘连，少复位一样就漏一样。
TOPBAR_TB_ASS = rf"{{\b0\fs{TOPBAR_TB_SIZE}}}{TOPBAR_SETDASH_ASS}"
...
def colorize_topbar_score(line: str) -> str:
    ...
    for token in tokens[lead:]:
        ...
        if tb:
            ...
            # 同一个数字会时粗时细——**同一个元素按谁赢了渲成两个样子**，
            # 比一直粗或一直细都糟。⚠️ 加了 Light 之后这一下也不能省。
            # ⚠️ **裸数字不带括号**（2026-08-31 重申，见 `TOPBAR_TB_SIZE` 的
            # 注释）：ASS 没有行内上标，用「字号小一大截 ＋ 连字符那支暗色」
            # 让它读成注脚。颜色和字号都动了，所以要连着两次复位。
            piece += f"{TOPBAR_TB_ASS}{tb}{TOPBAR_RESET_ASS}"
