...
#   edge-tts 是唯一通道**（`explainer.py` 里 `azure_tts` 出现 0 次、
#   `explainer.yml` / `interview-clip.yml` 里 `AZURE_SPEECH` 出现 0 次）。
#   那儿比这儿更该修，只是不在这次的改动范围里——**没修，别当成修过了**。
#
...
def _tts_one_uncached(text: str, path: Path, voice: str, rate: str,
    ...
    # 盖掉，拼不出一个坏音频。
    marks: list[dict] = []
    for attempt in range(len(_EDGE_BACKOFF) + 1):
        try:
            marks = asyncio.run(one())
            break
        except Exception as exc:                       # noqa: BLE001
            retry, why = edge_tts_verdict(exc)
            if not retry or attempt == len(_EDGE_BACKOFF):
            ...
            pause = _EDGE_BACKOFF[attempt]
            print(f"[TTS] edge-tts 没送到（{why}），{pause:.0f}s 后重试第 "
                  f"{attempt + 1}/{len(_EDGE_BACKOFF)} 次：「{text[:18]}…」")
            time.sleep(pause)
