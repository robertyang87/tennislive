...
扛住的那一刻，讲夺冠就找捧杯」。这条一直是人工终审（机械工具 pick_cover_frames
只能按脸/清晰度初筛，挑不出「情绪」）。MiniMax M3 有视觉（probe_minimax_vision
实测确认），这一环能自动化了。

⚠️ 只做「从候选里挑」，不生成候选——候选还是 pick_cover_frames.py 机械初筛
出来的（脸正、上身直立、表情清楚）。视觉模型做的是最后一跳：按情绪打分选。
