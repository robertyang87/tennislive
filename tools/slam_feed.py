...
⚠️ 轮次按 `roundCode` 转成对外写法（和 `spec_wording.round_display` 同一套
词汇：第一轮 … 第四轮 / 1/4决赛 / 半决赛 / 决赛），资格赛写「资格赛第N轮」。
⚠️ **R16 这一档写「第四轮」不写「1/8决赛」**：两个写法账号所有者都点了名
（2026-09-08「应该用美网第四轮或者用美网 1/8 决赛」），而这条路知道自己在
大满贯的 128 签上，第几轮算得出来——`round_display` 那条路只拿到一个轮次名，
算不出来，所以它给的是「1/8决赛」。两条路各按自己知道的东西写。
"""
...
# roundCode → 对外写法（账号所有者 2026-09-08 翻回来：第四轮 / 1/4决赛 /
# 半决赛 / 决赛，不写 N 强）。⚠️ 这是**第二份**轮次表，`spec_wording`
# 那份认的是中文轮次名、这份认的是大满贯的 roundCode——键不同所以没合并，
# 但**输出词汇必须一致**，判据钉在 `test_大满贯那张轮次表和措辞闸对得上`。
