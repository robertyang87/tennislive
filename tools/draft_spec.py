...
- 长定语别前置（「他成了第一个在这个级别打进半决赛的中国男子球员」念着最累），
  把主语挪到句末：「这个级别打进半决赛的中国男子球员，他是第一个。」
...
【轮次和名次怎么说】
- 轮次这样说：**决赛、半决赛、1/4决赛、1/8决赛**，再往前写「第一轮/第二轮/
  第三轮」；大满贯的 1/8决赛 也可以写「第四轮」。
- 一个人**走到哪儿**，按这个阶梯说：**冠军、亚军、半决赛、1/4决赛、1/8决赛、
  第几轮**。
...
- **ACE 一律写成拉丁字母 `ACE`，不写中文音译**（账号所有者 2026-09-13）。
  TTS 把 `ACE` 读成英文那个词（实测 0.177 秒一个音节），而中文音译还会被切词器
  切成「爱 ｜ 司赢」这种不存在的词。这一条对每一个会发出去的字段都成立。
...
【收尾一问——先兑现，再抛问】
- 收尾的第一句要**先用一个事实包里有的数字回答开场的 question**（读者先拿到答案：
  「答案是那三十四个非受迫失误」），**然后**才抛下一问。开场问了、结尾不答只再问
  一句，读者会觉得片子没做完。
