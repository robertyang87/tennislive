...
用法：
    ...
    python3 tools/fetch_atp_cover_photo.py --site cincinnatiopen.com \\
        --match-date 2026-08-16 --limit 30 --list
    python3 tools/fetch_atp_cover_photo.py --site cincinnatiopen.com \\
...
def run(site: str, match_day: date, pick: str, out: Path | None,
    ...
    if not out:
        notes.append("—— 候选（--pick 用文件名里的一段挑）——")
        for url, post in hits[:limit]:
    ...
    chosen = [u for u, _ in hits if pick and pick.lower() in u.lower()]
    ...
    if len(chosen) > 1:
        return 1, notes + [f"--pick {pick!r} 撞上 {len(chosen)} 张，写细一点："
                           + "、".join(c.rsplit("/", 1)[-1] for c in chosen)]

    blob, w, h, label = best_variant(chosen[0])
