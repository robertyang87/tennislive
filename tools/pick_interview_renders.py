...
⚠️ **只放行「终审补齐了」的 spec。** render 那头有三道人工编辑闸
...
不抄第二份——写两处必分叉）；不齐的列成「等终审补齐」打到 stderr，
...
`--stale` 拿 `at` 反查「投出去超过 N 小时还没有 render.json」的条目——
...
待 dispatch 的 slug**；「等终审补齐」走 stderr，不混进这份名单。
"""
...
# 别往那个模块顶层加第三方 import）。
from build_interview_clip import _LEGACY_NO_OPENING, _NO_TAKEAWAY_LEGACY  # noqa: E402
...
def missing_for_render(slug: str, spec: dict) -> list[str]:
    ...
    **判定必须和闸同一个口径**（含豁免表）：这儿判「齐了」而闸判「不齐」，
    ...
    if spec.get("transcript_verified") is not True:
        missing.append("transcript_verified（转写还没人工核对）")
...
def todo_slugs() -> tuple[list[str], list[tuple[str, list[str]]]]:
    """→ (该 dispatch 的, [(还差终审的 slug, 缺什么)])。

    两份都不含「已 render」和「已 dispatch」的——前者有产物了，后者正在路上
    （或该由 `--stale` 去追问，不该在这儿重复投）。
    """
    done = set(_load_state().get("slugs", []))
    done |= _rendered_slugs()
    ready: list[str] = []
    waiting: list[tuple[str, list[str]]] = []
    for p in sorted(SPECS.glob("*.json")):
        if p.name.endswith(".draft.json") or p.stem in done:
            continue
        try:
...
def main() -> int:
    ...
    for s in ready:
        print(s)
    # 等终审的走 stderr：stdout 第二行起是给 workflow 切的名单，混进去就会把
    # 一条不齐的 spec dispatch 出去——正是这次要修的那个卡死
    if waiting:
        print(f"[等终审补齐] {len(waiting)} 条（不 dispatch）：", file=sys.stderr)
