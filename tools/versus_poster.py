...
#: ⚠️ **内部线的间距仍然是为小尺寸夸张过的**（单打边线离外框 2.4 单位，
#: 真实是 4.5/36≈12.5% 处）：横版当年就这么画才在 35px 下读得出，转置
#: 保持同一套间距、同一个渲染密度（35.1px ÷ 26.5 单位 ≈ 1.32 px/单位，
#: 横版是 35/26≈1.35）——「长宽比和实际一致」钉的是**外框**，不是把
#: 每条线都按英尺摆（那样 17px 宽里单打边线离边只有 1.9px，糊成双描边）。
SCORE_COURT_ICON_EM = 1.35   # 球场图标的**高度** ÷ 那一行的字号（宽由 viewBox 比例自出）
#: ⚠️ **线宽要按自己的缩放算**：竖版 26.5 单位缩到 1.35em×26px≈35.1px，
#: 1.35 单位 → **1.79px**，和钟的 1.93px 是一套（横版时代是 1.82px，几乎没动）。
...
_COURT_ICON = (
    ...
    '<path d="M4.2 0.7v25.1"/><path d="M11.0 0.7v25.1"/>'           # 单打边线
    '<path d="M4.2 6.7h6.8"/><path d="M4.2 19.8h6.8"/>'             # 两条发球线
