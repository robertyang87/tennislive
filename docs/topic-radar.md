...
| 小红书热榜 | 官方 `edith` 域 404、`www` 域 500，改走今日热榜镜像 | ⚠️ 镜像，20 条 |
| 微信 | 指数拿不到（见下），接的是 24h 热文榜镜像 | ⚠️ 镜像，30 条 |

⚠️ **微信指数就算拿得到也不是发现工具。** 它是「给词查数」——你得先知道要查
哪个词，回答不了「今天什么热」。而 `search.weixin.qq.com/cgi-bin/wxaweb/wxindex`
对无签名请求一律 `{"code":-1,"msg":"process failed."}`，签名要从微信小程序里
取，runner 上拿不到。所以接的是热文榜：它是**标题**榜，反而比一个指数数字
更适合拿来发现话题。
