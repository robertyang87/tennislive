...
### 取样深度：整套东西里最容易出错的一处，栽过两次

**第二次栽得比第一次狠，而且是全局性的。** 频道按时间倒序返回，赛事集中在
一年里某几周，深度不够就一条都扫不到——**和"这家不发采访"长得一模一样**。

第一次是澳网（1 月的赛事，7 月扫 100 条全空）。以为改一个源就完了。
后来顺手把美网从 300 加到 600，从 **11 条变 53 条**，才发现是全局问题：
...
光这五个源就漏了 **222 条**，比我当时全库的 361 条的一半还多。

**新增源时宁可给大。** 多扫几百条的代价（几十秒）远小于漏掉一整届赛事。
这条已写进 `data/oncourt_sources.json` 的 `_depth_note`。
