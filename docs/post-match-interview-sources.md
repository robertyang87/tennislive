...
**顶栏不写比分。** `push.matchup` 的顺序**不保证胜者在前**（`@wta` 的标题按
签位排，照着推过一次，推错了）——把比分插进两个名字中间等于用词序断言谁赢了。
赛事、轮次、谁跟谁已经答完「这是什么比赛」，比分交给字幕。
...
- **顶栏那道绿竖条 `▍`（U+258D）得意黑里没有**，本地是回退到思源黑体才画出来的，
  而回退在 runner 上不保证。`_HEAD_MARK` 内联把字体写死，不赌 fontconfig。
