...
**探空档的模型不能和主校验同一个。** `verify_transcript` 钉死 `small.en`，
英语专用模型对着他加禄语同样只给空白——拿它探「是不是非英语」是自问自答。
`probe_gap_speech` 单独用多语种模型（`GAP_WHISPER_MODEL`，默认 `small`）
把切出来的那几秒重听一遍，**连它判定的语种一起报**，落在 `caption_gaps.md`
并打进日志。
