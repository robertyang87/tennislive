...
- **`shang-trungelliti-us-open-2026-r1`**（美网男单第一轮，2026-08-31 夜场 13 号外场，
  特伦杰利蒂 0-6 7-6(5) 1-6 6-3 6-4 胜商竣程）——**片子本体全部做完并验过**：17 段
  band 版式、比分板逐段现量、旁白拿真语音跑过（16 段全装得下，最长留白 3.21s）、
  小红书正文 800 字。**卡的只有封面实拍，而这一条是查到底的真空。**
  - **照片确实拍了**：Getty 上有，说明逐字对上（`Juncheng Shang reacts against Marco
    Trungelliti … Day Two of the 2026 US Open … August 31, 2026`），署名
    **`Brad Penner / USTA via Getty Images`**——拍的人是 USTA 自己的摄影师。
    但 Getty 用不了：预览 594×594 **带水印**、rights-managed，而美网没有 WTA 那种
    存无水印原图的 CDN。**Getty 只用来把「还没发」和「没拍」分开，不当渠道。**
  - **官方 usopen.org 从头到尾没发过这一场**（不是晚，是没有）：赛后 26 小时里
    9/1 发了 431 张、9/2 又 107 张，本场始终 0；按 `tag?tags=1130`（本场 match_id）
    查到 76 张 photo，落在本场时段之后的**一张都没有**。
  - 根子是覆盖率：**同一比赛日 245 个球员，官方当天 151 张只覆盖 13 个（5.3%）**，
    全是主球场那一档。⚠️ 这条已经写进 CLAUDE.md，**开工前就该跑一次**那个集合运算。
  - ⚠️ **两个会骗人的信号，都栽过**：① 「某位摄影师今天发了几张」不能当先行信号——
    Penner 的当日数从 0 变 2 时，两张都是 2016/2018 的资料图（赛事方在灌瓦林卡回顾包）；
    ② **探相邻帧号找不到未发布的图**——已发的 `f_20260831_BJPB4571`／`BJPB6230`
    返回 200，相邻九帧全 404，CDN 只放 feed 里有的。
  - 其余渠道逐条跑过全空：AP、USA TODAY 每日图集（那两天的网球辑只有大坂造型和名人时尚）、
    WTA `photo-resources`（不收大满贯）、官方新闻接口 200 条零命中、`playergalleries` 404、
    `atptour.com` 连 WebFetch 也 403、ESPN 阿根廷 202 空壳、Infobae 是 fallback logo、
    La Nación 是阿尔卡拉斯、tennis.com 那张拍摄日是 2026-06-05、tennisexplorer 通用头像、
    新浪网球整页没提这场。⚠️ canaltenis 那篇讲这一场的西语赛报**配图署名是
    `©Generali Open`**（基茨比厄尔的红土站）——**署名行才是判据，文件名最会骗人**。
  - **核图判据（下次找到候选直接用）**：商竣程**亮青绿色上衣＋白帽**，特伦杰利蒂
    白衣深裤，**13 号场**，广告板 `J.P.Morgan`／`CHASE`／`Harvey`。封面主体是商竣程
    （中国球员，输赢不管）。
  - ⚠️ **README 上面那条「市场本地化 Bing 新闻搜索」2026-09-02 补试过了，出了图但用不了**：
    `mkt=es-AR` 一次翻出八家阿根廷媒体写这一场，其中 `infodelestero` 那张原图
    **3840×2409**（铺 1080×1440 是 1.67×，连 `_low_res_why` 都不用写）——**打开一看是
    特伦杰利蒂**（白衣＋阿根廷天蓝配色、深蓝帽，`us open` 蓝背景板），另外两家
    （`diariolaopinion` 600×400、`rionegro` 1597×872）是同一张的不同裁切。
    **赢家有图、输家没有**——而这条片子的封面主体必须是商竣程（刷到这条的人是冲他来的）。
    ⚠️ 记下来是因为它是个**会重复出现的形状**：赢家那边的本国媒体几乎必然发图，
    所以「这一场有没有图」和「我们要的那个人有没有图」是两个问题，`mkt=` 那条渠道
    只解决前一个。中文那半边（`mkt=zh-CN`）同一轮是空的，新浪网球整页没提这场。
  - **下一步**：这条**已经过了时效**（美网夜场在 flashscore 里结构性地永远是 `f_2_-1`，
    站得住的边界是「它结束在哪个北京日」＝ 2026-09-01 24:00，早已过）。所以它留在这儿
    **是记录，不是候车**——真要复活得账号所有者点头。README 上面那条「市场本地化新闻
    搜索（Bing 新闻限定 `mkt=zh-CN`／`es-AR`）」这一轮**没试过**，是唯一还剩的渠道。
