...
- **`anisimova-eala`**（辛辛那提 WTA1000 第三轮，2026-08-17，阿尼西莫娃 4-6 6-4 6-2 逆转伊埃拉）
  - 账号所有者 2026-08-18 直接给出这条 YouTube 链接、点名**优先做**，理由是
    「伊埃拉的热度最高」——因此**封面主体是伊埃拉，不是赢球的阿尼西莫娃**
    （`cover.subject: "伊埃拉"`），钩子和推送文案都按她的视角写：
    「决胜盘一度二比零领先\n仍连丢六局告负」。世界第 10 的阿尼西莫娃在决胜盘
    0-2 落后的情况下连赢六局逆转——两人首次交手。真正的分水岭是数据：
    阿尼西莫娃的非受迫失误三盘从 25→13→7 一路收窄，制胜分稳定在 15~16 个
  - ⚠️ **同一批指令里账号所有者还纠正了一条剪辑问题**：「一个球没结束就不要
    切到下一个画面」。第一版按「离线估旁白长度」反推窗口长度，结果好几段的
    `end` 落在源片一个静止比分持续显示的中途，没有验证那个瞬间是不是死球——
    典型的「为了压哑场而在源片连续镜头中途硬切」。**已经整条重做**：全部
    10 段的 `start`/`end` 现在**只取自 `probe.json` 的 `scene_cuts`**（源片
    自己的镜头切点），而不是随便找一个凑数的时刻；旁白改成往长了写（补充
    已核实的事实，而不是砍窗口）来填满这些天然更长的连续镜头。缺一个环节
    都会留下同一个坑，见每段 `_why` 里「这一段的 END 就是源片自己的切点，
    不是我在中途插进去的刀」那句话
  - **片子还没渲染**——`cover.portrait.image` 一直是字面量 `"PENDING"`，卡在
    `cover_photo_problem` 那道闸（和另外两条一样，`probe.json` 是唯一落地的产物）
  - probe 已经跑过：`output/2026-08-18/reel/anisimova-eala/`（6 张缩略图墙、
    逐分／记分板对照表、captions 全在），**10 段**（重做后从 13 段合并压缩）
    spec 全部写完，`--dry-run` 和 `--check-narration`（zh-CN-YunjianNeural +6%）
    都过了——9 段全部装得下、单段最长哑场 3.46s（门槛 4.0s），小红书正文
    674 字也写好了——**唯一缺的就是这一格**
  ...
  - 四类源分别按两个人名查过一遍（2026-08-18 06:2x Z），**这一场一张都没有**：
    - WTA `photo-resources`：两人都没有对得上的
    - AP 通讯社：伊埃拉命中四条、阿尼西莫娃命中两条，全是别的赛事的资料图
      （伯明翰／温网／国家银行公开赛／斯图加特／2025 温网／2025 中网），不是这一场
  ...
  - **接回来怎么做**：把两个文件 `git mv` 回 `specs/reels/`，把
    `cover.portrait.image` 换成真实图片路径（`fit`/`focus`/`focus_y`/`zoom` 按实际
    图片重新量，当前写的 `focus_y: 0.4` 只是占位猜测——**优先找伊埃拉的实拍**，
    她是封面主体），跑一次完整 `mode=render`（这条从来没渲过，不是重渲），
    `check_reel_landed` 0 项不合格后合并 → `mode=push` ＋ `push=true`。
  - ⚠️ **这条已经确认是当前唯一在做的候选**——账号所有者同一批指令里说
    「其他的任务先暂停」，也确认 ATP 那半边有独立任务在做（科博利那条已经
    推送过），这条线只做 WTA。
