...
- **`anisimova-eala`**（辛辛那提 WTA1000 第三轮，2026-08-17，阿尼西莫娃
  4-6 6-4 6-2 逆转伊埃拉）——**2026-08-18 已经不在这个目录里了**，
  账号所有者当面批准破例（「先渲染伊埃拉的」→ 问是否算破例 →
  「破例，先用抽帧渲」），spec 挪回了 `specs/reels/anisimova-eala.json`，
  `cover.portrait` 用抽帧（`frame_at: 270.0`）顶上，`anisimova-eala` 已经
  显式挂进 `tools/build_match_reel.py` 的 `LEGACY_SOFT_COVERS`（注释里写清楚
  这不是常规豁免——**官方图一上线还是要换图重渲**）。封面主体是伊埃拉
  （热度最高），钩子按她的视角写：「决胜盘一度二比零领先\n仍连丢六局告负」。
  这条片子当天渲完之后如果封面还没换成官方图，会被重新 `git mv` 回这个
  目录等图；真等到图落地，按上面「不许拿抽帧」那条硬闸，也要重渲一遍换图。
