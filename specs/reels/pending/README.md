...
- **`tiafoe-tien`**（辛辛那提 ATP1000 第三轮，蒂亚福 6-4 4-6 6-4 逆转
  勒纳·钱，两小时十分）——**2026-08-21 挪出去了**，正是上面几条笔记预计的
  「8/21 前后」那一批：`?search=Tiafoe` 那个接口本身**不索引 alt_text**，
  按 `after=2026-08-19T00:00:00` 拉当天整批、逐条读 `alt_text` 才认出
  10 张带蒂亚福的候选，按四道闸门选中 `CincyOpen8.19.26BJ_510.jpg`
  （回头怒目握拳，2000×1334，赛事媒体库自有摄影师原图，非缩放版）。
  分辨率差一点点没过 1.00 的地板（fill=0.93×），写了 `_low_res_why`
  认领——同一天全部实拍上传都封顶在这个量级，不是漏找了更大版本。
  ⚠️ **顺带在渲染前拦下了一个真实的合规问题**：原 spec 有三段 `quote`
  （网前对话＋两段赛后场边采访）过不了 `test_赛场之上的quote段不许是
  赛后采访`——账号所有者 2026-08-15「赛场之上视频不要赛后采访」那条
  重申覆盖的是**画面**，不只是字幕。两段真正的记者采访（`ATP1000` 台标
  话筒那两段，164.88-200.72s）整段删除；网前打趣那段（149.84-163.0s，
  两球员即兴互动，非记者采访）保留画面，但把逐句双语字幕改成一句中文
  旁白概述，原始引语仍留在 `editorial.human_context.facts`／`_source`
  作为文字信源。改完用 Azure 3.5~6.6% 提速的最坏情形重新核过每段余量，
  没有一段会撞上 4.0s 哑场闸。
  - ⚠️ 顺带修了一个真 bug：`tools/build_match_reel.py` 的
    ...
    ⚠️ 这条修复没有随本次 diff 一起提交——去掉那两段真正的采访 `quote` 之后
    这条 spec 里已经不再触发这条崩溃路径了，谁下次真的需要修它，按上面这段
    描述重现即可。
